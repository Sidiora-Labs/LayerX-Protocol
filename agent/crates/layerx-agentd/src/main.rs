...
use layerx_agentd::human_runtime::{HumanAuthorityBoundary, ProductionHumanOperations, RemoteHumanAuthority, UnifiedAgentOwner};
...
fn parse_hex<const N: usize>(name: &str) -> Result<[u8; N], String> {
    ...
    if value.len() != N * 2 { return Err(format!("{name} has the wrong width")); }
    let mut bytes = [0; N];
    for (index, byte) in bytes.iter_mut().enumerate() { *byte = u8::from_str_radix(&value[index * 2..index * 2 + 2], 16).map_err(|_| format!("{name} is not hexadecimal"))?; }
    Ok(bytes)
}
...
fn human_peers() -> Result<BTreeMap<u32, (String, String)>, String> {
    ...
    for entry in required("LAYERX_AGENT_HUMAN_PEERS")?.split(',') {
        ...
        if fields.len() != 3 || fields[1].is_empty() || fields[2].is_empty() { return Err("human peer map is invalid".to_owned()); }
        let uid = fields[0].parse().map_err(|_| "human peer uid is invalid")?;
        if peers.insert(uid, (fields[1].to_owned(), fields[2].to_owned())).is_some() { return Err("human peer uid is duplicated".to_owned()); }
    }
    if peers.is_empty() { return Err("human peer map is empty".to_owned()); }
    Ok(peers)
}
...
fn verified_limit() -> Result<LimitConfig, String> {
    ...
    let scope = match required("LAYERX_AGENT_HUMAN_LIMIT_SCOPE")?.as_str() {
        "tenant" => LimitScope::Tenant(scope_bytes), "agent" => LimitScope::Agent(scope_bytes),
        "session" => LimitScope::Session(scope_bytes), "capability" => LimitScope::Capability(scope_bytes),
        "counterparty" => LimitScope::Counterparty(scope_bytes), _ => return Err("human limit scope is invalid".to_owned()),
    };
    Ok(LimitConfig { id: LimitId(parse_hex("LAYERX_AGENT_HUMAN_LIMIT_ID")?), name: required("LAYERX_AGENT_HUMAN_LIMIT_NAME")?, scope, ceiling: required("LAYERX_AGENT_HUMAN_LIMIT_CEILING")?.parse().map_err(|_| "human limit ceiling is invalid")?, consumed: required("LAYERX_AGENT_HUMAN_LIMIT_CONSUMED")?.parse().map_err(|_| "human limit consumed is invalid")? })
}
...
fn start_human_owner() -> Result<mpsc::Receiver<Result<(), String>>, String> {
    ...
    let limits = Limits { maximum_frame_bytes: required("LAYERX_AGENT_HUMAN_MAX_FRAME_BYTES")?.parse().map_err(|_| "human max frame is invalid")?, maximum_connections: required("LAYERX_AGENT_HUMAN_MAX_CONNECTIONS")?.parse().map_err(|_| "human max connections is invalid")?, maximum_streams: required("LAYERX_AGENT_HUMAN_MAX_STREAMS")?.parse().map_err(|_| "human max streams is invalid")?, maximum_queued_bytes: required("LAYERX_AGENT_HUMAN_MAX_QUEUED_BYTES")?.parse().map_err(|_| "human max queue is invalid")?, deadline }.validate().map_err(|_| "human LNI limits are invalid")?;
    ...
    let session_secret_path=PathBuf::from(required("LAYERX_AGENT_HUMAN_SESSION_OPERATOR_SECRET_FILE")?);
    if !node_path.is_absolute() || !store_path.is_absolute() || !socket_path.is_absolute() || !session_key_root.is_absolute() || !session_secret_path.is_absolute() { return Err("human daemon paths must be absolute".to_owned()); }
    let node = Client::connect(ClientConfig { endpoint: node_path, handshake: HandshakeConfig { built_interface_version: Version::V1_0, expected_protocol_version: required("LAYERX_AGENT_HUMAN_PROTOCOL_VERSION")?.parse().map_err(|_| "human protocol version is invalid")?, expected_network_id: required("LAYERX_AGENT_HUMAN_NETWORK_ID")?.parse().map_err(|_| "human network id is invalid")? }, limits, reconnect: ReconnectPolicy { maximum_attempts: required("LAYERX_AGENT_HUMAN_RECONNECT_ATTEMPTS")?.parse().map_err(|_| "human reconnect attempts are invalid")?, base_delay: Duration::from_millis(parse_u64("LAYERX_AGENT_HUMAN_RECONNECT_BASE_MS")?), maximum_delay: Duration::from_millis(parse_u64("LAYERX_AGENT_HUMAN_RECONNECT_MAX_MS")?), jitter_percent: required("LAYERX_AGENT_HUMAN_RECONNECT_JITTER_PERCENT")?.parse().map_err(|_| "human reconnect jitter is invalid")? } }).map_err(|error| format!("human node LNI is unavailable: {error:?}"))?;
    let authority = RemoteHumanAuthority::connect(&required("LAYERX_AGENT_HUMAN_AUTHORITY_ENDPOINT")?, required("LAYERX_AGENT_HUMAN_AUTHORITY_BEARER")?, deadline, required("LAYERX_AGENT_HUMAN_AUTHORITY_MAX_BYTES")?.parse().map_err(|_| "human authority bound is invalid")?).map_err(|error| format!("human authority is invalid: {error:?}"))?;
    for (uid, (principal, tenant)) in &peers { authority.registry(&HumanPeer { uid: *uid, principal: principal.clone(), tenant: tenant.clone() }).map_err(|error| format!("human authority readiness failed: {error:?}"))?; }
    let shared_store = Arc::new(Mutex::new(Store::open(store_path).map_err(|error| format!("human store is unavailable: {error}"))?));
    let operations = ProductionHumanOperations::new(authority, node, Arc::clone(&shared_store), &peers, required("LAYERX_AGENT_HUMAN_MAX_PAYLOAD_BYTES")?.parse().map_err(|_| "human payload bound is invalid")?, parse_u64("LAYERX_AGENT_HUMAN_TIMESTAMP_SPAN")?).map_err(|error| format!("human operations are invalid: {error:?}"))?;
    let socket_uid=required("LAYERX_AGENT_HUMAN_SOCKET_UID")?.parse().map_err(|_|"human socket uid is invalid")?;
    let operator_secret=layerx_agentd::config::read_protected_source(&session_secret_path,4096).map_err(|error|format!("human session operator secret is unavailable: {error:?}"))?;
    let session_keys=SessionKeyRegistry::open(session_key_root,operator_secret,required("LAYERX_AGENT_HUMAN_NETWORK_ID")?.parse().map_err(|_|"human network id is invalid")?,socket_uid).map_err(|error|format!("human session key registry is invalid: {error:?}"))?;
    let owner = UnifiedAgentOwner::new(operations, shared_store, &peers, vec![verified_limit()?],session_keys).map_err(|error| format!("human owner is invalid: {error:?}"))?;
    let server = HumanUnixServer::bind(HumanListenerConfig { endpoint: socket_path, owner_uid: socket_uid, owner_gid: required("LAYERX_AGENT_HUMAN_SOCKET_GID")?.parse().map_err(|_| "human socket gid is invalid")?, mode: u32::from_str_radix(&required("LAYERX_AGENT_HUMAN_SOCKET_MODE")?, 8).map_err(|_| "human socket mode is invalid")?, maximum_frame_bytes: limits.maximum_frame_bytes, deadline, peers }, owner).map_err(|error| format!("human listener is invalid: {error:?}"))?;
    let (sender, receiver) = mpsc::sync_channel(1);
    thread::Builder::new().name("layerx-agent-human".to_owned()).spawn(move || { let _ = sender.send(server.serve().map_err(|error| format!("human listener stopped: {error:?}"))); }).map_err(|error| format!("human listener thread failed: {error}"))?;
    Ok(receiver)
}
...
fn serve(config: Config) -> Result<(), String> {
    ...
    listener.set_nonblocking(true).map_err(|error| format!("agent program listener nonblocking setup failed: {error}"))?;
    loop {
        ...
        let mut stream = match listener.accept() {
            Ok((stream, _)) => stream,
            Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => { thread::sleep(Duration::from_millis(25)); continue; }
