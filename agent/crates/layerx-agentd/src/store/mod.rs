...
impl Store {
    ...
    pub fn replace_local_with_companion(
        ...
        let Some(previous) = self.entries.get(previous_key) else { return Err(StoreError::Corrupt("missing local record to replace")); };
        if previous.class != StorageClass::LocalOnly || self.entries.contains_key(&replacement_key) || self.entries.get(&companion_key).is_some_and(|value| value.class != StorageClass::LocalOnly) { return Err(StoreError::Corrupt("invalid local record replacement")); }
        let before = self.entries.clone();
        self.entries.remove(previous_key);
        self.entries.insert(replacement_key, StoredValue { class: StorageClass::LocalOnly, bytes: replacement_bytes });
        self.entries.insert(companion_key, StoredValue { class: StorageClass::LocalOnly, bytes: companion_bytes });
        if let Err(error) = self.persist() { self.entries = before; return Err(error); }
        Ok(())
    }
    ...
    pub fn update_local_with_companion(
        ...
        if self.entries.get(&key).is_none_or(|value| value.class != StorageClass::LocalOnly)
