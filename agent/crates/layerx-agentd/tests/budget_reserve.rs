...
fn concurrent_requests_obey_all_five_scopes() {
    let limiter = Arc::new(BudgetLimiter::new(configs(1_000))
    ...
    let workers: Vec<_> = (1_u8..=20)
        ...
        .collect();
    let accepted = workers.into_iter()
...
fn refusal_names_limit_and_leaks_no_reservation() {
    let limiter = BudgetLimiter::new(configs(100))
        .unwrap_or_else(|error| panic!("limiter: {error:?}"));
    for id in 0_u16..1_000 {
        ...
        let error = reserve(&limiter, &request).unwrap_err();
        match error {
            LimitRefusal::Exceeded { name, ceiling, consumed, requested, .. } => {
...
fn terminal_and_expiry_release_are_deterministic_unknown_is_held() {
    let limiter = BudgetLimiter::new(configs(1_000))
        .unwrap_or_else(|error| panic!("limiter: {error:?}"));
    reserve(&limiter, &request(1, 100)).unwrap_or_else(|error| panic!("reserve: {error:?}"));
    assert_eq!(release(&limiter, [1; 32], ReleaseKind::Unknown, 200), Ok(false));
    assert_eq!(release(&limiter, [1; 32], ReleaseKind::Executed, 2), Ok(true));
