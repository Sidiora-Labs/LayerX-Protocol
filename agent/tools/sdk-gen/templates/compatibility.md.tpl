...
|---|---|---|---|---|
| `{{SDK}}` | `{{CONTRACT}}` | `{{NODE_INTERFACE}}` | `{{DAEMON}}` | supported |
...
The approval module was introduced additively in contract `{{APPROVAL_INTRODUCED}}`. SDK clients negotiated below that minor must treat approval operations and lifecycle events as unavailable; no fallback may synthesize them.
...
| Approval holds and decisions | daemon-enforced | The SDK preserves the approval disclosure, state, decision outcome and lifecycle event exactly. Approval confers no protocol authority, and bypassing the daemon bypasses the restriction. |
