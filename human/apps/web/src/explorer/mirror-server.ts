...
function executableConfiguration():Readonly<{executable:string;configuration:string}>{
  const executable=process.env.LAYERX_MIRROR_VERIFIER_EXECUTABLE;const configuration=process.env.LAYERX_MIRROR_VERIFIER_CONFIG;
  if(executable===undefined||configuration===undefined||!isAbsolute(executable)||!isAbsolute(configuration))throw new MirrorUnavailableError();
...
async function execute(request:Json):Promise<Json>{
  const {executable,configuration}=executableConfiguration();const input=JSON.stringify(request);return new Promise((resolve,reject)=>{const child=execFile(executable,[configuration],{encoding:"utf8",maxBuffer:MAX_EXECUTABLE_OUTPUT_BYTES,timeout:TIMEOUT_MS,windowsHide:true},(error,stdout)=>{if(error!==null){reject(new MirrorUnavailableError());return;}try{resolve(record(JSON.parse(stdout)));}catch{reject(new MirrorUnavailableError());}});child.stdin?.end(input);});
}
...
export async function verifyEvidenceFromMirrors(input:Readonly<{kind:"receipt"|"state-inclusion";evidence:string}>):Promise<EvidenceVerificationReport>{
  const response=await execute(evidenceRequest(input.kind,input.evidence));if(response.ok!==true){const code=response.error;if(code==="divergent")throw new MirrorDivergentError();if(code==="source-unavailable"||code==="unavailable"||code==="rate-limited"||code==="rpc-divergent")throw new MirrorUnavailableError();throw new MirrorRefusedError();}
