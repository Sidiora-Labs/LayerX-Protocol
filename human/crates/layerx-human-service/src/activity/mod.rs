...
impl ReceiptEvidence {
    ...
    #[must_use]
    pub fn canonical(&self) -> Option<&[u8]> { self.canonical.as_deref() }
    #[must_use]
    pub const fn authority(&self) -> Option<&layerx_proof::receipt::AuthorizedBatch> { self.authority.as_ref() }
}
...
impl Feed {
    ...
    pub fn record_journey(
        ...
        let receipts: Vec<StoredReceipt> = status
            .receipt_digests()
            .iter()
            .zip(status.receipt_material()).zip(status.receipt_authorities())
            .filter_map(|((digest,material),authority)| digest.map(|digest| StoredReceipt {
...
#[derive(Clone,Debug,Deserialize,Serialize)]
struct StoredReceiptAuthority{batch_id:[u8;32],asset:[u8;32],previous_state_root:[u8;32],resulting_state_root:[u8;32],sequencer_public_key:[u8;32]}
impl StoredReceiptAuthority{fn from_public(value:&layerx_proof::receipt::AuthorizedBatch)->Self{Self{batch_id:value.batch_id(),asset:value.asset(),previous_state_root:value.previous_state_root(),resulting_state_root:value.resulting_state_root(),sequencer_public_key:value.sequencer_public_key()}}}
    ...
    let receipts = revision
        ...
        .map(|receipt| {
            Ok(ReceiptEvidence {
                ...
                authority: receipt.authority.as_ref().map(|value|layerx_proof::receipt::AuthorizedBatch::new(value.batch_id,value.asset,value.previous_state_root,value.resulting_state_root,value.sequencer_public_key)),
            })
        })
