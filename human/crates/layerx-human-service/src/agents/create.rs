...
impl CreationJourney {
    ...
    pub fn list(scope: &PrincipalScope<'_>) -> Result<Vec<Self>, AgentCreationError> {
        ...
        for key in scope.keys(Table::Journeys) {
            ...
            let row = scope.get(Table::Journeys, &key)
        ...
        journeys.sort_by(|left, right| right.record.started_at.cmp(&left.record.started_at)
    ...
    pub fn projection(&self) -> CreationProjection {
        CreationProjection {
            ...
            status: self.status(),
            verified_evidence: self.record.stages.iter().map(|stage| stage.evidence_digest).collect::<Option<Vec<_>>>().unwrap_or_default(),
            ...
            session_expires_at: self.record.started_at.checked_add(self.record.preset.session_lifetime_seconds).unwrap_or_else(|| unreachable!()),
            protocol_grant_id: self.stage(CreationStage::SessionProvision).object_id.unwrap_or([0; 32]),
            period_seconds: self.record.preset.budget_period_seconds,
            owner_account: self.record.owner_account.clone(),
            budget_account: self.budget_account().unwrap_or_else(|_| unreachable!()).canonical().to_owned(),
    ...
    fn run_session<C: ScopedAgentCreationContract>(
        ...
        let evidence = agent.provision_session_scoped(scope, registry, SessionProvision {
            action_key,
            did: self.did()?,
            activity_types,
            daemon_scopes: self.record.preset.session_scopes.clone(),
            ...
            primary_authority: self.record.public_key.ok_or(AgentCreationError::CustodyRequired)?,
...
fn validate_record(record: &JourneyRecord, expected: [u8; 32]) -> Result<(), AgentCreationError> {
    if record.version != RECORD_VERSION
        ...
        || record.started_at.checked_add(record.preset.session_lifetime_seconds).is_none()
        || record.did.is_empty()
