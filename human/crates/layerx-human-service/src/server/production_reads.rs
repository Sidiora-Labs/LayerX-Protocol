...
pub(super) fn activity_entry_json(
    ...
    let evidence=detail.evidence().iter().map(|link|json!({"evidence_id":format!("evd_{}",link.reference()),"class":"layerx-receipt","verification":"receipt-verified"})).collect::<Vec<_>>();
    Ok(
        json!({"entry_id":id.as_str(),"kind":activity_kind(detail.kind()),"state":activity_status(detail.status()),"state_copy_key":format!("activity.state.{}",activity_status(detail.status())),"summary_copy_key":detail.sentence(),"occurred_at":entry.occurred_at(),"stages":stages,"evidence":evidence,"money":money,"fees":fees}),
    )
}
...
fn evidence_get(
    ...
    if let Some(row) = scope.get(crate::store::Table::Cache, &bundle_key) {
        let _ = crate::activity::EvidenceBundle::decode(row.bytes())
            .map_err(|_| ApiFailure::upstream_degraded())?;
        return response(
            json!({"evidence_id":id,"class":"local-journey-state","verification":"receipt-verified","content_type":"application/vnd.layerx.evidence-bundle","bytes_base64":STANDARD.encode(row.bytes())}),
        );
    }
    ...
    for journey in JourneyEngine::list(scope).map_err(failure)? {
        ...
        for index in 0..status.phases().len() {
            ...
            if evidence.receipt_digest == expected {
                return response(
                    json!({"evidence_id": id, "class": "layerx-receipt", "verification": "receipt-verified",
...
pub(super) fn journey_json(journey: &JourneyEngine) -> Result<Value, ApiFailure> {
    ...
    let evidence = status
        ...
        .iter()
        .flatten()
        .map(|digest| evidence_ref(*digest))
        .collect::<Vec<_>>();
...
fn evidence_ref(digest: [u8; 32]) -> Value {
    json!({"evidence_id": format!("evd_{}", hex(&digest)), "class": "layerx-receipt", "verification": "receipt-verified"})
}
