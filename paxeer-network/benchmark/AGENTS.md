...
| `PAX_HOME` | `$HOME/.pax` | Final chain data dir. If != ~/.pax, init in ~/.pax then `mv` |
| `PORT_OFFSET` | `0` | Added to all ports (RPC, P2P, pprof, gRPC, etc.) |
| `PAXD_BIN` | `""` | Pre-built binary path. If set, skip build + copy to ~/go/bin/paxd |
| `LOG_FILE` | `""` | Redirect paxd output to file |
...
Each scenario gets its own binary, home dir, and port set (offset by 100). Results are printed at the end with median/avg/min/max TPS. Raw data in `/tmp/pax-bench/<label>/tps.txt`.
...
go tool pprof -top -diff_base /tmp/pax-bench/<baseline>/pprof/cpu.pb.gz /tmp/pax-bench/<candidate>/pprof/cpu.pb.gz

# Allocation diff
go tool pprof -alloc_space -top -diff_base /tmp/pax-bench/<baseline>/pprof/heap.pb.gz /tmp/pax-bench/<candidate>/pprof/heap.pb.gz
```
