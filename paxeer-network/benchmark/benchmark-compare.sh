...
#
# Each scenario gets its own paxd binary, data dir, port set, and log file.
# Init is sequential (fast, uses ~/.pax then moves). Run is parallel.
...
cleanup() {
  ...
  for i in $(seq 0 $((NUM-1))); do
    label="${LABELS[$i]}"
    pkill -f "paxd-${label}" 2>/dev/null || true
  done
  sleep 2
...
for i in $(seq 0 $((NUM-1))); do
  ...
  cp ~/go/bin/paxd "$BASE_DIR/$label/paxd-${label}"
  echo "done"
done
echo ""
...
for i in $(seq 0 $((NUM-1))); do
  label="${LABELS[$i]}"
  paxd="$BASE_DIR/$label/paxd-${label}"
