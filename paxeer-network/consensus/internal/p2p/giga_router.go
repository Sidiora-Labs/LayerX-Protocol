...
import (
	...
	"github.com/sidiora-labs/paxeer-network/consensus/internal/p2p/giga"
...
func NewGigaRouter(cfg *GigaRouterConfig, key NodeSecretKey) (*GigaRouter, error) {
	...
	//
	// TODO(autobahn): once storage/ledger_db/block.BlockDB has a writer wired
...
//
// TODO(autobahn): switch this to read from storage/ledger_db/block.BlockDB
...
// Pax's RetainHeight and exposes only a height index (no GetBlockByHash).
...
// storage/ledger_db/block.BlockDB.GetBlockByHash once a writer is wired into
...
func (r *GigaRouter) dialAndRunConn(ctx context.Context, key NodePublicKey, hp tcp.HostPort) error {
	...
	return scope.Run(ctx, func(ctx context.Context, s scope.Scope) error {
		...
		if !hConn.msg.PaxGigaConnection {
			return fmt.Errorf("not a pax giga connection")
		}
...
func (r *GigaRouter) RunInboundConn(ctx context.Context, hConn *handshakedConn) error {
	if !hConn.msg.PaxGigaConnection {
		return fmt.Errorf("not a PaxGiga connection")
	}
