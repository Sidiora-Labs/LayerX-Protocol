...
func NewRouter(
	...
	// connections pax-v2 can effectively handle currently.
			...
			s.Spawn(func() error {
				...
				err := scope.Run(ctx, func(ctx context.Context, s scope.Scope) error {
					...
					hConn, err := handshake(handshakeCtx, tcpConn, r.privKey, handshakeSpec{
						...
						PexAddrs:          pexAddrs,
						PaxGigaConnection: r.giga.IsPresent(),
					})
					if err != nil {
						return fmt.Errorf("handshake(): %w", err)
					}
					if giga, ok := r.giga.Get(); ok && hConn.msg.PaxGigaConnection {
						release()
