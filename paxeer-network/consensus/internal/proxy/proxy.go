...
func (app *Proxy) EvmBalance(addr common.Address, paxAddr []byte) uint256.Int {
	defer addTimeSample(app.metrics.MethodTiming.With("method", "evm_balance", "type", "sync"))()
	return app.app.EvmBalance(addr, paxAddr)
}
...
func (app *Proxy) CheckTxSafe(ctx context.Context, req *types.RequestCheckTxV2) (res *types.ResponseCheckTxV2, err error) {
	...
	if res.IsEVM {
		...
		if len(res.PaxSenderAddress) == 0 {
			return nil, fmt.Errorf("EVM response missing PaxSenderAddress")
		}
	}
	return res, nil
}
