...
contract CW1155ERC1155Pointer is ERC1155, ERC2981 {
    ...
    function safeTransferFrom(
        ...
        string memory f = _formatPayload("from", _doubleQuotes(AddrPrecompile.getPaxAddr(from)));
        string memory t = _formatPayload("to", _doubleQuotes(AddrPrecompile.getPaxAddr(to)));
    ...
    function safeBatchTransferFrom(
        ...
        string memory payload = string.concat("{\"send_batch\":{\"from\":\"", AddrPrecompile.getPaxAddr(from));
        payload = string.concat(payload, "\",\"to\":\"");
        payload = string.concat(payload, AddrPrecompile.getPaxAddr(to));
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        string memory op = _curlyBrace(_formatPayload("operator", _doubleQuotes(AddrPrecompile.getPaxAddr(operator))));
        if (approved) {
    ...
    function burn(address account, uint256 id, uint256 amount) public virtual {
        ...
        string memory f = _formatPayload("from", _doubleQuotes(AddrPrecompile.getPaxAddr(account)));
        string memory tId = _formatPayload("token_id", _doubleQuotes(Strings.toString(id)));
        string memory amt = _formatPayload("amount", _doubleQuotes(Strings.toString(amount)));
    ...
    function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) public virtual {
        ...
        string memory payload = string.concat("{\"burn_batch\":{\"from\":\"", AddrPrecompile.getPaxAddr(account));
        payload = string.concat(payload, "\",\"batch\":[");
        for (uint256 i = 0; i < ids.length; i++) {
            string memory batch = string.concat("{\"token_id\":\"", Strings.toString(ids[i]));
            batch = string.concat(batch, "\",\"amount\":\"");
    ...
    function balanceOf(address account, uint256 id) public view override returns (uint256) {
        ...
        string memory own = _formatPayload("owner", _doubleQuotes(AddrPrecompile.getPaxAddr(account)));
        string memory tId = _formatPayload("token_id", _doubleQuotes(Strings.toString(id)));
    ...
    function balanceOfBatch(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            string memory ownerToken = string.concat("{\"owner\":\"", AddrPrecompile.getPaxAddr(accounts[i]));
    ...
    function isApprovedForAll(address owner_, address operator) public view override returns (bool) {
        string memory own = _formatPayload("owner", _doubleQuotes(AddrPrecompile.getPaxAddr(owner_)));
        string memory op = _formatPayload("operator", _doubleQuotes(AddrPrecompile.getPaxAddr(operator)));
