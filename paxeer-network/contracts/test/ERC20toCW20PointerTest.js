        ...
        describe("pointer functions", function () {
            ...
            describe("transfer()", function () {
                it("should transfer", async function () {
                    ...
                    const paxBlock = await ethers.provider.send('pax_getBlockByNumber', ['0x' + blockNumber.toString(16), true]);
                    // check to address is the pointer address not the direct casted contract address
                    expect(paxBlock.transactions[0].to.toLowerCase()).to.equal((await pointer.getAddress()).toLowerCase());
                    ...
                    const paxReceipts = await ethers.provider.send('pax_getBlockReceipts', ['0x' + blockNumber.toString(16)]);
                    expect(ethReceipts.length).to.equal(1);
                    expect(paxReceipts.length).to.equal(1);
                    expect(ethReceipts[0].transactionHash).to.equal(paxReceipts[0].transactionHash);

                    const ethTx = await ethers.provider.send('pax_getTransactionReceipt', [receipt.hash]);
            ...
            describe("approve()", function () {
                it("should approve", async function () {
                    ...
                    // pax_ includes synthetic logs -> expect 1
                    const paxlogs = await ethers.provider.send('pax_getLogs', [filter]);
                    expect(paxlogs.length).to.equal(1);
                    expect(paxlogs[0]["address"].toLowerCase()).to.equal((await pointer.getAddress()).toLowerCase());
                    expect(paxlogs[0]["topics"][0]).to.equal(ethers.id("Approval(address,address,uint256)"));
                    expect(paxlogs[0]["topics"][1].substring(26)).to.equal(owner.substring(2).toLowerCase());
                    expect(paxlogs[0]["topics"][2].substring(26)).to.equal(spender.substring(2).toLowerCase());
                });
