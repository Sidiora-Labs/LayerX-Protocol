...
describe("ERC721 to CW721 Pointer", function () {
    ...
    before(async function () {
        ...
        cw721Address = await deployWasm(WASM.CW721, admin.paxAddress, "cw721", {
            name: "Test",
            symbol: "TEST",
            minter: admin.paxAddress
        })

        await executeWasm(cw721Address,  { mint : { token_id : "1", owner : admin.paxAddress, token_uri: "token uri 1"}});
        await executeWasm(cw721Address,  { mint : { token_id : "2", owner : accounts[0].paxAddress, token_uri: "token uri 2"}});
        await executeWasm(cw721Address,  { mint : { token_id : "3", owner : accounts[1].paxAddress, token_uri: "token uri 3"}});
    ...
    describe("write", function(){
        ...
        it("transfer from", async function () {
            ...
            // do same for eth_getBlockReceipts and pax_getBlockReceipts
            ...
            const paxBlockReceipts = await ethers.provider.send('pax_getBlockReceipts', ['0x' + receipt.blockNumber.toString(16)]);
            expect(paxBlockReceipts.length).to.equal(1);
        ...
        it("set approval for all", async function () {
            ...
            const paxReceipt = await ethers.provider.send('pax_getTransactionReceipt', [receipt.hash]);
            expect(paxReceipt.logs.length).to.equal(1);
