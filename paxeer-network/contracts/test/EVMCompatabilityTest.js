  ...
  describe("EVMCompatibilityTester", function () {
    ...
    describe("JSON-RPC", function() {
      ...
      it("Should retrieve a transaction receipt", async function () {
        ...
        expect(receipt.type).to.equal(2); // pax is failing this
        expect(receipt.status).to.equal(1);
        expect(receipt.to).to.equal(await evmTester.getAddress());
        expect(receipt.from).to.equal(owner.address);
        expect(receipt.cumulativeGasUsed).to.be.greaterThanOrEqual(0); // on paxlocal, this is 0

        // undefined / null on anvil and goerli
        // expect(receipt.contractAddress).to.be.equal(null); // seeing this be null (pax devnet) and not null (anvil, goerli)
      ...
      it("Should fetch past logs", async function () {
        ...
        // current chain height — Pax's eth_getLogs caps a single request
    ...
    describe("Uhpx/Wei testing", function() {
      it("Send 1 uhpx to contract", async function() {
        const uhpx = ethers.parseUnits("1", 12);
        ...
        expect(contractBalance - initialBalance).to.equal(uhpx);
