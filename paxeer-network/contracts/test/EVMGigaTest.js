 ...
 * 
 * Tests cover:
 * - Native PAX transfers between accounts
...
describe("GIGA EVM Tests", function () {
  ...
  describe("Native Transfers", function () {
    it("should transfer native PAX between accounts", async function () {
  ...
  describe("Multi-Hop Swap (Cross-Contract Token Transfers)", function () {
    ...
    before(async function () {
      // Deploy 4 tokens (simulates WPAX, USDC, DRG, and another token in the path)
  ...
  //   1. Proxy token with delegatecall (like Pax's USDC proxy)
  ...
  describe("Proxy + Callback Multi-Hop Swap (Mainnet Repro)", function () {
    ...
    before(async function () {
      ...
      // Deploy proxy token (like Pax's USDC proxy)
  ...
  describe("Self-Destruct With Storage Cleanup", function () {
    ...
    it("should seed storage on init and sweep balance on later-tx selfdestruct", async function () {
      ...
      expect(await contract.sumKeys()).to.equal(expectedSum);
      console.log(`        later-tx selfdestruct swept ${ethers.formatEther(seedValue)} PAX, storage retained`);
    });
