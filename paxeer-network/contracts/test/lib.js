...
// polling `paxd q tx <hash>` (which doesn't work under Autobahn — the
...
async function getCosmosTx(provider, evmTxHash) {
    return await provider.send("pax_getCosmosTx", [evmTxHash])
}

async function getEvmTx(provider, cosmosTxHash) {
    return await provider.send("pax_getEvmTx", [cosmosTxHash])
}
...
async function evmSend(addr, fromKey, amount="10000000000000000000000000") {
    // paxd tx evm send prints "Transaction hash: 0x..." on its own format
    ...
    const output = await execute(`paxd tx evm send ${addr} ${amount} --from ${fromKey} -b sync -y`);
    const evmTxHash = output.replace(/.*0x/, "0x").trim()
    await waitForReceipt(evmTxHash)
    return evmTxHash
}

async function bankSend(toAddr, fromKey, amount="100000000000", denom="uhpx") {
    const senderAddr = await getKeyPaxAddress(fromKey)
    // Non-self-send: recipient's `denom` balance goes up by `amount`.
    // Self-send: `denom` amount cancels; the recipient (also the sender)
    // pays fees in uhpx, so uhpx drops. Track whichever balance is
    // expected to change post-commit so the wait is a side-effect check.
    const trackedDenom = senderAddr === toAddr ? "uhpx" : denom
    const before = await getPaxBalanceBigInt(toAddr, trackedDenom)
    const result = await execute(`paxd tx bank send ${fromKey} ${toAddr} ${amount}${denom} -b sync -o json --fees 20000uhpx -y`);
    const parsed = JSON.parse(result)
    if (parsed.code !== 0) throw new Error(`bank send rejected: ${parsed.raw_log}`)
    await waitForCondition(
        async () => (await getPaxBalanceBigInt(toAddr, trackedDenom)) !== before,
        `${toAddr} ${trackedDenom} balance to change from ${before}`,
    )
    return result
}

async function fundPaxAddress(paxAddr, amount="100000000000", denom="uhpx", funder=adminKeyName) {
    const before = await getPaxBalanceBigInt(paxAddr, denom)
    const result = await execute(`paxd tx bank send ${funder} ${paxAddr} ${amount}${denom} -b sync -o json --fees 20000uhpx -y`);
    const parsed = JSON.parse(result)
    if (parsed.code !== 0) throw new Error(`fundPaxAddress rejected: ${parsed.raw_log}`)
    const target = before + BigInt(amount)
    await waitForCondition(
        async () => (await getPaxBalanceBigInt(paxAddr, denom)) >= target,
        `${paxAddr} ${denom} balance >= ${target}`,
    )
    return result
}

// BigInt variant of getPaxBalance. Genesis-funded accounts hold balances
// well above 2^53, where JS Number arithmetic loses precision (a
// `before + amount` comparison can silently round wrong). Polling
// helpers should use this; existing Number-returning getPaxBalance is
// kept for callers that don't run into the precision range.
async function getPaxBalanceBigInt(paxAddr, denom="uhpx") {
    const result = await execute(`paxd query bank balances ${paxAddr} -o json`);
...
// paxAddr, or null if the account doesn't exist yet. A sender's
...
async function getAccountSequence(paxAddr) {
    try {
        const out = await execute(`paxd query account ${paxAddr} -o json`)
...
async function getPaxBalance(paxAddr, denom="uhpx") {
    const result = await execute(`paxd query bank balances ${paxAddr} -o json`);
    const balances = JSON.parse(result)
    for(let b of balances.balances) {
        if(b.denom === denom) {
...
async function addKey(name) {
    try {
        return await execute(`paxd keys add ${name}`, `printf "12345678\\n12345678\\n"`)
    } catch(e) {}
}

async function importKey(name, keyfile) {
    try {
        return await execute(`paxd keys import ${name} ${keyfile}`, `printf "12345678\\n12345678\\n"`)
    } catch(e) {
...
async function getNativeAccount(keyName) {
    ...
    const paxAddress = await getKeyPaxAddress(keyName)
    ...
    if (keyName !== adminKeyName) {
        await fundPaxAddress(paxAddress)
    }
    const evmAddress = await getEvmAddress(paxAddress)
    return {
        paxAddress,
        evmAddress
    }
...
async function getKeyPaxAddress(name) {
    return (await execute(`paxd keys show ${name} -a`)).trim()
}
...
async function associateKey(keyName) {
    try {
        // paxd tx evm associate-address has a custom (non-cosmos-JSON) output
        ...
        // wait is acceptable.
        await execute(`paxd tx evm associate-address --from ${keyName} -b sync`)
        await waitForBlocks()
    } catch (e) {
...
async function associateKeyStrict(keyName) {
    ...
    await execute(`paxd tx evm associate-address --from ${keyName} -b sync`)
    await waitForCondition(
        async () => (await getEvmAddressAssociation(paxAddress)).associated === true,
        `${paxAddress} to have an associated EVM address`,
    )
}
...
async function incrementPointerVersion(provider, pointerType, offset) {
    if(await isDocker()) {
        ...
        for(let i=0; i<4; i++) {
            const resultStr = await execCommand(`docker exec pax-node-${i} curl -s -X POST http://localhost:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"test_incrementPointerVersion","params":["${pointerType}", ${offset}],"id":1}'`)
...
async function createTokenFactoryTokenAndMint(name, amount, recipient, from=adminKeyName) {
    ...
    const create_cmd = `paxd tx tokenfactory create-denom ${name} --from ${from} --gas=5000000 --fees=1000000uhpx -y --broadcast-mode sync -o json`
    ...
    await waitForCondition(
        async () => {
            try {
                const out = await execute(`paxd query tokenfactory denom-authority-metadata ${token_denom} --output json`)
    ...
    const mint_cmd = `paxd tx tokenfactory mint ${amount}${token_denom} --from ${from} --gas=5000000 --fees=1000000uhpx -y --broadcast-mode sync -o json`
    const mintResp = JSON.parse(await execute(mint_cmd))
    if (mintResp.code !== 0) throw new Error(`mint rejected: ${mintResp.raw_log}`)
    await waitForCondition(
        async () => (await getPaxBalanceBigInt(fromPaxAddr, token_denom)) >= target,
        `${from} ${token_denom} balance >= ${target}`,
    )

    // Step 3: bank send to recipient. Wait for the recipient to hold the amount.
    const send_cmd = `paxd tx bank send ${from} ${recipient} ${amount}${token_denom} --from ${from} --gas=5000000 --fees=1000000uhpx -y --broadcast-mode sync -o json`
    const sendResp = JSON.parse(await execute(send_cmd))
    if (sendResp.code !== 0) throw new Error(`bank send rejected: ${sendResp.raw_log}`)
    await waitForCondition(
        async () => (await getPaxBalanceBigInt(recipient, token_denom)) >= target,
...
async function getPointerForNative(name) {
    const command = `paxd query evm pointer NATIVE ${name} -o json`
...
async function getMaxWasmCodeId() {
    try {
        const out = await execute(`paxd query wasm list-code --reverse --limit 1 -o json`)
...
async function listContractsByCode(codeId) {
    try {
        const out = await execute(`paxd query wasm list-contract-by-code ${codeId} -o json`)
...
async function storeWasm(path, from=adminKeyName) {
    ...
    const command = `paxd tx wasm store ${path} --from ${from} --gas=5000000 --fees=1000000uhpx -y -b sync -o json`
...
async function getPointerForCw20(cw20Address) {
    const command = `paxd query evm pointer CW20 ${cw20Address} -o json`
    const output = await execute(command);
    return JSON.parse(output);
}

async function getPointerForCw721(cw721Address) {
    const command = `paxd query evm pointer CW721 ${cw721Address} -o json`
    const output = await execute(command);
    return JSON.parse(output);
}

async function getPointerForCw1155(cw1155Address) {
    const command = `paxd query evm pointer CW1155 ${cw1155Address} -o json`
    const output = await execute(command);
    return JSON.parse(output);
}

async function deployErc20PointerForCw20(provider, cw20Address, attempts=10, from=adminKeyName, evmRpc="") {
    let command = `paxd tx evm register-evm-pointer CW20 ${cw20Address} --from=${from} -b sync`
    if (evmRpc) {
...
async function deployErc20PointerNative(provider, name, from=adminKeyName, evmRpc="") {
    let command = `paxd tx evm call-precompile pointer addNativePointer ${name} --from=${from} -b sync`
    if (evmRpc) {
        command = command + ` --evm-rpc=${evmRpc}`
    }
    const output = await execute(command);
...
async function deployErc721PointerForCw721(provider, cw721Address, from=adminKeyName, evmRpc="") {
    let command = `paxd tx evm register-evm-pointer CW721 ${cw721Address} --from=${from} -b sync`
    if (evmRpc) {
        command = command + ` --evm-rpc=${evmRpc}`
    }
    const output = await execute(command);
...
async function deployErc1155PointerForCw1155(provider, cw1155Address, from=adminKeyName, evmRpc="") {
    let command = `paxd tx evm register-evm-pointer CW1155 ${cw1155Address} --from=${from} -b sync`
    if (evmRpc) {
        command = command + ` --evm-rpc=${evmRpc}`
    }
    const output = await execute(command);
...
async function instantiateWasm(codeId, adminAddr, label, args = {}, from=adminKeyName) {
    ...
    const command = `paxd tx wasm instantiate ${codeId} "${jsonString}" --label ${label} --admin ${adminAddr} --from ${from} --gas=5000000 --fees=1000000uhpx -y -b sync -o json`;
    const response = JSON.parse(await execute(command));
...
async function proposeCW20toERC20Upgrade(erc20Address, cw20Address, title="erc20-pointer", version=99, description="erc20 pointer",fees="200000uhpx", from=adminKeyName) {
    const maxIdBefore = await maxProposalId()
    const command = `paxd tx evm add-cw-erc20-pointer "${title}" "${description}" ${erc20Address} ${version} 200000000uhpx ${cw20Address} --from ${from} --fees ${fees} -y -o json -b sync`
    const response = JSON.parse(await execute(command))
...
async function proposeParamChange(title, description, changes, deposit="200000000uhpx", fees="200000uhpx", from=adminKeyName, expedited=true) {
    const proposal = {
        title,
        description,
        changes,
    ...
    const maxIdBefore = await maxProposalId();
    const command = `paxd tx gov submit-proposal param-change ${tempFile} --from ${from} --fees ${fees} -y -o json -b sync`;
    const output = await execute(command);
...
async function findProposalByTitle(title, maxIdBefore, txHashForError) {
    ...
    while (Date.now() < deadline) {
        ...
        for (let id = maxIdBefore + 1; id <= cur; id++) {
            let detail;
            try {
                detail = JSON.parse(await execute(`paxd q gov proposal ${id} -o json`));
            } catch (e) {
...
async function maxProposalId() {
    let out;
    try {
        // paxd exits non-zero ("Error: no proposals found") on an empty
        // gov set; the try/catch treats that as id=0.
        out = await execute(`paxd q gov proposals --reverse --limit 1 -o json 2>/dev/null`);
    } catch (e) {
        return 0;
    }
...
async function proposeDisableWasm(title="Disable WASM", description="Disable cosmwasm store code and instantiate operations", deposit="200000000uhpx", fees="200000uhpx", from=adminKeyName) {
    const changes = [
        {
            subspace: "wasm",
            key: "uploadAccess",
...
async function proposeEnableWasm(title="Enable WASM", description="Enable cosmwasm store code and instantiate operations", deposit="200000000uhpx", fees="200000uhpx", from=adminKeyName) {
    const changes = [
        {
            subspace: "wasm",
            key: "uploadAccess",
...
async function disableWasm(from=adminKeyName) {
    const proposalId = await proposeDisableWasm("Disable WASM", "Disable cosmwasm store code and instantiate operations", "200000000uhpx", "200000uhpx", from);
    return await passProposal(proposalId);
}

async function enableWasm(from=adminKeyName) {
    const proposalId = await proposeEnableWasm("Enable WASM", "Enable cosmwasm store code and instantiate operations", "200000000uhpx", "200000uhpx", from);
    return await passProposal(proposalId);
}

async function getWasmParams() {
    const uploadAccess = await execute(`paxd query params subspace wasm uploadAccess -o json`);
    const instantiateAccess = await execute(`paxd query params subspace wasm instantiateAccess -o json`);
    return {
...
async function passProposal(proposalId,  desposit="200000000uhpx", fees="200000uhpx", from=adminKeyName) {
    if(await isDocker()) {
        await executeOnAllNodes(`paxd tx gov vote ${proposalId} yes --from node_admin -b sync -y --fees ${fees}`)
    } else {
        await execute(`paxd tx gov vote ${proposalId} yes --from ${from} -b sync -y --fees ${fees}`)
    }
    // Poll for proposal status with shorter delay for faster tests
    for(let i=0; i<200; i++) {
        const proposal = await execute(`paxd q gov proposal ${proposalId} -o json`)
...
async function registerPointerForERC20(erc20Address, fees="20000uhpx", from=adminKeyName) {
    return await registerPointerForCw(erc20Address, "ERC20", fees, from)
}

async function registerPointerForERC721(erc721Address, fees="20000uhpx", from=adminKeyName) {
    return await registerPointerForCw(erc721Address, "ERC721", fees, from)
}

async function registerPointerForERC1155(erc1155Address, fees="200000uhpx", from=adminKeyName) {
...
async function registerPointerForCw(cwAddress, type, fees, from) {
    const command = `paxd tx evm register-cw-pointer ${type} ${cwAddress} --from ${from} --fees ${fees} -b sync -y -o json`
    const response = JSON.parse(await execute(command))
    ...
    try {
        await waitForCondition(
            async () => {
                const out = await execute(`paxd query evm pointer ${type} ${cwAddress} -o json`)
...
async function getPaxAddress(evmAddress) {
    const command = `paxd q evm pax-addr ${evmAddress} -o json`
    const output = await execute(command);
    const response = JSON.parse(output)
    return response.pax_address
}

async function getEvmAddress(paxAddress) {
    return (await getEvmAddressAssociation(paxAddress)).evm_address
}

async function getEvmAddressAssociation(paxAddress) {
    const command = `paxd q evm evm-addr ${paxAddress} -o json`
    const output = await execute(command);
    return JSON.parse(output)
}
...
async function setupSigners(signers) {
    ...
    for(let signer of signers) {
        ...
        await resp.wait()
        const paxAddress = await getPaxAddress(evmAddress);
        result.push({
            paxAddress,
            evmAddress,
            signer,
...
async function queryWasm(contractAddress, operation, args={}){
    ...
    const command = `paxd query wasm contract-state smart ${contractAddress} "${jsonString}" --output json`;
    const output = await execute(command);
    return JSON.parse(output)
}

async function executeWasm(contractAddress, msg, coins = "0uhpx") {
    const jsonString = JSON.stringify(msg).replace(/"/g, '\\"'); // Properly escape JSON string
    const command = `paxd tx wasm execute ${contractAddress} "${jsonString}" --amount ${coins} --from ${adminKeyName} --gas=5000000 --fees=1000000uhpx -y -b sync -o json`;
    return await waitForAdminTxCommit(command)
}

async function associateWasm(contractAddress) {
    const command = `paxd tx evm associate-contract-address ${contractAddress} --from ${adminKeyName} --gas=5000000 --fees=1000000uhpx -y -b sync -o json`;
    return await waitForAdminTxCommit(command)
}

// Current latest block height as observed by the local node.
async function getCurrentBlockHeight() {
    const status = await execute(`paxd status`)
...
async function findInclusionBlock(txhashHex, fromHeight, toHeight) {
    ...
    for (let h = fromHeight; h <= toHeight; h++) {
        try {
            const block = JSON.parse(await execute(`paxd query block ${h}`))
...
async function waitForAdminTxCommit(command) {
    const senderAddr = await getKeyPaxAddress(adminKeyName)
...
async function printClaimMsg(sender, claimer) {
    const command = `paxd tx evm print-claim ${claimer} --from ${sender} -y`;
    try { return await execute(command); }
    catch(e) { console.log(e); }
}

async function printClaimMsgBySender(sender, claimer, senderAddr) {
    const command = `paxd tx evm print-claim-by-sender ${claimer} ${senderAddr} --from ${sender} -y`;
    try { return await execute(command); }
    catch(e) { console.log(e); }
}

async function printClaimSpecificMsg(sender, claimer, ...assets) {
    const command = `paxd tx evm print-claim-specific ${claimer} ${assets.join(' ')} --from ${sender} -y`;
    try { return await execute(command); }
    catch(e) { console.log(e); }
}
...
async function isDocker() {
    return new Promise((resolve, reject) => {
        exec("docker ps --filter 'name=pax-node-0' --format '{{.Names}}'", (error, stdout, stderr) => {
            if (stdout.includes('pax-node-0')) {
                resolve(true)
            } else {
                resolve(false)
            }
...
async function executeOnAllNodes(command, interaction=`printf "12345678\\n"`){
    if (await isDocker()) {
        command = command.replace(/\.\.\//g, "/pax-protocol/pax-chain/");
        command = command.replace("/pax-protocol/pax-chain//pax-protocol/pax-chain/", "/pax-protocol/pax-chain/")
        let response;
        for(let i=0; i<4; i++) {
            const nodeCommand = `docker exec pax-node-${i} /bin/bash -c 'export PATH=$PATH:/root/go/bin:/root/.foundry/bin && ${interaction} | ${command}'`;
...
async function execute(command, interaction=`printf "12345678\\n"`){
    if (await isDocker()) {
        command = command.replace(/\.\.\//g, "/pax-protocol/pax-chain/");
        command = command.replace("/pax-protocol/pax-chain//pax-protocol/pax-chain/", "/pax-protocol/pax-chain/")
        command = `docker exec pax-node-0 /bin/bash -c 'export PATH=$PATH:/root/go/bin:/root/.foundry/bin && ${interaction} | ${command}'`;
    }
    return await execCommand(command);
}

function execCommand(command) {
    // Cap shelled-out child processes (typically `docker exec ... paxd ...`)
