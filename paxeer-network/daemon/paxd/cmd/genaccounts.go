...
import (
	...
	authvesting "github.com/sidiora-labs/paxeer-network/sdk/x/auth/vesting/types"
...
The association between the pax address and the eth address will also be created here if using keyring-backend test.
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if keyringBackend == keyring.BackendTest {
				// associate the eth address with the pax address through the genesis file
				evmGenState := evm.GetGenesisStateFromAppState(depCdc, appState)
				paxEthAddrAssociation := evmtypes.AddressAssociation{
					PaxAddress: addr.String(),
					EthAddress: ethAddr.Hex(),
				}
				evmGenState.AddressAssociations = append(evmGenState.AddressAssociations, &paxEthAddrAssociation)
...
func getPrivateKeyOfAddr(kb keyring.Keyring, addr sdk.Address) (*ecdsa.PrivateKey, error) {
	...
	for _, key := range keys {
		...
		if localInfo.GetAddress().Equals(addr) {
			...
			// Need to use private key to convert to pax address here
