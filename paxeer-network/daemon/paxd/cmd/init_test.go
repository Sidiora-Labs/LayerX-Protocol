...
import (
	...
	"github.com/sidiora-labs/paxeer-network/node/genesis"
...
func TestInitModeConfiguration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Write app.toml using our custom template that includes PaxDB configs
			...
			customAppTemplate := srvconfig.ManualConfigTemplate + paxdbconfig.StateCommitConfigTemplate + paxdbconfig.StateStoreConfigTemplate +
			...
			require.Equal(t, paxdbconfig.DefaultReceiptStoreConfig().AsyncWriteBuffer, v.GetInt("receipt-store.async-write-buffer"))
			require.Equal(t, paxdbconfig.DefaultReceiptStoreConfig().PruneIntervalSeconds, v.GetInt("receipt-store.prune-interval-seconds"))
		})
	}
}
