...
func getTotalSupply(genState *GenesisState) (sdk.Coins, error) {
	...
	weiInUhpx, weiRemainder := SplitUhpxWeiAmount(totalWeiBalance)
	...
	if err != nil {
		if !weiInUhpx.IsZero() {
			return nil, fmt.Errorf("base denom is not registered %s yet there exists wei balance %s", err, weiInUhpx)
		}
	} else {
		totalSupply = totalSupply.Add(sdk.NewCoin(baseDenom, weiInUhpx))
	}
