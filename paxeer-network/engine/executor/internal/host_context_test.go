...
func TestNewHostContextConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Nil PaxSstoreSetGasEIP2200",
			chainConfig: &params.ChainConfig{
				PaxSstoreSetGasEIP2200: nil,
			},
			expectedDelta: 0,
		},
		{
			...
			chainConfig: &params.ChainConfig{
				PaxSstoreSetGasEIP2200: func() *uint64 { v := uint64(20000); return &v }(),
			},
			expectedDelta: 0,
		},
		{
			...
			chainConfig: &params.ChainConfig{
				PaxSstoreSetGasEIP2200: func() *uint64 { v := uint64(72000); return &v }(),
			},
			expectedDelta: 52000,
		},
		{
			...
			chainConfig: &params.ChainConfig{
				PaxSstoreSetGasEIP2200: func() *uint64 { v := uint64(100000); return &v }(),
			},
			expectedDelta: 80000,
		},
		{
			...
			chainConfig: &params.ChainConfig{
				PaxSstoreSetGasEIP2200: func() *uint64 { v := uint64(10000); return &v }(),
			},
			expectedDelta: -10000,
		},
		{
			...
			chainConfig: &params.ChainConfig{
				PaxSstoreSetGasEIP2200: func() *uint64 { v := uint64(0); return &v }(),
			},
			expectedDelta: -20000,
		},
	}
...
func TestNewHostContextConfig_OverflowPanic(t *testing.T) {
	...
	chainConfig := &params.ChainConfig{
		PaxSstoreSetGasEIP2200: &overflowValue,
	}

	require.Panics(t, func() {
		NewHostContextConfig(chainConfig)
	}, "Should panic when PaxSstoreSetGasEIP2200 exceeds math.MaxInt64")
}
...
func TestNewHostContextConfig_MaxSafeValue(t *testing.T) {
	...
	chainConfig := &params.ChainConfig{
		PaxSstoreSetGasEIP2200: &maxSafeValue,
	}
	...
	}, "Should not panic when PaxSstoreSetGasEIP2200 equals math.MaxInt64")
}

// TestSstoreGasDeltaCalculation tests the SSTORE gas delta calculation logic
// that determines how much extra/less gas to charge for Pax's custom SSTORE cost.
func TestSstoreGasDeltaCalculation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			paxSstoreGas:  StandardSstoreSetGasEIP2200,
			expectedDelta: 0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Replicate the delta calculation logic: Pax cost - standard cost
			delta := int64(tt.paxSstoreGas) - int64(StandardSstoreSetGasEIP2200)

			require.Equal(t, tt.expectedDelta, delta,
				"Delta for paxSstoreGas=%d should be %d", tt.paxSstoreGas, tt.expectedDelta)
		})
	}
}
