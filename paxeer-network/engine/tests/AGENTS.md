...
- **Test data location:** `engine/tests/data/`
- **Skip list:** `engine/tests/data/skip_list.json`
...
go test -v -run TestGigaVsV2_StateTests ./engine/tests/...

# Run specific category
STATE_TEST_DIR=stExample go test -v -run TestGigaVsV2_StateTests ./engine/tests/...

# Run specific test within category
STATE_TEST_DIR=stExample STATE_TEST_NAME=add11 go test -v -run TestGigaVsV2_StateTests ./engine/tests/...

# Ignore skip list (run skipped tests)
IGNORE_SKIP_LIST=true STATE_TEST_DIR=stCreate2 go test -v -run TestGigaVsV2_StateTests ./engine/tests/...

# Use regular KVStore (for debugging)
USE_REGULAR_STORE=true STATE_TEST_DIR=stChainId go test -v -run TestGigaVsV2_StateTests ./engine/tests/...
```
...
STATE_TEST_DIR=stCategory1 go test -v -run TestGigaVsV2_StateTests ./engine/tests/... -timeout 30m 2>&1 | tee /tmp/results_stCategory1.log &
STATE_TEST_DIR=stCategory2 go test -v -run TestGigaVsV2_StateTests ./engine/tests/... -timeout 30m 2>&1 | tee /tmp/results_stCategory2.log &
wait
```
...
**File:** `engine/tests/data/skip_list.json`
...
1. **Run test with `IGNORE_SKIP_LIST=true`:**
   ```bash
   IGNORE_SKIP_LIST=true STATE_TEST_DIR=stNewCategory go test -v -run TestGigaVsV2_StateTests ./engine/tests/... -timeout 30m 2>&1 | tee /tmp/results.log
   ```
...
6. **Verify tests pass:**
   ```bash
   STATE_TEST_DIR=stNewCategory go test -v -run TestGigaVsV2_StateTests ./engine/tests/... -timeout 10m
   ```

7. **Commit and push after each batch:**
   ```bash
   git add engine/tests/data/skip_list.json
