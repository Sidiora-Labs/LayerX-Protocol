...
// Note: Rebuilds the transaction with Pax's chain ID since fixtures use chain ID 1
func BuildTransaction(st *StateTestJSON, subtest StateTestPost) (*ethtypes.Transaction, common.Address, error) {
	...
	if maxFeePerGas.Sign() > 0 || st.Transaction.MaxFeePerGas != "" {
		...
		tx = ethtypes.NewTx(&ethtypes.DynamicFeeTx{
			ChainID:    big.NewInt(config.DefaultChainID), // Use Pax's chain ID
			Nonce:      nonce,
	...
	// Sign the transaction with Pax's chain ID
...
// EncodeTxForApp encodes a signed transaction for the Pax app
