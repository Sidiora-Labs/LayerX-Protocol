...
// pax-node-* containers. Subtests read it (and maxFaults) from here.
var (
	clusterSize int
	maxFaults   int
)
...
// currently-running pax-node-* container's logs. Guards against accidental
...
func assertAutobahnEnabled(t *testing.T) {
	...
	if len(names) == 0 {
		t.Fatalf("no running pax-node-* containers")
	}
	for _, name := range names {
		// paxd writes logs to a file inside the container (not stdout), so we
		// grep via docker exec rather than `docker logs`. Each container only
		// has its own paxd-<id>.log under the repo-relative build/generated/logs.
		cmd := exec.Command("docker", "exec", name, "sh", "-c",
			"grep -q 'GigaRouter initialized' build/generated/logs/paxd-*.log")
...
func setupCluster() error {
	...
	expected, err := countPaxContainers()
	if err != nil {
		return fmt.Errorf("count cluster containers: %w", err)
	}
	if expected == 0 {
		return fmt.Errorf("no pax-node-* containers found after docker-cluster-start")
	}
...
// countPaxContainers returns the number of pax-node-* containers that exist
// (running or not yet started).
func countPaxContainers() (int, error) {
...
// restartNode re-invokes the container's paxd-start script inside pax-node-<i>.
// The script backgrounds paxd and exits, so `docker exec -d` is the right mode:
// it returns immediately while paxd keeps running.
//
// Precondition: paxd must NOT already be running on the target. start_pax.sh
// unconditionally spawns a new paxd process; calling this while one is alive
// produces two paxd instances in the same container (port/CMS-lock conflict).
// Callers should `killNode` first, or extend the script to pkill defensively.
func restartNode(t *testing.T, i int) {
	t.Helper()
	t.Logf("restarting paxd on node %d...", i)
	name := fmt.Sprintf("pax-node-%d", i)
	cmd := exec.Command("docker", "exec", "-d",
		"-e", fmt.Sprintf("ID=%d", i),
		name, "/usr/bin/start_pax.sh")
	if out, err := cmd.CombinedOutput(); err != nil {
...
func testRecovery(t *testing.T) {
	...
	// Poll for the chain to advance. Give the restarted paxd time to init
	...
	// node is among them, and start_pax.sh truncates its log on restart (`>`
...
func testBankTransfer(t *testing.T) {
	...
	createOut := dockerExec(t, "pax-node-0",
		"printf '12345678\n12345678\n' | paxd keys add test_recipient --output json 2>/dev/null")
	var key struct {
	...
	sendCmd := fmt.Sprintf(
		"printf '12345678\n' | paxd tx bank send node_admin %s 1000000uhpx "+
			"--chain-id pax --fees 2000uhpx -b sync -y --output json",
		key.Address)
	dockerExec(t, "pax-node-0", sendCmd)

	// Poll for balance. Tolerate transient query failures before the tx finalizes.
	t.Log("waiting for tx to finalize...")
	queryCmd := fmt.Sprintf("paxd q bank balances %s --denom uhpx --output json 2>/dev/null", key.Address)
	var balance string
	for attempt := 0; attempt < 15; attempt++ {
		out, _ := exec.Command("docker", "exec", "pax-node-0", "sh", "-c", queryCmd).Output()
		var b struct {
	...
	t.Logf("balance: %s uhpx", balance)
	if balance != "1000000" {
		t.Fatalf("expected balance 1000000, got %s", balance)
	}
}

// killNode kills paxd inside pax-node-<i> via pkill. Tolerates non-zero exit
// (e.g. the process already gone).
func killNode(t *testing.T, i int) {
	t.Helper()
	t.Logf("killing paxd on node %d...", i)
	dockerExecAllowFail(fmt.Sprintf("pax-node-%d", i), "pkill paxd")
}
