...
echo "$beginning_block_height" > $paxhome/integration_test/contracts/tfk_beginning_block_height.txt
echo "$keyaddress"  > $paxhome/integration_test/contracts/tfk_creator_id.txt
...
create_denom_and_wait() {
    ...
    local resp; resp=$(printf "12345678\n" | $paxdbin tx tokenfactory create-denom "$subdenom" \
        -y --from="$keyname" --chain-id="$chainid" --gas=500000 --fees=100000uhpx \
    ...
    _wait_until "tokenfactory denom $new_token_denom" "
        admin=\$($paxdbin q tokenfactory denom-authority-metadata $new_token_denom -o json 2>/dev/null | jq -r '.authority_metadata.admin // \"\"')
        [ -n \"\$admin\" ]
    " || return 1
...
echo "$first_set_block_height" > $paxhome/integration_test/contracts/tfk_first_set_block_height.txt
...
echo "$second_set_block_height" > $paxhome/integration_test/contracts/tfk_second_set_block_height.txt
...
echo "$third_set_block_height" > $paxhome/integration_test/contracts/tfk_third_set_block_height.txt

num_denoms=$(paxd q tokenfactory denoms-from-creator $CREATOR_ID --output json | jq -r ".denoms | length")
echo $num_denoms
