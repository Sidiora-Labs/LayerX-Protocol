...
CHAIN_ID=${FLATKV_EVM_FIXTURE_CHAIN_ID:-pax}
...
out_dir="$paxhome/integration_test/contracts"
...
run_paxd tx evm associate-address \
  --from "$FROM" \
  "${KEYRING_ARGS[@]}" \
  --chain-id "$CHAIN_ID" \
  -b sync \
...
if ! transfer_out=$(run_paxd tx evm send "$RECIPIENT_ADDR" "$TRANSFER_VALUE_WEI" \
  --from "$FROM" \
  "${KEYRING_ARGS[@]}" \
  --chain-id "$CHAIN_ID" \
  --evm-rpc "$RPC_URL" \
...
if ! deploy_out=$(run_paxd tx evm deploy "$contract_hex_file" \
  --from "$FROM" \
  "${KEYRING_ARGS[@]}" \
  --chain-id "$CHAIN_ID" \
  --evm-rpc "$RPC_URL" \
...
if [ "$BULK_STORAGE_KEYS" -gt 0 ]; then
  ...
  while [ "$deployed_bulk" -lt "$BULK_STORAGE_KEYS" ]; do
    ...
    if ! bulk_out=$(run_paxd tx evm deploy "$bulk_contract_hex_file" \
      --from "$FROM" \
      "${KEYRING_ARGS[@]}" \
      --chain-id "$CHAIN_ID" \
      --evm-rpc "$RPC_URL" \
