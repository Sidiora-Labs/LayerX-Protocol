...
# (storage/state_db/sc/flatkv/store_catchup_test.go). Neither reproduces the
...
#
# Voting-weight assumption: pax-node-3 is killed because the 4-validator
...
CRASH_NODE="pax-node-${CRASH_NODE_INDEX}"
SURVIVOR_NODE=${FLATKV_CRASH_SURVIVOR:-pax-node-0}
...
# paxd process can't gracefully flush flatkv pending writes; this is the
# essential difference vs the upgrade-test stop/start cycle.
echo "Killing $CRASH_NODE with SIGKILL..."
docker exec "$CRASH_NODE" pkill -9 -f "paxd start" >/dev/null 2>&1 || true
...
sleep 2
if docker exec "$CRASH_NODE" pgrep -f "paxd start" >/dev/null 2>&1; then
...
# exec session stays open after start_pax.sh backgrounds paxd and returns;
# a non-detached docker exec closes stdout/stderr when start_pax.sh exits,
# which would kill the freshly-spawned paxd process.
#
# step5_start_pax.sh truncates paxd-${ID}.log on restart (`>` not `>>`),
...
docker exec -d -e "ID=${CRASH_NODE_INDEX}" "$CRASH_NODE" /usr/bin/start_pax.sh

# Probe for "paxd is running" over a window rather than a single sleep so
# a slow startup (pebble WAL recovery after SIGKILL, tendermint state
# repair, etc.) is not misclassified as "stayed down". If paxd is not
...
echo "$CRASH_NODE paxd process is running after restart"
...
for i in $(seq 0 $((NODE_COUNT - 1))); do
  wait_for_height "pax-node-$i" "$POST_RESTART_COMPARE_FLOOR" "$CATCHUP_TIMEOUT"
done

# Step 6: build paxdb everywhere (the crashed node may have had its previous
...
if [ -z "$COMPARE_VERSION" ] || [ "$COMPARE_VERSION" -lt 1 ]; then
  ...
  for i in $(seq 0 $((NODE_COUNT - 1))); do
    echo "  pax-node-$i height = $(node_height "pax-node-$i")" >&2
  done
  exit 1
fi
