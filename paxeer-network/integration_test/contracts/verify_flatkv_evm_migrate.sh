...
dump_node_log() {
  ...
  echo "==================== ${node} paxd log error excerpt ====================" >&2
  ...
  echo "==================== ${node} paxd log (head 220 lines) ====================" >&2
  docker exec "$node" head -220 "$logfile" >&2 2>/dev/null \
    || echo "(could not read ${logfile})" >&2
  echo "==================== ${node} paxd log (tail 400 lines) ====================" >&2
  docker exec "$node" tail -400 "$logfile" >&2 2>/dev/null \
    || echo "(could not read ${logfile})" >&2
  echo "==================== ${node} pgrep paxd ====================" >&2
  docker exec "$node" pgrep -af "paxd" >&2 2>/dev/null \
    || echo "(no paxd processes)" >&2
  echo "==================== ${node} docker logs (last 200 lines) ====================" >&2
  docker logs --tail 200 "$node" >&2 || true
}

node_height() {
  docker exec "$1" build/paxd status 2>/dev/null \
...
node_logged_committed_height() {
  ...
  local node_id=${node#pax-node-}
  local logfile="/pax-protocol/pax-chain/build/generated/logs/paxd-${node_id}.log"
  local host_logfile="build/generated/logs/paxd-${node_id}.log"
  local height=""
...
node_last_sign_height() {
  ...
  height=$(docker exec "$node" jq -r '.height // "0"' /root/.pax/data/priv_validator_state.json 2>/dev/null \
    || echo 0)
...
print_migration_summaries() {
  ...
  for i in $(seq 0 $((NODE_COUNT - 1))); do
    ...
    local logfile="/pax-protocol/pax-chain/build/generated/logs/paxd-${i}.log"
    local summary=""
    local keys_migrated=""
  ...
  for i in $(seq 0 $((NODE_COUNT - 1))); do
    ...
    docker exec -d \
      ...
      "$node" bash -lc '
        ...
        /usr/bin/start_pax.sh
      '
  done

  wait_for_all_paxd_start "$label"
}

wait_for_all_paxd_stop() {
  local label=$1
  local timeout=$2
  local elapsed=0
...
stop_all_validators() {
  ...
  for i in $(seq 0 $((NODE_COUNT - 1))); do
    node="pax-node-$i"
    docker exec "$node" pkill -TERM -f "paxd start" >/dev/null 2>&1 || true &
  done
  wait
  wait_for_all_paxd_stop "$label" "$STOP_TIMEOUT"
}
...
configure_halt_height() {
  ...
  written_halt=$(docker exec "pax-node-0" grep -E '^halt-height' "$APP_CONFIG" \
    | tail -1 | awk -F'=' '{print $2}' | tr -d ' "' || true)
  if [ -z "$written_halt" ] || [ "$written_halt" != "$height" ]; then
    echo "ERROR: pax-node-0 app.toml has halt-height='$written_halt' after rewrite; expected '$height'" >&2
    exit 1
  fi
}
...
wait_for_all_paxd_stop "halted at height $HALT_HEIGHT" "$HALT_TIMEOUT"
...
for i in $(seq 0 $((NODE_COUNT - 1))); do
  node="pax-node-$i"
  # The TOML key must match the FlagSC* constant in node/paxdb.go
  ...
  # read it -- the paxd log showed KeysToMigratePerBlock:1024 (default)
...
written_batch=$(docker exec "pax-node-0" grep -E '^sc-keys-to-migrate-per-block' "$APP_CONFIG" \
  | tail -1 | awk -F'=' '{print $2}' | tr -d ' "' || true)
if [ -z "$written_batch" ] || [ "$written_batch" != "$KEYS_TO_MIGRATE_PER_BLOCK" ]; then
  echo "ERROR: pax-node-0 app.toml has sc-keys-to-migrate-per-block='$written_batch' after rewrite; expected '$KEYS_TO_MIGRATE_PER_BLOCK'" >&2
  exit 1
fi
...
#
# Poll paxdb migrate-evm-status against each node's flatkv dir. The tool
...
while [ "$elapsed" -lt "$COMPLETION_TIMEOUT" ]; do
  ...
  for i in $(seq 0 $((NODE_COUNT - 1))); do
    ...
    raw_status=$(docker exec "$node" bash -lc \
      "build/paxdb migrate-evm-status --db-dir '$FLATKV_DIR' 2>$status_err" || true)
    # paxdb opens FlatKV and its dependencies may write diagnostics to stdout
