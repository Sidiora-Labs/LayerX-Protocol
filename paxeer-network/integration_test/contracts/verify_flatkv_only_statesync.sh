...
DONOR_NODE=${FLATKV_ONLY_STATESYNC_DONOR:-pax-node-0}
SECOND_RPC_NODE=${FLATKV_ONLY_STATESYNC_SECOND_RPC:-pax-node-1}
THIRD_PEER_NODE=${FLATKV_ONLY_STATESYNC_THIRD_PEER:-pax-node-2}
FLATKV_DIR=${FLATKV_DIR:-/root/.pax/data/state_commit/flatkv}
APP_CONFIG=${APP_CONFIG:-/root/.pax/config/app.toml}
...
dump_node_log() {
  ...
  echo "==================== ${node} paxd log ${logfile} (last 240 lines) ====================" >&2
...
assert_flatkv_only_layout() {
  ...
  for i in $(seq 0 $((NODE_COUNT - 1))); do
    ...
    if ! docker exec "$node" bash -lc "
      ...
      test -d '$FLATKV_DIR'
      for path in /root/.pax/data/state_commit/memiavl /root/.pax/data/committer.db; do
...
configure_statesync() {
  ...
  docker exec "$victim" bash -lc "
    set -euo pipefail
    peers=\$(grep -v '^$' /pax-protocol/pax-chain/build/generated/persistent_peers.txt | grep -v '$victim_ip' | paste -sd ',' -)
    sed -i.bak \
...
assert_statesync_configured() {
  ...
  if ! docker exec "$victim" bash -lc "
    set -euo pipefail
    section=\$(awk '/^\[statesync\]/{flag=1;print;next} /^\[/{flag=0} flag' /root/.pax/config/config.toml)
...
for i in $(seq 0 $((NODE_COUNT - 1))); do
  wait_for_evm_rpc "pax-node-$i" 60
done

for i in $(seq 0 $((NODE_COUNT - 1))); do
  ensure_paxdb "pax-node-$i"
done
...
docker exec "$VICTIM_NODE" bash -lc "
  set -euo pipefail
  cp /root/.pax/data/priv_validator_state.json /tmp/flatkv-only-priv-validator-state.json
  ...
  mv /tmp/flatkv-only-priv-validator-state.json /root/.pax/data/priv_validator_state.json
