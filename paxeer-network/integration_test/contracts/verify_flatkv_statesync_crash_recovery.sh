...
VICTIM_NODE="pax-node-${VICTIM_INDEX}"
DONOR_NODE=${FLATKV_STATESYNC_DONOR:-pax-node-0}
SECOND_RPC_NODE=${FLATKV_STATESYNC_SECOND_RPC:-pax-node-1}
...
dump_node_log() {
  ...
  node_id=${node#pax-node-}
  if [ "$node_id" = "$node" ]; then
    logfile="/pax-protocol/pax-chain/build/generated/logs/rpc-node.log"
  else
    logfile="/pax-protocol/pax-chain/build/generated/logs/paxd-${node_id}.log"
  fi
  echo "==================== ${node} paxd log ${logfile} (last 200 lines) ====================" >&2
...
dump_statesync_config() {
  local node=$1
  echo "--- ${node} effective [statesync] section of /root/.pax/config/config.toml ---" >&2
  docker exec "$node" bash -lc \
    "awk '/^\[statesync\]/{flag=1;print;next} /^\[/{flag=0} flag' /root/.pax/config/config.toml" >&2 || true
}

node_height() {
  local node=$1
  docker exec "$node" build/paxd status 2>/dev/null \
...
ensure_paxdb() {
  local node=$1
  if docker exec "$node" test -x /pax-protocol/pax-chain/build/paxdb >/dev/null 2>&1; then
    return 0
  fi
  echo "Building paxdb on $node..."
  docker exec -e GOPROXY="${GOPROXY:-https://proxy.golang.org,direct}" "$node" bash -lc \
    "cd /pax-protocol/pax-chain && $GO_BIN build -o build/paxdb ./storage/tools/cmd/paxdb"
}
...
# (pax-cosmos config uses dotted path state-commit.sc-write-mode, so the
...
dump_state_commit_section() {
  local node=$1
  echo "--- ${node} effective [state-commit] section of /root/.pax/config/app.toml ---" >&2
  docker exec "$node" bash -lc \
    "awk '/^\[state-commit\]/{flag=1;print;next} /^\[/{flag=0} flag' /root/.pax/config/app.toml" >&2 || true
}
...
dump_parsed_sc_config() {
  local node=$1
  local node_id=${node#pax-node-}
  local logfile="/pax-protocol/pax-chain/build/generated/logs/paxd-${node_id}.log"
  echo "--- ${node} parsed PaxDB SC config (from log, source of truth) ---" >&2
  docker exec "$node" bash -lc \
    "grep -m1 'PaxDB SC is enabled now' '$logfile' 2>/dev/null" >&2 \
    || echo "(no 'PaxDB SC is enabled now' line found)" >&2
}
...
dump_flatkv_bucket_summary() {
  local node=$1
  ensure_paxdb "$node" >/dev/null 2>&1 || true
  echo "--- ${node} FlatKV bucket row counts and fixture presence ---" >&2
  docker exec "$node" bash -lc "
    set +e
    out_dir=/tmp/flatkv-debug-${node}
    rm -rf \"\$out_dir\" && mkdir -p \"\$out_dir\"
    cd /pax-protocol/pax-chain
    if ! build/paxdb dump-flatkv --db-dir $FLATKV_DIR --output-dir \"\$out_dir\" >/dev/null 2>&1; then
...
dump_snapshot_diagnostics() {
  ...
  echo "--- effective [state-sync] section of ~/.pax/config/app.toml ---" >&2
  docker exec "$node" bash -lc "awk '/^\[state-sync\]/{flag=1;print;next} /^\[/{flag=0} flag' /root/.pax/config/app.toml" >&2 || true
...
configure_statesync() {
  ...
  docker exec "$victim" bash -lc "
    set -euo pipefail
    peers=\$(grep -v '^$' /pax-protocol/pax-chain/build/generated/persistent_peers.txt | paste -sd ',' -)
    ...
    sed -i.bak \
      ...
      /root/.pax/config/config.toml
    sed -i.bak -e \"s|^persistent-peers *=.*|persistent-peers = \\\"\${peers}\\\"|\" /root/.pax/config/config.toml
  "
}
...
assert_statesync_configured() {
  ...
  if docker exec "$node" bash -lc "
    set -euo pipefail
    section=\$(awk '/^\[statesync\]/{flag=1;next} /^\[/{flag=0} flag' /root/.pax/config/config.toml)
    printf '%s\n' \"\$section\" | awk '
...
start_victim() {
  docker exec -d -e "ID=${VICTIM_INDEX}" "$VICTIM_NODE" /usr/bin/start_pax.sh
}
...
wait_for_process() {
  ...
  local elapsed=0
  while [ "$elapsed" -lt "$timeout" ]; do
    if docker exec "$node" pgrep -f "paxd start" >/dev/null 2>&1; then
      echo "$node paxd process is running"
      return 0
    fi
    sleep 1
    elapsed=$((elapsed + 1))
...
wait_for_statesync_log_and_kill() {
  ...
  local timeout=$2
  local elapsed=0
  local log_path="/pax-protocol/pax-chain/build/generated/logs/paxd-${VICTIM_INDEX}.log"
  ...
  while [ "$elapsed" -lt "$timeout" ]; do
    if docker exec "$node" bash -lc "grep -Eiq '$regex' '$log_path' 2>/dev/null"; then
      ...
      docker exec "$node" pkill -9 -f "paxd start" >/dev/null 2>&1 || true
      return 0
    fi
    if ! docker exec "$node" pgrep -f "paxd start" >/dev/null 2>&1; then
...
assert_flatkv_dump_contains_fixture() {
  ...
  if ! docker exec "$node" bash -lc "
    ...
    # persisted by Pax's EVM keeper, so the recipient never appears in
...
docker exec "$VICTIM_NODE" pkill -f "paxd start" >/dev/null 2>&1 || true
sleep 2
...
docker exec "$VICTIM_NODE" bash -lc "
  set -euo pipefail
  cp /root/.pax/data/priv_validator_state.json /tmp/flatkv-priv-validator-state.json
  rm -rf /root/.pax/data /root/.pax/wasm /pax-protocol/pax-chain/build/generated/node_${VICTIM_INDEX}/snapshots
  mkdir -p /root/.pax/data /pax-protocol/pax-chain/build/generated/node_${VICTIM_INDEX}/snapshots
  mv /tmp/flatkv-priv-validator-state.json /root/.pax/data/priv_validator_state.json
  sed -i.bak -e 's|^snapshot-directory *=.*|snapshot-directory = \"./build/generated/node_${VICTIM_INDEX}/snapshots\"|' /root/.pax/config/app.toml
"
...
sleep 2
if docker exec "$VICTIM_NODE" pgrep -f "paxd start" >/dev/null 2>&1; then
