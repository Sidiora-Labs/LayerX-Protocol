...
DONOR_NODE=${FLATKV_TOTAL_LOSS_DONOR:-pax-node-0}
SECOND_RPC_NODE=${FLATKV_TOTAL_LOSS_SECOND_RPC:-pax-node-1}
...
stop_victim() {
  ...
  docker exec "$VICTIM_NODE" pkill -TERM -f "paxd start" >/dev/null 2>&1 || true
  while [ "$elapsed" -lt "$timeout" ]; do
    if ! docker exec "$VICTIM_NODE" pgrep -f "paxd start" >/dev/null 2>&1; then
      echo "$VICTIM_NODE paxd process stopped"
      return 0
    fi
    sleep 1
    elapsed=$((elapsed + 1))
  ...
  docker exec "$VICTIM_NODE" pkill -9 -f "paxd start" >/dev/null 2>&1 || true
  sleep 1
  if docker exec "$VICTIM_NODE" pgrep -f "paxd start" >/dev/null 2>&1; then
  ...
  echo "$VICTIM_NODE paxd process stopped after SIGKILL"
}
...
assert_flatkv_dump_contains_fixture() {
  ...
  if ! docker exec "$node" bash -lc "
    ...
    # default nonce=0 / codehash=keccak('') values that Pax's EVM keeper
