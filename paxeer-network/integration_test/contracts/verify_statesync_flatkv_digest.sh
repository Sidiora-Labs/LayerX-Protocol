...
# (pax-rpc-node) FlatKV and the donor's (pax-node-0) FlatKV at the same
...
DONOR=${FLATKV_DIGEST_DONOR:-pax-node-0}
RECEIVER=${FLATKV_DIGEST_RECEIVER:-pax-rpc-node}
...
dump_node_log() {
  ...
  if [ "$node_id" = "$node" ]; then
    # pax-rpc-node (or any non pax-node-N container) writes to rpc-node.log
    # via docker/rpcnode/scripts/step2_start_pax.sh.
    logfile="/pax-protocol/pax-chain/build/generated/logs/rpc-node.log"
  else
    # Validator nodes write to paxd-<ID>.log via
    # docker/localnode/scripts/step5_start_pax.sh.
