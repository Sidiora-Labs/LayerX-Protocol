...
3 chain types are supported, `paxlocal`, `devnet` (arctic-1) and `testnet` (atlantic-2). The configs for each chain are stored in `./hardhat.config.js`.

If running on `paxlocal`, the script assumes that a local instance of the chain is running by running `/scripts/initialize_local_chain.sh`.
...
- Deployer Pax address: `pax1rtpakm7w9egh0n7xngzm6vrln0szv6yea7kez6`
