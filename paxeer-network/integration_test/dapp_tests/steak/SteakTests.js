...
describe("Steak", async function () {
  ...
  async function setupAccount(baseName, associate = true, amount="100000000000", denom="uhpx", funder='admin') {
    const uniqueName = `${baseName}-${uuidv4()}`;

    const account = await addAccount(uniqueName);
    await fundPaxAddress(account.address, amount, denom, funder);
    if (associate) {
  ...
  describe("Bonding and unbonding", async function () {
    ...
    it("Unassociated account should be able to bond", async function () {
      const unassociatedAccount = await setupAccount("unassociated", false, '2000000', 'uhpx', owner.address);
      ...
      const newUnassociatedAccount = await setupAccount("unassociated", false, '2000000', 'uhpx', owner.address);
  ...
  after(async function () {
    ...
    console.log(`Resetting to ${originalPaxdConfig}`)
