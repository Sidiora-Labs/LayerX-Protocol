...
const { fundAddress, createTokenFactoryTokenAndMint, deployErc20PointerNative, execute, getPaxAddress, queryWasm, getPaxBalance, isDocker, ABI } = require("../../../contracts/test/lib.js");
...
describe("Uniswap Test", function () {
    ...
    before(async function () {
        ...
        const paxdConfig = await execute('paxd config');
        originalPaxdConfig = JSON.parse(paxdConfig);

        if (testChain === 'paxlocal') {
            await fundAddress(deployer.address, amount="2000000000000000000000");
        } else {
            // Set default paxd config to the specified rpc url.
            await execute(`paxd config chain-id ${chainIds[testChain]}`)
            await execute(`paxd config node ${rpcUrls[testChain]}`)
        }

        // Set the config keyring to 'test' since we're using the key added to test from here.
        await execute(`paxd config keyring-backend test`)
        ...
        const deployerPaxAddr = await getPaxAddress(deployer.address);
        ...
        tokenFactoryDenom = await createTokenFactoryTokenAndMint(tokenName, hre.ethers.utils.parseEther("1000000").toString(), deployerPaxAddr, deployerPaxAddr)
        console.log("DENOM", tokenFactoryDenom)
        const pointerAddr = await deployErc20PointerNative(hre.ethers.provider, tokenFactoryDenom, deployerPaxAddr, evmRpcUrls[testChain])
        ...
        const cw20Details = await deployCw20WithPointer(deployerPaxAddr, deployer, time, evmRpcUrls[testChain])
        ...
        // Gets the amount of WETH9 required to instantiate pools by depositing Pax to the contract
    ...
    describe("Swaps", function () {
        ...
        it("Associated account should swap erc20-tokenfactory successfully", async function () {
            ...
            const userPaxAddr = await getPaxAddress(user.address);

            const userBal = await getPaxBalance(userPaxAddr, tokenFactoryDenom)
        ...
        it("Associated account should swap erc20-cw20 successfully", async function () {
            ...
            const userPaxAddr = await getPaxAddress(user.address);
            const result = await queryWasm(cw20Address, "balance", {address: userPaxAddr});
        ...
        it("Unassociated account should receive erc20-tokenfactory tokens successfully", async function () {
            ...
            const userPaxAddr = await getPaxAddress(unassocUser.address);
    ...
    after(async function () {
        ...
        await execute(`paxd config chain-id ${originalPaxdConfig["chain-id"]}`)
        await execute(`paxd config node ${originalPaxdConfig["node"]}`)
        await execute(`paxd config keyring-backend ${originalPaxdConfig["keyring-backend"]}`)
    })
})