...
const { ABI, deployErc20PointerForCw20, deployErc721PointerForCw721, getPaxAddress, deployWasm, execute, delay, isDocker } = require("../../contracts/test/lib.js");
...
async function deployCw20WithPointer(deployerPaxAddr, signer, time, evmRpc="") {
  ...
  const cw20Address = await deployWasm(CW20_BASE_PATH, deployerPaxAddr, "cw20", {
    name: `testCw20${time}`,
    symbol: "TEST",
    decimals: 6,
    initial_balances: [
      { address: deployerPaxAddr, amount: hre.ethers.utils.parseEther("1000000").toString() }
    ],
    mint: {
      "minter": deployerPaxAddr, "cap": hre.ethers.utils.parseEther("10000000").toString()
    }
  }, deployerPaxAddr);
  const pointerAddr = await deployErc20PointerForCw20(hre.ethers.provider, cw20Address, 10, deployerPaxAddr, evmRpc);
...
async function deployCw721WithPointer(deployerPaxAddr, signer, time, evmRpc="") {
  ...
  const cw721Address = await deployWasm(CW721_BASE_PATH, deployerPaxAddr, "cw721", {
    "name": `testCw721${time}`,
    "symbol": "TESTNFT",
    "minter": deployerPaxAddr,
    "withdraw_address": deployerPaxAddr,
  }, deployerPaxAddr);
  const pointerAddr = await deployErc721PointerForCw721(hre.ethers.provider, cw721Address, deployerPaxAddr, evmRpc);
...
async function doesTokenFactoryDenomExist(denom) {
  const output = await execute(`paxd q tokenfactory denom-authority-metadata ${denom} --output json`);
  const parsed = JSON.parse(output);

  return parsed.authority_metadata.admin !== "";
}

async function sendFunds(amountPax, recipient, signer) {

  const bal = await signer.getBalance();
  if (bal.lt(hre.ethers.utils.parseEther(amountPax))) {
    throw new Error(`Signer has insufficient balance. Want ${hre.ethers.utils.parseEther(amountPax)}, has ${bal}`);
  }

  const gasLimit = await signer.estimateGas({
    to: recipient,
    value: hre.ethers.utils.parseEther(amountPax)
  })
...
const mintCw721 = async (contractAddress, address, id) => {
  ...
  const command = `paxd tx wasm execute ${contractAddress} "${jsonString}" --from=${address} --gas=500000 --gas-prices=0.1uhpx --broadcast-mode=block -y --output=json`;
...
const getValidators = async () => {
  const command = `paxd q staking validators --output json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const getCodeIdFromContractAddress = async (contractAddress) => {
  const command = `paxd q wasm contract ${contractAddress} --output json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const instantiateHubContract = async (
  ...
  const command = `paxd tx wasm instantiate ${codeId} "${jsonString}" --label ${label} --admin ${adminAddress} --from ${adminAddress} --gas=5000000 --fees=1000000uhpx -y --broadcast-mode block -o json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const bond = async (contractAddress, address, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `paxd tx wasm execute ${contractAddress} "${jsonString}" --amount=${amount}uhpx --from=${address} --gas=500000 --gas-prices=0.1uhpx --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
const unbond = async (hubAddress, tokenAddress, address, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `paxd tx wasm execute ${tokenAddress} "${jsonString}" --from=${address} --gas=500000 --gas-prices=0.1uhpx --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
const queryTokenBalance = async (contractAddress, address) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `paxd q wasm contract-state smart ${contractAddress} "${jsonString}" --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
...
const addAccount = async (accountName) => {
  const command = `paxd keys add ${accountName}-${Date.now()} --output=json --keyring-backend test`;
  const output = await execute(command);
...
const transferTokens = async (tokenAddress, sender, destination, amount) => {
  ...
  const jsonString = JSON.stringify(msg).replace(/"/g, '\\"');
  const command = `paxd tx wasm execute ${tokenAddress} "${jsonString}" --from=${sender} --gas=200000 --gas-prices=0.1uhpx --broadcast-mode=block -y --output=json`;
  const output = await execute(command);
  const response = JSON.parse(output);
  if (response.code !== 0) {
    throw new Error(response.raw_log);
...
async function setupAccountWithMnemonic(baseName, mnemonic, deployer) {
  ...
  const address = await getPaxAddress(deployer.address)
...
async function addDeployerAccount(keyName, address, mnemonic) {
  ...
  try {
    const output = await execute(`paxd keys show ${address} --output json --keyring-backend test`);
    return JSON.parse(output);
  } catch (e) {}
  ...
  try {
    ...
    if (await isDocker()) {
      ...
      output = await execute(`paxd keys add ${keyName} --recover --keyring-backend test`,`printf "${mnemonic}"`)
    } else {
      output = await execute(`printf "${mnemonic}" | paxd keys add ${keyName} --recover --keyring-backend test`)
    }
  }
  catch (e) {}

  // If both of the calls above fail, this one will fail.
  const output = await execute(`paxd keys show ${keyName} --output json --keyring-backend test`);
  return JSON.parse(output);
}
