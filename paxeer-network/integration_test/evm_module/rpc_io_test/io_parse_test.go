...
func TestParseIOFile_ExpectResponseHeader(t *testing.T) {
	content := `>> {"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":["latest",false]}
<< {"jsonrpc":"2.0","id":1,"result":{}}
@ expect_response_header Pax-Legacy-RPC-Deprecation
`
	pairs, err := parseIOFile(content)
	if err != nil {
		t.Fatalf("parse: %v", err)
	...
	if len(pairs[0].ExpectResponseHeaders) != 1 || pairs[0].ExpectResponseHeaders[0] != "Pax-Legacy-RPC-Deprecation" {
