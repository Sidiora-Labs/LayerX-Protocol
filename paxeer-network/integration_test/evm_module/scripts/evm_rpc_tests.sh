#!/usr/bin/env bash
set -e
# Run execution-apis .io/.iox tests against local Pax EVM RPC. Start cluster first: make docker-cluster-start
# Usage: from repo root: ./integration_test/evm_module/scripts/evm_rpc_tests.sh
cd "$(dirname "$0")/../../.."

CONTAINER="${PAX_EVM_IO_TX_CONTAINER:-pax-node-0}"
PASSWORD="${PAX_EVM_IO_TX_PASSWORD:-12345678}"
FROM="${PAX_EVM_IO_TX_FROM:-admin}"
RECIPIENT="${PAX_EVM_IO_TX_RECIPIENT:-0xF87A299e6bC7bEba58dbBe5a5Aa21d49bCD16D52}"
PROJECT_ROOT="${PAX_EVM_IO_PROJECT_ROOT:-/pax-protocol/pax-chain}"
...
EVM_RPC_URL="${PAX_EVM_RPC_URL:-http://localhost:8545}"
KEYRING_ARGS=()
if [[ -n "${PAX_EVM_IO_KEYRING_BACKEND:-}" ]]; then
  KEYRING_ARGS+=(--keyring-backend "$PAX_EVM_IO_KEYRING_BACKEND")
fi

run() {
  docker exec --env PAX_EVM_IO_PASSWORD="$PASSWORD" "$CONTAINER" /bin/bash -c \
    'export PATH=$PATH:/root/go/bin && printf "%s\n" "$PAX_EVM_IO_PASSWORD" | "$@"' bash "$@"
}
...
# under set -e.
FROM_ADDR=$(run paxd keys show "$FROM" "${KEYRING_ARGS[@]}" -a 2>/dev/null) || FROM_ADDR=
...
get_from_seq() {
  ...
  local s
  s=$(run paxd q account "$FROM_ADDR" -o json 2>/dev/null | jq -r '.sequence // ""' 2>/dev/null) || true
  echo "$s"
}
...
# Seed chain with block/tx/contract; export PAX_EVM_IO_SEED_BLOCK so .iox __SEED__ tag resolves to deploy block.
...
# leaves PAX_EVM_IO_SEED_BLOCK unset and skips the __SEED__ fixtures.
...
run paxd tx evm associate-address --from "$FROM" "${KEYRING_ARGS[@]}" --chain-id pax -b sync -y 2>/dev/null || true
wait_from_seq_advance "$SEQ_BEFORE_ASSOC"
SEQ_BEFORE_SEND=$(get_from_seq)
run paxd tx evm send "$RECIPIENT" 1 --from "$FROM" "${KEYRING_ARGS[@]}" --chain-id pax --evm-rpc "$EVM_RPC_URL" -b sync -y
wait_from_seq_advance "$SEQ_BEFORE_SEND"
SEQ_BEFORE_MINIMAL=$(get_from_seq)
DEPLOY_OUT=$(run paxd tx evm deploy /tmp/minimal_contract.hex --from "$FROM" "${KEYRING_ARGS[@]}" --chain-id pax --evm-rpc "$EVM_RPC_URL" -b sync -y 2>&1) || true
...
if [[ -n "$DEPLOY_TX" ]]; then
  ...
  if [[ -n "$SEED" ]]; then
    export PAX_EVM_IO_SEED_BLOCK="$SEED"
    export PAX_EVM_IO_DEPLOY_TX_HASH="$DEPLOY_TX"
  fi
fi

# Deploy reverter contract (reverts with Error("user error")); export PAX_EVM_IO_REVERTER_ADDRESS for .iox __REVERTER__ tag.
...
REVERTER_OUT=$(run paxd tx evm deploy /tmp/reverter_contract.hex --from "$FROM" "${KEYRING_ARGS[@]}" --chain-id pax --evm-rpc "$EVM_RPC_URL" -b sync -y 2>&1) || true
...
if [[ -n "$REVERTER_TX" ]]; then
  ...
  if [[ -n "$REVERTER_ADDR" ]]; then
    export PAX_EVM_IO_REVERTER_ADDRESS="$REVERTER_ADDR"
  fi
fi
if [[ -z "${PAX_EVM_IO_REVERTER_ADDRESS:-}" ]]; then
  echo "WARNING: Reverter contract not deployed (deploy or receipt lookup failed). Tests using __REVERTER__ will be skipped." >&2
fi

export PAX_EVM_IO_RUN_INTEGRATION=1
...
export PAX_EVM_WS_RUN_INTEGRATION=1
