 ...
 *   1. Verifies both endpoints (local Pax EVM RPC + local geth --dev reference) are
 *      reachable before deploying, since most specs compare geth against Pax.
...
import { gethRpc, isReachable, paxRpc } from '../utils/chainUtils';
...
import { fundAdminOnPax, generateMnemonic, paxAddressFromMnemonic } from '../utils/cosmosUtils';
import {
    isWasmEnabled,
    deployCw20,
    registerCw20Pointer,
...
describe('new_rpc_tests bootstrap', function () {
    ...
    before(async () => {
        // Use PAX_ADMIN_MNEMONIC when provided (e.g. a pre-funded key for a non-docker
    ...
    it('Pax EVM RPC is reachable', async () => {
        const ok = await isReachable(Endpoints.pax.evmRpc);
        expect(ok, `Pax EVM RPC at ${Endpoints.pax.evmRpc} is not reachable`).to.equal(true);
    });
    ...
    it('captures chain ids from both endpoints', async () => {
        const [paxChainId, gethChainId] = await Promise.all([
            paxRpc().send('eth_chainId', []),
        ...
        state.chainIds = {
            pax: Number(BigInt(paxChainId)),
            eth: Number(BigInt(gethChainId)),
        };
    });

    it('captures block heights before any deploys', async () => {
        const [paxBlock, gethBlock] = await Promise.all([
            paxRpc().getBlockNumber(),
            gethRpc().getBlockNumber(),
        ]);
        state.blocks = {
            paxBeforeDeploy: paxBlock,
            paxErc20Deploy: -1,
            paxAfterDeploy: -1,
    ...
    it('funds and associates the admin on Pax through docker node', async () => {
        await fundAdminOnPax(admin.address, adminMnemonic, paxRpc());
        expect(
    ...
    it('deploys the canonical TestERC20 and mints to admin', async () => {
        ...
        state.blocks!.paxErc20Deploy = receipt.blockNumber;
    ...
    it('deploys the SimpleAccount7702 delegation target on Pax', async () => {
    ...
    it('deploys the RealGasBurner on Pax', async () => {
    ...
    it('mirrors the TestERC20 deploy on the geth reference and mints to the geth admin', async () => {
        ...
        // Same contract, same constructor (initialOwner = deployer), same mint as Pax
    ...
    it('pre-funds a pool of fresh EVM accounts', async () => {
        pool = Array.from({ length: POOL_SIZE }, () => EvmAccount.random(paxRpc()));
    ...
    it('deploys a CW20 + ERC20 pointer when wasm is enabled', async function () {
        ...
        // stays undefined and buildRichPaxBlock omits the pointer / cw20 transfer.
        ...
        const actor = EvmAccount.random(paxRpc());
        ...
        // CW20 pointer resolves it to the same pax address we mint to below.
        ...
        // holds a CW20 balance under its pubkey-derived pax address.
        const adminPax = await paxAddressFromMnemonic(adminMnemonic);
        const holders = [adminPax, actor.paxAddress(), ...pool.map(p => p.paxAddress())];
        const initMsg: Cw20InitMsg = {
            name: 'RpcTests CW20',
            symbol: 'RPCW',
            decimals: CW20_DECIMALS,
            initial_balances: holders.map(address => ({ address, amount: CW20_MINT })),
            mint: { minter: adminPax },
        };
        ...
        expect(cw20, 'CW20 contract address').to.match(/^pax1[0-9a-z]+$/);
    ...
    it('records the post-deploy block height and writes runtime/runtime.json', async () => {
        ...
        const paxBefore = state.blocks!.paxBeforeDeploy;
        const paxAfter = await waitUntil(
            async () => {
                const h = await paxRpc().getBlockNumber();
                return h > paxBefore ? h : null;
            },
            { timeoutMs: 30_000, label: 'Pax block height to advance past bootstrap' },
        );
        state.blocks!.paxAfterDeploy = paxAfter;
        state.bootstrappedAt = new Date().toISOString();

        const finalised = state as RuntimeState;
        writeRuntimeState(finalised);

        expect(finalised.blocks.paxAfterDeploy).to.be.greaterThan(
            finalised.blocks.paxBeforeDeploy,
            'expected Pax to advance at least one block during bootstrap',
        );
