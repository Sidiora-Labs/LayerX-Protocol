...
describe('eth_blockNumber Tests', function () {
    ...
    describe('eth_blockNumber Queries', () => {
        it('returns a canonical hex quantity > 0', async () => {
            const hex = await pax.send('eth_blockNumber', []);
    ...
    describe('schema matching', () => {
        it('Pax and geth both return canonical hex quantities', async () => {
            const [paxHex, gethHex] = await Promise.all([
                pax.send('eth_blockNumber', []),
                geth.send('eth_blockNumber', []),
            ]);
            expect(paxHex, 'pax').to.match(HEX_QUANTITY);
        ...
        it('has no leading zeros in the hex encoding', async () => {
            const hex: string = await pax.send('eth_blockNumber', []);
    ...
    describe('empty / null handling', () => {
        ...
        it('always returns a non-empty hex string (raw transport)', async () => {
            const body = await rawPax<string>('eth_blockNumber', []);
