...
import { bothProviders, rawPax, rawGeth, captureRpcError, expectJsonRpcError } from '../utils/chainUtils';
...
describe('eth_call Tests', function () {
    ...
    before(async function () {
        ...
        erc20Pax = runtime.contracts.erc20;
        erc20Geth = runtime.contracts.erc20Geth;
        paxAdmin = runtime.funded.admin;
        gethAdmin = runtime.funded.gethAdmin.address;

        [minter, alice, aliceRevert] = claimPool(runtime, pax, 3, 'eth_call');
    ...
    describe('eth_call Queries', () => {
        it('omitting the block tag defaults to latest for eth_call queries', async () => {
            const [withoutTag, withLatest] = await Promise.all([
                pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }]),
                pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, 'latest']),
            ]);
            expect(withoutTag).to.equal(withLatest);
        });

        it('a call against an EOA (no code set with EIP7702) returns 0x', async () => {
            const result = await pax.send('eth_call', [
                { to: paxAdmin, data: '0x12345678' },
                'latest',
            ]);
        ...
        it('simulating an ERC20 transfer does not change state', async () => {
            const before = erc20.decodeBalance(
                await pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, 'latest']),
            );
            const simulated = await pax.send('eth_call', [
                { from: paxAdmin, to: erc20Pax, data: erc20.transfer(paxAdmin, ethers.parseEther('1')) },
                'latest',
            ]);
        ...
        it('a call at a block before the contract was deployed returns 0x', async () => {
            const result = await pax.send('eth_call', [
                { to: erc20Pax, data: erc20.balanceOf(paxAdmin) },
                ethers.toQuantity(runtime.blocks.paxBeforeDeploy),
            ]);
            expect(result).to.equal('0x');
        });
        ...
        it('respects historical state across distinct mint blocks and does not leak latest state', async () => {
            ...
            const blockBeforeFirstMint = await pax.getBlockNumber();
            const erc20Contract = new ethers.Contract(erc20Pax, erc20Iface, minter.wallet);
            ...
            const balanceAt = async (block: number): Promise<bigint> =>
                erc20.decodeBalance(
                    await pax.send('eth_call', [
                        { to: erc20Pax, data: erc20.balanceOf(recipient) },
        ...
        it('accepts an EIP-1898 blockNumber object equivalently to a tag', async () => {
            const latest = await pax.getBlockNumber();
            const [viaTag, viaObject] = await Promise.all([
                pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, ethers.toQuantity(latest)]),
                pax.send('eth_call', [
                    { to: erc20Pax, data: erc20.balanceOf(paxAdmin) },
        ...
        it('accepts an EIP-1898 blockHash object equivalently to a tag', async () => {
            const latest = await pax.getBlock('latest');
            expect(latest, 'latest block should exist').to.not.equal(null);
            const [viaTag, viaObject] = await Promise.all([
                pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, ethers.toQuantity(latest!.number)]),
                pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, { blockHash: latest!.hash! }]),
            ]);
            expect(viaObject).to.equal(viaTag);
        });

        it('returns the same result across latest, pending, safe and finalized tags', async () => {
            const latest = await pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, 'latest']);
            ...
            for (const tag of ['pending', 'safe', 'finalized']) {
                const result = await pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(paxAdmin) }, tag]);
        ...
        it('Honours a state override that replaces contract bytecode', async () => {
            ...
            const result = await pax.send('eth_call', [
                { to: erc20Pax, data: erc20.balanceOf(paxAdmin) },
                'latest',
                { [erc20Pax.toLowerCase()]: { code: stub } },
            ]);
        ...
        it('eth_call returns correct data with pax precompile calls (Staking Precompile)', async () => {
            ...
            const result = await pax.send('eth_call', [{ to: STAKING_PRECOMPILE_ADDRESS, data }, 'latest']);
            ...
            for (const v of validators) {
                expect(v.operatorAddress, 'bech32 valoper address').to.match(/^paxvaloper1[0-9a-z]{38}$/);
    ...
    describe('empty / null handling', () => {
        it('a void call returns the canonical "0x", not null (raw transport)', async () => {
            const body = await rawPax<string>('eth_call', [{ to: paxAdmin, data: '0x' }, 'latest']);
        ...
        it('a successful read never returns null (raw transport)', async () => {
            const body = await rawPax<string>('eth_call', [
                { to: erc20Pax, data: erc20.balanceOf(paxAdmin) },
                'latest',
            ]);
            expect(body.error, JSON.stringify(body.error)).to.equal(undefined);
    ...
    describe('wrong params / error handling', () => {
        it('rejects an invalid block tag identically to geth (-32602, exact message)', async () => {
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data }, 'banana']),
        ...
        it('rejects a malformed from address identically to geth (-32602, exact message)', async () => {
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ from: '0xdeadbeef', to: erc20Pax, data }, 'latest']),
        ...
        it('rejects an odd-length to address identically to geth (-32602, exact message)', async () => {
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: '0x123', data }, 'latest']),
        ...
        it('rejects non-hex data identically to geth (-32602, exact message)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data: 'notHex' }, 'latest']),
        ...
        it('rejects odd-length data identically to geth (-32602, exact message)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data: '0x123' }, 'latest']),
        ...
        it('rejects a non-hex gas value identically to geth (-32602, exact message)', async () => {
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data, gas: '-0x1' }, 'latest']),
        ...
        it('rejects non-array params identically to geth (-32602 "non-array args")', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_call', { to: erc20Pax, data: erc20.balanceOf(paxAdmin) }),
        ...
        it('rejects too many positional args identically to geth (-32602 "want at most 4")', async () => {
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data }, 'latest', {}, {}, {}]),
        ...
        it('rejects gas below the intrinsic minimum identically to geth (-32000, exact "want" value)', async () => {
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data, gas: '0x10' }, 'latest']),
        ...
        it('surfaces a revert with identical code 3, message and ABI-encoded error data on both', async () => {
            ...
            const data = erc20.transfer(paxAdmin, huge);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data }, 'latest']),
                rawGeth('eth_call', [{ to: erc20Geth, data }, 'latest']),
            ]);
        ...
        describe('an ERC20 transfer behaves identically across legacy / access-list / EIP-1559 call shapes', () => {
            ...
            before(async () => {
                const head = await pax.send('eth_getBlockByNumber', ['latest', false]);
        ...
        it('[divergence] far-future block: both -32000 but different messages', async () => {
            ...
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data }, ethers.toQuantity(latest + 1_000_000)]),
                rawGeth('eth_call', [{ to: erc20Geth, data }, '0xffffffff']),
            ]);
            expect(s.error?.code, 'pax code').to.equal(-32000);
        ...
        it('[divergence] unknown block hash: both -32000 but different messages', async () => {
            ...
            const data = erc20.balanceOf(paxAdmin);
            const [s, g] = await Promise.all([
                rawPax('eth_call', [{ to: erc20Pax, data }, { blockHash: zeroHash }]),
    ...
    describe('state override (5th parameter)', () => {
        it('balance override lets a zero-balance address spend in the simulated call', async () => {
            ...
            const paxBody = await rawPax('eth_call', [
            ...
            expect(paxBody.result, 'balance-overridden call succeeds').to.equal('0x');
        ...
        it('code override replaces a contract bytecode for the duration of the call', async () => {
            ...
            const paxBody = await rawPax<string>('eth_call', [
                {
                    to: randomAddr,
            ...
            expect(paxBody.result, 'code override: echo stub returns calldata').to.equal(
        ...
        it('nonce override is reflected in eth_call simulation', async () => {
            ...
            const paxBody = await rawPax('eth_call', [
                { to: runtime.contracts.erc20, data: erc20.balanceOf(paxAdmin) },
                'latest',
                { [paxAdmin]: { nonce: '0xff' } },
            ]);
            // The balance read is unaffected by the nonce override.
            expect(paxBody.result).to.match(/^0x/);
    ...
    describe('value forwarding & deploy simulation', () => {
        it('eth_call with a value field does not transfer ETH (simulation only)', async () => {
            const recipient = ethers.Wallet.createRandom().address;
            const balanceBefore = await pax.getBalance(recipient, 'latest');

            // This should succeed (or produce a deterministic result) without touching balances.
            await rawPax('eth_call', [
                { from: paxAdmin, to: recipient, value: ethers.toQuantity(ethers.parseEther('1')) },
                'latest',
            ]);

            const balanceAfter = await pax.getBalance(recipient, 'latest');
        ...
        it('eth_call without `to` (deployment simulation) returns runtime bytecode or 0x', async () => {
            ...
            const body = await rawPax<string>('eth_call', [{ data: initCode }, 'latest']);
    ...
    describe('EIP-7702 delegated execution', () => {
        ...
        it('dispatches into the delegated implementation and returns 0x for a void executeBatch', async () => {
            ...
            const code = await pax.send('eth_getCode', [alice.address, 'latest']);
            expect(code.toLowerCase()).to.equal(delegationDesignator(simpleAccountAddress));

            const balanceBefore = erc20.decodeBalance(
                await pax.send('eth_call', [{ to: erc20Pax, data: erc20.balanceOf(alice.address) }, 'latest']),
            );
            const mintCall = {
                target: erc20Pax,
                value: 0n,
        ...
        it('propagates an inner revert as code 3 execution reverted', async () => {
            ...
            const transferCall = {
                target: erc20Pax,
                value: 0n,
                data: erc20.transfer(paxAdmin, ethers.parseEther('1000000000')),
            };
