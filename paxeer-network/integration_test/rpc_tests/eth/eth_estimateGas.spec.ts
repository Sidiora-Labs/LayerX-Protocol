...
describe('eth_estimateGas Tests', function () {
    ...
    const claimPool = (count: number, salt: string): EvmAccount[] =>
        claimFromPool(runtime, pax, count, salt);
    ...
    describe('eth_estimateGas Queries', () => {
        it('a bare native transfer costs exactly the intrinsic 21000', async () => {
            expect(await estimate(pax, { from: paxAdmin, to: BOB, value: '0x1' })).to.equal(INTRINSIC);
        });

        it('a zero-value transfer still costs the intrinsic 21000', async () => {
            expect(await estimate(pax, { from: paxAdmin, to: BOB, value: '0x0' })).to.equal(INTRINSIC);
        });

        it('a self-transfer costs the intrinsic 21000', async () => {
            expect(await estimate(pax, { from: paxAdmin, to: paxAdmin, value: '0x1' })).to.equal(
                INTRINSIC,
            );
        });

        it('calldata on a plain transfer raises the estimate above the intrinsic', async () => {
            const withData = await estimate(pax, { from: paxAdmin, to: BOB, data: '0x1234567890' });
        ...
        it('an ERC20 approve and mint both estimate above the intrinsic', async () => {
            const [approveEst, mintEst] = await Promise.all([
                ...
                estimate(pax, {
                    from: paxAdmin,
                    to: erc20Pax,
                    data: erc20Iface.encodeFunctionData('mint', [paxAdmin, ethers.parseEther('100')]),
                }),
            ]);
        ...
        it('a contract deployment estimates into the hundreds of thousands of gas', async () => {
            const deployData =
                ...
                ethers.AbiCoder.defaultAbiCoder().encode(['address'], [paxAdmin]).slice(2);
            const est = await estimate(pax, { data: deployData });
            expect(est > 500_000n).to.equal(true);
        });

        it('estimate gas calls accepts an explicit latest block tag', async () => {
            const est = await estimate(pax, { from: paxAdmin, to: BOB, value: '0x1' }, 'latest');
        ...
        it('estimating against pending agrees with latest for a stable call', async () => {
            const [atLatest, atPending] = await Promise.all([
                estimate(pax, { from: paxAdmin, to: erc20Pax, data: transferData(BOB, 1n) }, 'latest'),
                estimate(pax, { from: paxAdmin, to: erc20Pax, data: transferData(BOB, 1n) }, 'pending'),
            ]);
    ...
    describe('transaction types', () => {
        const base = () => ({ from: paxAdmin, to: BOB, value: '0x1' });
        ...
        it('legacy (type 0) and EIP-1559 (type 2) estimate the same units as a bare transfer', async () => {
            const [legacy, eip1559] = await Promise.all([
                estimate(pax, { ...base(), type: '0x0' }),
                estimate(pax, { ...base(), type: '0x2' }),
            ]);
        ...
        it('an access-list (type 1) tx adds the EIP-2930 surcharge', async () => {
            const accessList = [
                { address: erc20Pax, storageKeys: ['0x' + '00'.repeat(32)] },
            ];
            const withAccessList = await estimate(pax, { ...base(), type: '0x1', accessList });
    ...
    describe('estimate accuracy', () => {
        it('the ERC20 transfer estimate bounds and closely tracks real gas used', async () => {
            const erc20 = new ethers.Contract(erc20Pax, erc20Iface, actor.wallet);
            ...
            const est = await estimate(pax, { from: actor.address, to: erc20Pax, data });

            const tx = await actor.wallet.sendTransaction({ to: erc20Pax, data, gasLimit: est });
        ...
        it('a native transfer estimate equals its exact gas used', async () => {
            const est = await estimate(pax, { from: actor.address, to: BOB, value: '0x1' });
        ...
        it('repeated estimates of the same call are deterministic', async () => {
            const tx = { from: paxAdmin, to: erc20Pax, data: transferData(BOB, 1n) };
            const results = await Promise.all(Array.from({ length: 5 }, () => estimate(pax, tx)));
    ...
    describe('precompiles', () => {
        ...
        it('the precompile estimate is deterministic and bounds real execution', async () => {
            ...
            const again = await estimate(pax, {
                from: actor.address,
                to: STAKING_PRECOMPILE_ADDRESS,
                data,
            });
    ...
    describe('schema matching vs geth', () => {
        it('a native transfer estimates to 21000 on both Pax and geth', async () => {
            const [s, g] = await Promise.all([
                estimate(pax, { from: paxAdmin, to: BOB, value: '0x1' }),
        ...
        it('an ERC20 transfer estimates identically on Pax and geth', async () => {
            ...
            const [s, g] = await Promise.all([
                estimate(pax, { from: paxAdmin, to: erc20Pax, data }),
        ...
        it('access-list, EIP-1559 and set-code estimates all match geth', async () => {
            ...
            for (const v of variants) {
                ...
                const [s, g] = await Promise.all([
                    estimate(pax, { from: paxAdmin, to: vTo ?? paxAdmin, ...rest }),
        ...
        it('an identical contract deployment estimates the same on Pax and geth', async () => {
            const deployData =
                bytecodeOf('TestERC20.sol', 'TestERC20') +
                ethers.AbiCoder.defaultAbiCoder().encode(['address'], [paxAdmin]).slice(2);
            const [s, g] = await Promise.all([
                estimate(pax, { data: deployData }),
    ...
    describe('empty / null handling', () => {
        it('returns a canonical quantity, never null (raw transport)', async () => {
            const body = await rawPax<string>('eth_estimateGas', [
                { from: paxAdmin, to: BOB, value: '0x1' },
            ]);
        ...
        it('a contract call returns a quantity, never null (raw transport)', async () => {
            const body = await rawPax<string>('eth_estimateGas', [
                { from: paxAdmin, to: erc20Pax, data: transferData(BOB, 1n) },
            ]);
            expect(body.error, JSON.stringify(body.error)).to.equal(undefined);
            expect(body.result).to.match(HEX_QUANTITY);
        });
    ...
    describe('wrong params / error handling', () => {
        it('a revert surfaces with identical code 3, message and decodable revert data on both', async () => {
            ...
            const [s, g] = await Promise.all([
                rawPax('eth_estimateGas', [{ from: sender, to: erc20Pax, data }]),
        ...
        it('a bad selector reverts with code 3 and empty data on both', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_estimateGas', [{ from: paxAdmin, to: erc20Pax, data: '0x12345678' }]),
        ...
        it('a gas cap below the requirement fails identically to geth (-32000 allowance)', async () => {
            const data = transferData(paxAdmin, ethers.parseEther('1'));
            const [s, g] = await Promise.all([
                rawPax('eth_estimateGas', [{ from: paxAdmin, to: erc20Pax, data, gas: '0x5208' }]),
        ...
        it('a malformed from address fails identically to geth (-32602, exact message)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_estimateGas', [{ from: '0xdead', to: BOB, value: '0x1' }]),
        ...
        it('[divergence] insufficient funds: both -32000 with the same clause, different gas prefix', async () => {
            ...
            const [s, g] = await Promise.all([
                rawPax('eth_estimateGas', [{ from: BOB, to: paxAdmin, value }]),
        ...
        it('[divergence] far-future block: both -32000 but different messages', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_estimateGas', [{ from: paxAdmin, to: BOB, value: '0x1' }, '0xffffffff']),
    ...
    describe('base fee increase doesnt change the gas estimates', () => {
        const getBaseFee = async (): Promise<bigint> => {
            const blk = await pax.send('eth_getBlockByNumber', ['latest', false]);
        ...
        it('gas estimates stay correct (and bound real usage) as the base fee rises', async function () {
            ...
            const estimateBefore = await estimate(pax, { from: actor.address, to: gasBurner, data: burnData });

            const { beforeBaseFee: before } = await burnGasBurst(pax, runtime, spammers);
            ...
            const estimateAfter = await estimate(pax, { from: actor.address, to: gasBurner, data: burnData });
            expect(estimateAfter, 'gas units are independent of the base fee').to.equal(estimateBefore);

            const fee = await pax.getFeeData();
            const tx = await actor.wallet.sendTransaction({
                to: gasBurner,
                data: burnData,
