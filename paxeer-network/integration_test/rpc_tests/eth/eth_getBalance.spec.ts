...
describe('eth_getBalance', function () {
    ...
    before(async () => {
        ...
        [spender] = claimPool(runtime, pax, 1, 'eth_getBalance');
    ...
    describe('happy path / schema', () => {
        it('returns the funded admin balance as a positive canonical quantity at latest', async () => {
            const res = await rawPax<string>('eth_getBalance', [paxAdmin, 'latest']);
        ...
        it('reports 0x0 for a fresh, unassociated account — identically on Pax and geth', async () => {
            const [s, g] = await Promise.all([
                rawPax<string>('eth_getBalance', [unassociated, 'latest']),
                rawGeth<string>('eth_getBalance', [unassociated, 'latest']),
            ]);
            expect(s.result, 'Pax: empty account is zero').to.equal('0x0');
        ...
        it('returns a canonical quantity for every supported block tag, equal across the recent tags', async () => {
            ...
            const results = await Promise.all(
                recent.map(t => rawPax<string>('eth_getBalance', [paxAdmin, t])),
            );
            ...
            const earliest = await rawPax<string>('eth_getBalance', [paxAdmin, 'earliest']);
            if (earliest.error) {
        ...
        it('returns the (zero) native balance of a contract address', async () => {
            ...
            const balance = await pax.getBalance(erc20Pax, 'latest');
    ...
    describe('balance tracks transfers across historical state', () => {
        it('debits the sender by value + gas while the pre-send block keeps the old balance', async () => {
            ...
            const blockBefore = await pax.getBlockNumber();
            const balanceBefore = await pax.getBalance(spender.address, blockBefore);
            ...
            const balanceAfter = await pax.getBalance(spender.address, 'latest');
            ...
            const balanceAtOldBlock = await pax.getBalance(spender.address, blockBefore);
        ...
        it('credits a fresh recipient and the credit is invisible before the funding block', async () => {
            ...
            expect(await pax.getBalance(recipient, 'latest'), 'recipient starts empty').to.equal(0n);
            ...
            expect(await pax.getBalance(recipient, fundingBlock), 'credited exactly value').to.equal(
                value,
            );
            expect(
                await pax.getBalance(recipient, fundingBlock - 1),
    ...
    describe('block specifiers (EIP-1898)', () => {
        ...
        before(async () => {
            ...
            const block = await pax.getBlock(fundingBlock);
        ...
        it('a blockNumber object matches the numeric tag and the known funded balance', async () => {
            const [viaTag, viaObject] = await Promise.all([
                rawPax<string>('eth_getBalance', [knownWallet, ethers.toQuantity(fundingBlock)]),
                rawPax<string>('eth_getBalance', [
                    knownWallet,
        ...
        it('a blockHash object matches the numeric tag and the known funded balance', async () => {
            const [viaNumber, viaHash] = await Promise.all([
                rawPax<string>('eth_getBalance', [knownWallet, ethers.toQuantity(fundingBlock)]),
                rawPax<string>('eth_getBalance', [knownWallet, { blockHash: fundingBlockHash }]),
            ]);
    ...
    describe('wrong params / error handling (parity with geth)', () => {
        ...
        it('omitting the block argument fails identically (-32602, missing required argument 1)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getBalance', [paxAdmin]),
        ...
        it('too many positional args fail identically (-32602, want at most 2)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getBalance', [paxAdmin, 'latest', {}]),
        ...
        it('non-array params fail identically (-32602, non-array args)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getBalance', { address: paxAdmin }),
        ...
        it('a malformed (too short) address fails identically (-32602, exact length message)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getBalance', ['0x1234', 'latest']),
