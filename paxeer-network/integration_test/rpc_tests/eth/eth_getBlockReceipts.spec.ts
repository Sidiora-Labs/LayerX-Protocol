...
import { cosmosBankSend, generatePaxAddress, bankBalanceUhpx, CosmosBankSend } from '../utils/cosmosUtils';
import {
    sharedRichBlock,
    sendSingleTx,
    sendRevertingTx,
...
// eth_getBlockReceipts: one Pax block with every tx type; assert receipt fields,
...
describe('eth_getBlockReceipts', function () {
    ...
    before(async function () {
        ...
        const extra = claimPool(runtime, pax, 2, 'eth_getBlockReceipts');
        ...
        paxOne = await sendSingleTx(pax, extra[0]);
        gethOne = await sendSingleTx(geth, gethSigner);
        paxFailed = await sendRevertingTx(pax, extra[1], runtime.contracts.erc20);
        ...
        // compare a creation receipt against Pax's (which omits the `to` field).
        ...
        const blk = await pax.send('eth_getBlockByNumber', [ethers.toQuantity(richPax.number), false]);
        baseFee = BigInt(blk.baseFeePerGas);
    });

    describe('receipt array shape (populated Pax block)', () => {
        it('returns one canonical receipt per transaction, in transaction-index order', async () => {
            const receipts = await blockReceipts(pax, richPax.number);
            expect(receipts.length, 'one receipt per sent tx').to.equal(richPax.txs.length);
            receipts.forEach((rc, i) => assertCanonicalReceipt(rc, richPax.hash, richPax.number, i));
        ...
        it('the by-number and by-hash lookups return byte-identical receipt arrays', async () => {
            const [viaNumber, viaHash] = await Promise.all([
                blockReceipts(pax, richPax.number),
    ...
    describe('cross-reference: receipts ⇄ block transactions (byNumber & byHash)', () => {
        it('byNumber, byHash and the receipts all describe the same ordered transaction set', async () => {
            const [bn, bh, receipts] = await Promise.all([
                pax.send('eth_getBlockByNumber', [ethers.toQuantity(richPax.number), true]),
                pax.send('eth_getBlockByHash', [richPax.hash, true]),
                blockReceipts(pax, richPax.number),
            ]);
        ...
        it('every receipt is byte-identical to its standalone eth_getTransactionReceipt', async () => {
            const receipts = await blockReceipts(pax, richPax.number);
            for (const rc of receipts) {
                const single = await pax.send('eth_getTransactionReceipt', [rc.transactionHash]);
    ...
    describe('cross-reference: receipts ⇄ eth_getTransactionBy* lookups', () => {
        it('byHash, byBlockHashAndIndex and byBlockNumberAndIndex return byte-identical objects', async () => {
            const block = await pax.send('eth_getBlockByNumber', [
                ethers.toQuantity(richPax.number),
                true,
            ]);
            for (const txInBlock of block.transactions) {
                ...
                const [byHash_, byBH, byBN] = await Promise.all([
                    pax.send('eth_getTransactionByHash', [txInBlock.hash]),
                    pax.send('eth_getTransactionByBlockHashAndIndex', [richPax.hash, i]),
                    pax.send('eth_getTransactionByBlockNumberAndIndex', [
                        ethers.toQuantity(richPax.number),
                        i,
                    ]),
                ]);
        ...
        it('the tx object and its receipt converge on every shared identity field', async () => {
            const receipts = await blockReceipts(pax, richPax.number);
            for (const rc of receipts) {
                const tx = await pax.send('eth_getTransactionByHash', [rc.transactionHash]);
        ...
        it('tx-only and receipt-only fields are disjoint — overlap is exactly the identity set', async () => {
            ...
            for (const rc of receipts) {
                const tx = await pax.send('eth_getTransactionByHash', [rc.transactionHash]);
                // Shared keys are identity/position fields, minus `to` for creations (Pax drops it).
        ...
        it('[divergence] geth stamps blockTimestamp on the tx object; Pax does not', async () => {
            const [s, g] = await Promise.all([
                pax.send('eth_getTransactionByHash', [paxOne.tx.hash]),
            ...
            expect(sKeys.filter(k => !gKeys.includes(k)), 'Pax-only tx fields').to.deep.equal([]);
            // It is purely informational: it equals the block's own timestamp, so it adds no
            // state — just saves a second round-trip. Pax omitting it is consistent with the
    ...
    describe('eth_getTransactionBy* — null & error semantics (parity with geth)', () => {
        it('byHash: empty params fail identically (-32602, missing argument 0)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getTransactionByHash', []),
        ...
        it('byHash: a wrong-length hash fails identically (-32602, common.Hash)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getTransactionByHash', ['0x1234']),
        ...
        it('byHash: an unknown hash returns null on both chains', async () => {
            ...
            const [s, g] = await Promise.all([
                pax.send('eth_getTransactionByHash', [unknown]),
        ...
        it('byBlockHashAndIndex: a missing index fails identically (-32602, argument 1)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getTransactionByBlockHashAndIndex', [richPax.hash]),
        ...
        it('byBlockHashAndIndex: an out-of-range index returns null on both chains', async () => {
            const [s, g] = await Promise.all([
                pax.send('eth_getTransactionByBlockHashAndIndex', [richPax.hash, '0xffff']),
        ...
        it('byBlockHashAndIndex on an unknown block returns null on both chains', async () => {
            const unknown = '0x' + 'ab'.repeat(32);
            const [s, g] = await Promise.all([
                rawPax('eth_getTransactionByBlockHashAndIndex', [unknown, '0x0']),
        ...
        it('byBlockNumberAndIndex on a future block returns null on both chains', async () => {
            ...
            const [s, g] = await Promise.all([
                rawPax('eth_getTransactionByBlockNumberAndIndex', [future, '0x0']),
            ...
            expect(g.error, 'geth does not error').to.equal(undefined);
            expect(s.result, 'Pax returns null for a future block').to.equal(null);
            expect(s.error, 'Pax does not error').to.equal(undefined);
        });
    });

    describe('eth_getRawTransaction* — raw signed tx (geth) & Pax divergence', () => {
        it('[divergence] Pax does not implement the raw-transaction endpoints (-32601)', async () => {
            const envelopes = await Promise.all([
                rawPax(RAW_TX_BY_HASH, [paxOne.tx.hash]),
                rawPax(RAW_TX_BY_BLOCK_HASH_AND_INDEX, [paxOne.hash, '0x0']),
                rawPax(RAW_TX_BY_BLOCK_NUMBER_AND_INDEX, [ethers.toQuantity(paxOne.number), '0x0']),
            ]);
            // geth serves these; Pax has not registered them, so every one is method-not-found.
        ...
        it('an unknown hash: geth returns empty "0x", Pax still answers -32601', async () => {
            const unknown = '0x' + 'ab'.repeat(32);
            const [g, s] = await Promise.all([
                geth.send(RAW_TX_BY_HASH, [unknown]),
                rawPax(RAW_TX_BY_HASH, [unknown]),
            ]);
    ...
    describe('gas, fees, tip and balances reconcile (multiple users)', () => {
        it('block.gasUsed equals Σ receipt gasUsed and cumulativeGasUsed is consistent', async () => {
            const [block, receipts] = await Promise.all([
                pax.send('eth_getBlockByNumber', [ethers.toQuantity(richPax.number), false]),
                blockReceipts(pax, richPax.number),
            ]);
            ...
            assertCumulativeGasSeries(ordered, BigInt(block.gasUsed), richPax.cosmosShellGas);
        });
    ...
    describe('logs & bloom', () => {
        it('the erc20 receipt emitted a Transfer log and its bloom matches its own logs', async () => {
            ...
            const rc = receipts.find(
                r => r.transactionHash === richPax.txs.find(t => t.kind === 'erc20')!.hash,
            )!;
    ...
    describe('contract deployment & precompile receipts', () => {
        it('the deployment receipt carries the created contractAddress with live code', async () => {
            ...
            const code = await pax.getCode(rc.contractAddress!, richPax.number);
    ...
    describe('failed transactions are still included', () => {
        it('[Pax] a reverted tx appears with status 0x0 and is counted in the block', async () => {
            expect(paxFailed.receipt.status, 'tx reverted').to.equal(0);
            const receipts = await blockReceipts(pax, paxFailed.receipt.blockNumber);
            const rc = receipts.find(r => r.transactionHash === paxFailed.hash);
            ...
            const single = await pax.send('eth_getTransactionReceipt', [paxFailed.hash]);
    ...
    describe('geth parity (single transaction)', () => {
        it('both chains expose the identical receipt field set', async () => {
            const [s, g] = await Promise.all([blockReceipts(pax, paxOne.number), blockReceipts(geth, gethOne.number)]);
            expect(s.length, 'Pax single-tx block').to.equal(1);
            expect(g.length, 'geth single-tx block').to.equal(1);
            assertCanonicalReceipt(s[0], paxOne.hash, paxOne.number, 0);
        ...
        it('creation receipts identify the creation via contractAddress (to, if present, is null)', async () => {
            ...
            // is the canonical creation signal. geth emits `to: null`, Pax omits the key — both are
            // spec-compliant, so assert the creation address and only check `to` where present.
            const paxReceipts = await blockReceipts(pax, richPax.number);
            const paxDeploy = paxReceipts.find(
                r => r.transactionHash === richPax.txs.find(t => t.kind === 'deploy')!.hash,
            )!;
            ...
            expect(paxDeploy.contractAddress, 'Pax creation contractAddress').to.match(ADDRESS);
    ...
    describe('dual-VM: a Cosmos bank send sharing the block', () => {
        // Pax executes native Cosmos txs and EVM txs in the same blocks, but the EVM
        ...
        let recipientPax: string;
        const AMOUNT_UHPX = 123_456n;

        before(async function () {
            this.timeout(180 * 1000);
            const evmSigner = claimPool(runtime, pax, 1, 'eth_getBlockReceipts:cosmos')[0];
            for (let attempt = 0; attempt < 4 && height === undefined; attempt++) {
                const recipient = await generatePaxAddress();
                const [cos, ev] = await Promise.all([
                    cosmosBankSend(runtime.funded.adminMnemonic, recipient, AMOUNT_UHPX),
                    sendSingleTx(pax, evmSigner),
                ]);
                if (cos.code === 0 && cos.height === ev.number) {
                    height = cos.height;
                    cosmos = cos;
                    evm = ev;
                    recipientPax = recipient;
                }
            }
            if (height === undefined) this.skip();
        });

        it('the Cosmos bank send and the EVM tx really share one Pax block', () => {
        ...
        it('eth_getBlockReceipts includes the EVM tx but never the Cosmos tx', async () => {
            const receipts = await blockReceipts(pax, height!);
        ...
        it('receipt count equals the EVM block tx count (Cosmos tx is not counted)', async () => {
            const [block, receipts] = await Promise.all([
                pax.send('eth_getBlockByNumber', [ethers.toQuantity(height!), false]),
                blockReceipts(pax, height!),
            ]);
            const cosmosAsEvmHash = '0x' + cosmos.hash.toLowerCase();
        ...
        it('the Cosmos tx hash is unknown to the EVM tx/receipt endpoints', async () => {
            ...
            const [tx, rc] = await Promise.all([
                pax.send('eth_getTransactionByHash', [cosmosAsEvmHash]),
                pax.send('eth_getTransactionReceipt', [cosmosAsEvmHash]),
            ]);
            expect(tx, 'no EVM tx for the Cosmos hash').to.equal(null);
            expect(rc, 'no EVM receipt for the Cosmos hash').to.equal(null);
        });

        it('the bank send actually moved uhpx in that block (state changed off the EVM)', async () => {
            const [before, after] = await Promise.all([
                bankBalanceUhpx(recipientPax, height! - 1),
                bankBalanceUhpx(recipientPax, height!),
            ]);
            expect(before, 'recipient started empty').to.equal(0n);
            expect(after, 'recipient credited the exact uhpx amount').to.equal(AMOUNT_UHPX);
        });
    });

    describe('lookup semantics', () => {
        it('the latest tag returns canonical, correctly indexed receipts', async () => {
            const receipts: any[] = await pax.send('eth_getBlockReceipts', ['latest']);
        ...
        it('the earliest (genesis) block returns an empty receipt array', async () => {
            const receipts = await pax.send('eth_getBlockReceipts', ['earliest']);
        ...
        it('an unknown block hash returns null on both chains', async () => {
            const unknown = '0x' + 'ab'.repeat(32);
            const [s, g] = await Promise.all([
                pax.send('eth_getBlockReceipts', [unknown]),
                geth.send('eth_getBlockReceipts', [unknown]),
            ]);
            expect(s, 'Pax unknown hash is null').to.equal(null);
    ...
    describe('wrong params / error handling (parity with geth)', () => {
        ...
        it('too many positional args fail identically (-32602, want at most 1)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getBlockReceipts', [richPax.hash, {}]),
        ...
        it('non-array params fail identically (-32602, non-array args)', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_getBlockReceipts', { block: 'latest' } as any),
        ...
        it('a far-future block returns null on both chains', async () => {
            const future = ethers.toQuantity((await pax.getBlockNumber()) + 10_000_000);
            const [s, g] = await Promise.all([
                rawPax('eth_getBlockReceipts', [future]),
            ...
            expect(g.result, 'geth returns null for a future block').to.equal(null);
            expect(s.error, 'Pax does not error on a future block').to.equal(undefined);
            expect(s.result, 'Pax returns null for a future block').to.equal(null);
        });
    });
});
