...
describe('eth_getFilterChanges', function () {
    ...
    before(async () => {
        ...
        [emitter] = claimPool(runtime, pax, 1, 'eth_getFilterChanges');
    });
    ...
    describe('log filter (incremental delivery)', () => {
        it('delivers only the logs that arrived since the previous poll', async () => {
            ...
            const fromBlock = ethers.toQuantity(await pax.getBlockNumber());
            const id = await pax.send('eth_newFilter', [
                { address, topics: [TRANSFER_TOPIC], fromBlock },
            ]);

            expect(await pax.send('eth_getFilterChanges', [id]), 'no events yet').to.deep.equal([]);

            await (await token.mint(emitter.address, ethers.parseEther('100'))).wait();
            const afterMint = await drainFilterChanges(pax, id, 1);
            ...
            const afterTransfer = await drainFilterChanges(pax, id, 1);
            expect(afterTransfer.length, 'only the new Transfer').to.equal(1);
            expect(await pax.send('eth_getFilterChanges', [id]), 'drained').to.deep.equal([]);

            const delivered = [...afterMint, ...afterTransfer];
            const viaGetLogs = await pax.send('eth_getLogs', [
        ...
        it('respects the address filter and returns logs only for that address', async () => {
            ...
            const got = await assertFilterChangesMatchGetLogs(pax, criteria, 'address filter');
    ...
    describe('topic & address filtering, parity with eth_getLogs', () => {
        ...
        for (const c of LOG_FILTER_MATRIX) {
            it(c.title, async () => {
                ...
                const got = await assertFilterChangesMatchGetLogs(pax, criteria, c.title);
    ...
    describe('block filter', () => {
        it('delivers the hashes of blocks mined since the previous poll', async () => {
            ...
            // filter cursor can trail the committed block on slow CI. Drain across polls — Pax mines
