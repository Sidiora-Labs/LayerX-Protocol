...
import { bothProviders, rawPax, rawGeth, expectJsonRpcError, blockGasInfo } from '../utils/chainUtils';
...
describe('eth_maxPriorityFeePerGas', function () {
    ...
    before(() => {
        ...
        spammers = claimPool(runtime, pax, 8, 'eth_maxPriorityFeePerGas');
    });

    describe('queries', () => {
        it('returns a canonical, non-negative hex quantity', async () => {
            const raw = await pax.send('eth_maxPriorityFeePerGas', []);
        ...
        it('stays within a sane bound (never more than the total gas price)', async () => {
            const [tip, price] = await Promise.all([maxPriorityFeePerGas(pax), gasPrice(pax)]);
        ...
        it('falls back to the 1-gwei default while the latest block is uncongested', async () => {
            const head = await blockGasInfo(pax, 'latest');
            ...
            const tip = await maxPriorityFeePerGas(pax);
        ...
        it('returns either the 1-gwei default or the latest-block 50th-pct tip (congestion rule)', async () => {
            const [tip, fh] = await Promise.all([
                maxPriorityFeePerGas(pax),
                feeHistory(pax, 1, 'latest', [50]),
            ]);
        ...
        it('switches off the 1-gwei default and reports the burst tip once a block is congested', async function () {
            let burstDone = false;
            const burst = burnGasBurst(pax, runtime, spammers, {
                rounds: 12,
                gasLimit: 4_600_000n,
                tip: BURST_TIP,
            }).finally(() => {
            ...
            while (Date.now() < deadline && !sample) {
                const b1 = await pax.getBlockNumber();
                const tip = await maxPriorityFeePerGas(pax);
                const info = await blockGasInfo(pax, 'latest');
                const b2 = await pax.getBlockNumber();
    ...
    describe('schema matching (parity with geth)', () => {
        it('both nodes return a canonical hex quantity', async () => {
            const [s, g] = await Promise.all([
                pax.send('eth_maxPriorityFeePerGas', []),
                geth.send('eth_maxPriorityFeePerGas', []),
            ]);
            expect(s, 'pax').to.match(HEX_QUANTITY);
    ...
    describe('wrong params / error handling (parity with geth)', () => {
        it('rejects extra positional parameters with -32602, identically to geth', async () => {
            const [s, g] = await Promise.all([
                rawPax('eth_maxPriorityFeePerGas', ['latest']),
