...
describe('eth_sendRawTransaction', function () {
    ...
    before(async () => {
        ...
        [sender, nonceTester, intrinsicTester] = claimPool(runtime, pax, 3, 'eth_sendRawTransaction');
    ...
    describe('happy path: accepts the canonical encoding of every tx type', () => {
        for (const type of TX_TYPES) {
            it(`accepts a ${TYPE_NAME[type]} (type ${type}) transfer and returns its hash`, async () => {
                const signed = await signRawTransfer(pax, sender, type);
                const returned = await sendRaw(pax, signed.raw);
                expect(returned, 'returned hash == keccak256(raw)').to.equal(signed.hash);

                const receipt = await pax.waitForTransaction(signed.hash, 1, 60_000);
                expect(receipt!.status, 'mined successfully').to.equal(1);

                const txObject = await pax.send('eth_getTransactionByHash', [signed.hash]);
    ...
    describe('wrong params / error handling', () => {
        it('[divergence] both reject a below-intrinsic-gas tx; geth is descriptive, Pax is generic', async () => {
            const [paxTx, gethTx] = await Promise.all([
                signBelowIntrinsicTx(pax, intrinsicTester),
            ...
            // geth rejects pre-execution with the exact reason; Pax rejects too (same -32000 code)
            ...
            expect(s.error, 'Pax rejects the below-intrinsic tx').to.not.equal(undefined);
        ...
        it('rejects a tx whose nonce is already used (stale nonce)', async () => {
            // Consume nonce 0, then re-submit a freshly signed tx pinned to nonce 0. Pax's Cosmos
            // ante reports "incorrect account sequence" where geth would say "nonce too low".
            const first = await signRawTransfer(pax, nonceTester, 2, { nonce: 0 });
            await sendRaw(pax, first.raw);
            await pax.waitForTransaction(first.hash, 1, 60_000);

            const stale = await signRawTransfer(pax, nonceTester, 2, { nonce: 0 });
            const res = await rawPax('eth_sendRawTransaction', [stale.raw]);
        ...
        it('a missing transaction argument is rejected with -32602', async () => {
            const res = await rawPax('eth_sendRawTransaction', []);
        ...
        it('[replay-protection] the same signed tx rejected on second submission', async () => {
            ...
            const signed = await signRawTransfer(pax, sender, 2);
            await sendRaw(pax, signed.raw);
            await pax.waitForTransaction(signed.hash, 1, 60_000);

            const replayRes = await rawPax('eth_sendRawTransaction', [signed.raw]);
            expect(replayRes.error, 'replay must be rejected').to.not.equal(undefined);
            // Pax dedups in the mempool cache ("tx already exists in cache") before the nonce
        ...
        it('[chain-id-mismatch] a tx signed for chain ID 1 (mainnet) is rejected', async () => {
            // A transfer signed for chain ID 1 (Ethereum mainnet): EIP-155 replay protection must make Pax reject it.
            const feeData = await pax.getFeeData();
            const network = await pax.getNetwork();
            const wallet = ethers.Wallet.createRandom().connect(pax);
            ...
            const res = await rawPax('eth_sendRawTransaction', [tx]);
