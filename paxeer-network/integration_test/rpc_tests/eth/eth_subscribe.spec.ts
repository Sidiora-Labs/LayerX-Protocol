...
import { paxRpc, waitUntil } from '../utils/chainUtils';
...
describe('eth_subscribe (WebSocket)', function () {
    ...
    before(async function () {
        ws = await WsClient.open(Endpoints.pax.evmWs);
        const runtime = readRuntimeState();
        [emitter] = claimPool(runtime, pax, 1, 'eth_subscribe');
    });
    ...
    describe('newHeads', () => {
        ...
        it('pushes a fully-formed header that matches the canonical block', async () => {
            ...
            for (const head of heads) {
                ...
                const block = await pax.send('eth_getBlockByNumber', [head.number, false]);
                expect(block, `canonical block exists for ${head.number}`).to.not.equal(null);
                assertNewHeadMatchesBlock(head, block);
                await assertNewHeadBaseFeeTracksChain(head, pax);
            }
        ...
        it('delivers strictly increasing block numbers tracking the chain head', async () => {
            ...
            const observed = BigInt(await pax.send('eth_blockNumber', []));
    ...
    describe('logs', () => {
        ...
        it('a blockHash filter replays exactly that block\'s matching logs', async () => {
            ...
            await waitUntil(
                async () => {
                    const seen = await pax.send('eth_getLogs', [{ blockHash, address }]);
