#!/usr/bin/env bash
#
# CI orchestrator for the Pax EVM JSON-RPC parity suite.
#
# Env knobs:
#   PAX_EVM_RPC     Pax EVM RPC URL                         (default http://localhost:8545)
#   RPC_ETH_GETH    geth reference URL                      (default http://127.0.0.1:9547)
#   PAX_TIMEOUT     seconds to wait for Pax RPC/blocks      (default 300)
...
GETH_PORT=9547
PAX_EVM_RPC_URL="${PAX_EVM_RPC:-http://localhost:8545}"
GETH_RPC_URL="${RPC_ETH_GETH:-http://127.0.0.1:${GETH_PORT}}"
PAX_TIMEOUT="${PAX_TIMEOUT:-300}"
...
wait_for_rpc "$PAX_EVM_RPC_URL" "Pax EVM RPC" "$PAX_TIMEOUT" \
    || die "Pax EVM RPC at $PAX_EVM_RPC_URL never came up (is the cluster started?)"
wait_for_block_production "$PAX_EVM_RPC_URL" "Pax chain" "$PAX_TIMEOUT" \
    || die "Pax chain at $PAX_EVM_RPC_URL is up but not producing blocks within ${PAX_TIMEOUT}s"
...
# The suite runs in a single process: every spec shares the one Pax chain, so a
