...
/** Pax keys use cosmos coin type 118; the matching EVM key derives from the same path. */
export const PAX_HD_PATH = "m/44'/118'/0'/0/0";

/** 10^12 wei == 1 uhpx. Pax rounds native balances to whole uhpx. */
export const WEI_PER_UHPX = 10n ** 12n;
/** Alias kept for the balance/fee-reconciliation specs that read this as `UHPX`. */
export const UHPX = WEI_PER_UHPX;

/** Pax's staking precompile address (used by precompile-call fixtures). */
...
/** Default Pax EVM chain id on the local devnet. */
export const DEFAULT_EVM_CHAIN_ID = 713714;

export const DOCKER_NODE = 'pax-node-0';
export const PAXD_ENV = 'export PATH=$PATH:/root/go/bin:/root/.foundry/bin';

/** Password for the in-container `admin`/test keyring (docker devnet only). */
export const DOCKER_KEY_PASSWORD = '12345678';

/** In-container EVM RPC the `paxd` CLI targets when registering CW20 pointers. */
...
/** Flat fee for the one-off CW20 store + instantiate (10M gas @ 3.5uhpx). */
export const WASM_FEE = calculateFee(10_000_000, GasPrice.fromString('3.5uhpx'));

/** Flat fee for a single CW20 execute, e.g. transfer (1.5M gas @ 3.5uhpx). */
export const EXEC_FEE = calculateFee(1_500_000, GasPrice.fromString('3.5uhpx'));
