...
import { paxRpc } from './chainUtils';
import { PAX_HD_PATH } from './constants';
...
export class EvmAccount {
    ...
    static fromMnemonic(mnemonic: string, provider = paxRpc()): EvmAccount {
        const wallet = ethers.HDNodeWallet.fromPhrase(mnemonic, '', PAX_HD_PATH).connect(provider);
        return new EvmAccount(wallet);
    }

    static fromPrivateKey(privateKey: string, provider = paxRpc()): EvmAccount {
        const wallet = new ethers.Wallet(privateKey, provider);
        return new EvmAccount(wallet);
    }

    static random(provider = paxRpc()): EvmAccount {
    ...
    paxAddress(): string {
        ...
        return toBech32('pax', rawSecp256k1PubkeyToRawAddress(compressed));
    }
}

/**
 * Send native pax (in wei) from `from` to `to` and wait for inclusion. Used by the
 ...
 * broadcast concurrently — Pax's mempool accepts gapless nonces from the same sender, so
...
/** The 0xef0100-prefixed delegation designator geth/Pax store as an EOA's code. */
