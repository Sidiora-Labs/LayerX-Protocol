...
import { DOCKER_NODE, PAXD_ENV, STAKING_PRECOMPILE_ADDRESS } from './constants';
...
/**
 * The canonical Ethereum log fields. Pax returns exactly these; a modern geth
...
/**
 * Pax's staking precompile emits real EVM logs (Delegate / Undelegate / …) under its
...
/** First bonded validator's `paxvaloper…` operator address, via the in-container CLI. */
export async function firstBondedValidator(): Promise<string> {
    const { stdout } = await exec(
        `docker exec ${DOCKER_NODE} /bin/bash -c '${PAXD_ENV} && paxd q staking validators -o json'`,
    );
 ...
 *
 * Each lands in its own block on Pax, giving callers exact block bounds and
 ...
 * elapses). On Pax's continuously-producing chain the log cursor can trail the tx receipt by a
