...
export function assertTxReceiptConsistency(tx: any, rc: any): void {
    ...
    for (const k of TX_RECEIPT_SHARED_KEYS) {
        // Pax currently drops `to` on creation receipts; compare only when present.
...
/**
 * Assert two tx objects expose the same field set, ignoring `blockTimestamp` — a known Pax
 * divergence (geth stamps it on tx objects; Pax does not). Both objects must be the SAME
 * transaction type, since the typed-tx EIPs gate which fee fields appear.
 */
export function assertTxKeysetParity(paxTx: any, gethTx: any): void {
    const keys = (o: any) =>
        Object.keys(o)
