...
import { generatePaxAddress } from './cosmosUtils';
import { cw20Transfer } from './wasmUtils';
import { HASH32, BLOOM256, NONCE8, HEX_QUANTITY, HEX_DATA, ADDRESS } from './format';
import { STAKING_PRECOMPILE_ADDRESS, UHPX, ZERO_HASH } from './constants';
export { STAKING_PRECOMPILE_ADDRESS, UHPX, ZERO_HASH };

/**
 * Shared fixtures + assertions for the eth_getBlockByNumber / eth_getBlockByHash parity specs.
 * Pax (a Cosmos chain) packs many transactions into one block, so we build a single "rich" block
...
export const PAX_ONLY_BLOCK_FIELDS = ['totalDifficulty'] as const;
...
export interface RichBlock {
    ...
    txs: SentTx[];
    /**
     * Gas used by the co-located pointer-backed CW20 transfer (a pure Cosmos tx). Pax folds
...
/**
 * The JSON-RPC transaction-receipt object. `to` is creation-dependent (Pax omits it on a
 ...
 */
export async function buildRichPaxBlock(
    provider: ethers.JsonRpcProvider,
    ...
    if (signers.length < 9) {
        throw new Error(`buildRichPaxBlock needs >= 9 signers, got ${signers.length}`);
    }
    ...
    const cosmosRecipient = wasm ? await generatePaxAddress() : undefined;
    ...
    throw new Error(`buildRichPaxBlock: could not pack one block after ${attempts} attempts: ${lastErr}`);
}
...
export async function sharedRichBlock(
    ...
    if (!cachedRichBlockPromise) {
        cachedRichBlockPromise = (async () => {
            ...
            cachedRichBlock = await buildRichPaxBlock(provider, runtime, signers);
 ...
 * nodes (same -32000 code): geth with a descriptive "intrinsic gas too low", Pax with a
 ...
 * CW20 transfer (a pure Cosmos tx): Pax folds that Cosmos-unit gas into the EVM receipts'
...
/**
 * Verify a Pax block's gas accounting against the EVM-visible (eth_*) receipts.
 *
...
export function assertCanonicalReceipt(
    ...
    // `to` is the one field that diverges: Pax omits it on a creation receipt while geth
    ...
    if (isCreation) {
        ...
        // `to` is either absent (Pax) or null (geth) — never an address.
...
// The raw-transaction endpoints return the RLP-encoded *signed* transaction. Pax does not
...
/**
 * Scan backwards from the chain head for a block with zero transactions. Pax mints empty
...
/** eth_getVMError wrapper: the stored VM error string for a tx (a Pax-specific method). */
