...
RESPONSE=$(printf "12345678\n" | paxd tx gov submit-proposal software-upgrade $VERSION \
    --title $VERSION --from node_admin --chain-id pax --fees 2000uhpx -b sync -y \
    --upgrade-height=$TARGET_HEIGHT --description "test $TYPE release" \
    $UPGRADE_INFO_FLAG --is-expedited --deposit 20000000uhpx --output json)
