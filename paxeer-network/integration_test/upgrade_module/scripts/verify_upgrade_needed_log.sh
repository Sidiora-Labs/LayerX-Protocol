...
if grep -q "UPGRADE \"$VERSION\" NEEDED at height: $TARGET_BLOCK_HEIGHT" "build/generated/logs/paxd-$NODE_ID.log"; then
   echo "PASS"
   exit 0
fi
...
if pgrep -f "paxd start --chain-id pax" > /dev/null; then
   BLOCK=$(timeout 5 paxd status 2>/dev/null | jq '.SyncInfo.latest_block_height' -r 2>/dev/null || echo "")
