...
while true; do
   ...
   if ! pgrep -f "paxd start --chain-id pax" > /dev/null; then
      echo "Paxd no longer running (panic)"
      break
   fi

   # Query the current block height of the node
   CURRENT_BLOCK_HEIGHT=$(timeout "$STATUS_TIMEOUT_SECONDS" paxd status 2>/dev/null | jq '.SyncInfo.latest_block_height' -r 2>/dev/null)
   if ! [[ "$CURRENT_BLOCK_HEIGHT" =~ ^[0-9]+$ ]]; then
       ((NO_PROGRESS_COUNTER++))
       if [[ "$NO_PROGRESS_COUNTER" -ge "$MAX_NO_PROGRESS_SECONDS" ]]; then
           echo "Paxd status unavailable for ${NO_PROGRESS_COUNTER}s while waiting for block $TARGET_BLOCK_HEIGHT"
           exit 1
       fi
       sleep 1
       continue
