	...
	testCases := []struct {
		...
		},
		{
			"account address generation is block dependent", func() {
				...
				err := suite.chainB.GetSimApp().BankKeeper.SendCoins(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), icaHostAccount, sdk.Coins{sdk.NewCoin("uhpx", sdk.NewInt(1))})
...
func (suite *InterchainAccountsTestSuite) TestOnRecvPacket() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			// send 100pax to interchain account wallet
			amount, _ := sdk.ParseCoinsNormalized("100uhpx")
