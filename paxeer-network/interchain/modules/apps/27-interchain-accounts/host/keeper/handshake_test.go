	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				err := suite.chainB.GetSimApp().BankKeeper.SendCoins(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), interchainAccAddr, sdk.Coins{sdk.NewCoin("uhpx", sdk.NewInt(1))})
				suite.Require().NoError(err)
