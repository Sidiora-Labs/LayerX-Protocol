...
use pax_cosmwasm::{
    Order, PaxMsg, PaxQueryWrapper, PositionDirection, OrderType,
};
...
pub fn instantiate(
    ...
    _info: MessageInfo,
    msg: InstantiateMsg,
) -> StdResult<Response<PaxMsg>> {
    MARS_ADDR.save(deps.storage, &msg.mars_address)?;
    Ok(Response::new())
}

#[entry_point]
pub fn sudo(deps: DepsMut<PaxQueryWrapper>, env: Env, msg: SudoMsg) -> Result<Response<PaxMsg>, StdError> {
    match msg {
...
pub fn process_settlements(
    _deps: DepsMut<PaxQueryWrapper>,
    _entries: Vec<SettlementEntry>,
    _epoch: i64,
) -> Result<Response<PaxMsg>, StdError> {
    Ok(Response::new())
}
...
pub fn process_bulk_order_placements(
    ...
    if env.block.height % 10 == 0 {
        ...
        let order = pax_cosmwasm::PaxMsg::PlaceOrders {
            funds: vec![],
