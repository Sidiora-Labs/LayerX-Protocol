...
//go:embed NativePaxTokensERC20.abi
//go:embed NativePaxTokensERC20.bin
var f embed.FS
...
func GetABI() []byte {
	bz, err := f.ReadFile("NativePaxTokensERC20.abi")
	if err != nil {
		panic("failed to read NativePaxTokensERC20 contract ABI")
	}
	return bz
}
...
func GetBin() []byte {
	...
	code, err := f.ReadFile("NativePaxTokensERC20.bin")
	if err != nil {
		panic("failed to read NativePaxTokensERC20 contract binary")
	}
	bz, err := hex.DecodeString(string(code))
	if err != nil {
		panic("failed to decode NativePaxTokensERC20 contract binary")
	}
	setCachedBin(bz)
