...
func GetQueryCmd(_ string) *cobra.Command {
	...
	cmd.AddCommand(CmdQueryPaxAddress())
...
func CmdQueryPaxAddress() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "pax-addr",
		Short: "gets pax address (pax...) by EVM address (0x...) if account has association set",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.PaxAddressByEVMAddress(context.Background(), &types.QueryPaxAddressByEVMAddressRequest{EvmAddress: args[0]})
			if err != nil {
				return err
			}
...
func CmdQueryEVMAddress() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "evm-addr",
		Short: "gets evm address (0x...) by Pax address (pax...) if account has association set",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			res, err := queryClient.EVMAddressByPaxAddress(context.Background(), &types.QueryEVMAddressByPaxAddressRequest{PaxAddress: args[0]})
			if err != nil {
				return err
			}
...
func CmdQueryERC20Payload() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "transfer":
				if len(args) != 3 {
					return errors.New("expected usage: `paxd tx evm erc20-payload transfer [to] [amount]`")
				}
			...
			case "approve":
				if len(args) != 3 {
					return errors.New("expected usage: `paxd tx evm erc20-payload approve [spender] [amount]`")
				}
			...
			case "transferFrom":
				if len(args) != 4 {
					return errors.New("expected usage: `paxd tx evm erc20-payload transferFrom [from] [to] [amount]`")
				}
...
func CmdQueryERC721Payload() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "approve":
				if len(args) != 3 {
					return errors.New("expected usage: `paxd tx evm erc721-payload approve [spender] [tokenId]`")
				}
				spender := common.HexToAddress(args[1])
			...
			case "transferFrom":
				if len(args) != 4 {
					return errors.New("expected usage: `paxd tx evm erc721-payload transferFrom [from] [to] [tokenId]`")
				}
				from := common.HexToAddress(args[1])
				to := common.HexToAddress(args[2])
			...
			case "setApprovalForAll":
				if len(args) != 3 {
					return errors.New("expected usage: `paxd tx evm erc721-payload setApprovalForAll [spender] [ture|false]`")
				}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "safeTransferFrom":
				if len(args) != 6 {
					return errors.New("expected usage: `paxd tx evm erc1155-payload safeTransferFrom [from] [to] [tokenId] [amount] [data]`")
				}
				from := common.HexToAddress(args[1])
				to := common.HexToAddress(args[2])
				id, _ := sdk.NewIntFromString(args[3])
			...
			case "safeBatchTransferFrom":
				if len(args) != 6 {
					return errors.New("expected usage: `paxd tx evm erc1155-payload safeBatchTransferFrom [from] [to] [tokenIds] [amounts] [data]`")
				}
				from := common.HexToAddress(args[1])
				to := common.HexToAddress(args[2])
			...
			case "setApprovalForAll":
				if len(args) != 3 {
					return errors.New("expected usage: `paxd tx evm erc1155-payload setApprovalForAll [spender] [ture|false]`")
				}
				op := common.HexToAddress(args[1])
				approved := args[2] == "true"
				bz, err = abi.Pack(args[0], op, approved)
