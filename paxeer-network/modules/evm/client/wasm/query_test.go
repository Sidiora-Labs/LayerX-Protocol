...
func TestHandleERC721RoyaltyInfo(t *testing.T) {
	...
	require.Nil(t, err)
	require.NotEmpty(t, res2)
	match, _ := regexp.MatchString(`{"receiver":"pax\w{39}","royalty_amount":"5"}`, string(res2))
...
func TestGetAddress(t *testing.T) {
	...
	k.SetAddressMapping(ctx, paxAddr1, evmAddr1)
	...
	res, err := h.HandleGetEvmAddress(ctx, paxAddr1.String())
	require.Nil(t, err)
	...
	getEvmAddrResp = &bindings.GetEvmAddressResponse{}
	res, err = h.HandleGetEvmAddress(ctx, paxAddr2.String())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getEvmAddrResp))
	require.True(t, getEvmAddrResp.Associated)
	getPaxAddrResp := &bindings.GetPaxAddressResponse{}
	res, err = h.HandleGetPaxAddress(ctx, evmAddr1.Hex())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getPaxAddrResp))
	require.True(t, getPaxAddrResp.Associated)
	require.Equal(t, paxAddr1.String(), getPaxAddrResp.PaxAddress)
	getPaxAddrResp = &bindings.GetPaxAddressResponse{}
	res, err = h.HandleGetPaxAddress(ctx, evmAddr2.Hex())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getPaxAddrResp))
	require.True(t, getPaxAddrResp.Associated)
}
