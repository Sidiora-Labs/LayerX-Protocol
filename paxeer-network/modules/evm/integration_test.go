...
func TestERC721RoyaltiesPointerToCW721Royalties(t *testing.T) {
	...
	adminPaxAddr, adminEvmAddr := testkeeper.MockAddressPair()
	k.SetAddressMapping(ctx, adminPaxAddr, adminEvmAddr)
	// deploy cw2981
	bz, err := os.ReadFile("../../contracts/wasm/cw721_royalties.wasm")
	if err != nil {
		panic(err)
	}
	codeID, err := k.WasmKeeper().Create(ctx, adminPaxAddr, bz, nil)
	require.Nil(t, err)
	instantiateMsg, err := json.Marshal(map[string]interface{}{"name": "test", "symbol": "TEST", "minter": adminPaxAddr.String()})
	require.Nil(t, err)
	cw2981Addr, _, err := k.WasmKeeper().Instantiate(ctx, codeID, adminPaxAddr, adminPaxAddr, instantiateMsg, "cw2981", sdk.NewCoins())
	require.Nil(t, err)
	...
	executeMsg, err := json.Marshal(map[string]interface{}{
		"mint": map[string]interface{}{
			"token_id": "1",
			"owner":    adminPaxAddr.String(),
			"extension": map[string]interface{}{
				"royalty_percentage":      1,
				"royalty_payment_address": adminPaxAddr.String(),
			},
		},
	})
	require.Nil(t, err)
	_, err = k.WasmKeeper().Execute(ctx, cw2981Addr, adminPaxAddr, executeMsg, sdk.NewCoins())
	require.Nil(t, err)
	...
	require.Nil(t, k.BankKeeper().AddCoins(ctx, paxAddr, sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(10000000))), true))
...
func TestCW721RoyaltiesPointerToERC721Royalties(t *testing.T) {
	...
	ret, err = testkeeper.EVMTestApp.WasmKeeper.QuerySmart(ctx, sdk.MustAccAddressFromBech32(res2.PointerAddress), query)
	require.Nil(t, err)
	require.Equal(t, fmt.Sprintf("{\"address\":\"%s\",\"royalty_amount\":\"1000\"}", paxAddr.String()), string(ret))
}
...
func TestCW1155RoyaltiesPointerToERC1155Royalties(t *testing.T) {
	...
	ret, err = testkeeper.EVMTestApp.WasmKeeper.QuerySmart(ctx, sdk.MustAccAddressFromBech32(res2.PointerAddress), query)
	require.Nil(t, err)
	require.Equal(t, fmt.Sprintf("{\"address\":\"%s\",\"royalty_amount\":\"50\"}", paxAddr.String()), string(ret))
}
...
func TestInvalidAssociateMsg(t *testing.T) {
	// EVM associate tx
	k := testkeeper.EVMTestApp.EvmKeeper
	ctx := testkeeper.EVMTestApp.GetContextForDeliverTx([]byte{}).WithBlockTime(time.Now()).WithChainID("pax-test").WithBlockHeight(1)
	privKey := testkeeper.MockPrivateKey()
	paxAddr, _ := testkeeper.PrivateKeyToAddresses(privKey)
	amt := sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(1000000)))
	k.BankKeeper().MintCoins(ctx, "evm", amt)
	k.BankKeeper().SendCoinsFromModuleToAccount(ctx, "evm", paxAddr, amt)
	testPrivHex := hex.EncodeToString(privKey.Bytes())
	key, _ := crypto.HexToECDSA(testPrivHex)
	...
	amsg := &types.MsgAssociate{
		Sender: paxAddr.String(), CustomMessage: customMsg,
	}
	txBuilder = testkeeper.EVMTestApp.GetTxConfig().NewTxBuilder()
	txBuilder.SetMsgs(amsg)
	signedTx := signTx(txBuilder, privKey, k.AccountKeeper().GetAccount(ctx, paxAddr))
	...
	amsg = &types.MsgAssociate{
		Sender: paxAddr.String(), CustomMessage: "",
	}
	txBuilder = testkeeper.EVMTestApp.GetTxConfig().NewTxBuilder()
	msgs := []sdk.Msg{}
	for i := 1; i <= 1000; i++ {
