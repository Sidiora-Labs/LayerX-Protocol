...
func (k *Keeper) SetCode(ctx sdk.Context, addr common.Address, code []byte) {
	...
	// set association with direct cast Pax address for the contract address
	if _, ok := k.GetPaxAddress(ctx, addr); !ok {
		k.SetAddressMapping(ctx, k.GetPaxAddressOrDefault(ctx, addr), addr)
	}
}
...
func (k *Keeper) GetCodeHash(ctx sdk.Context, addr common.Address) common.Hash {
	...
	if bz == nil {
		...
		if k.GetBalance(ctx, k.GetPaxAddressOrDefault(ctx, addr)).Cmp(utils.Big0) == 0 && k.GetNonce(ctx, addr) == 0 {
			return common.Hash{}
		}
