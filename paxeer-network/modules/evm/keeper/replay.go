...
func (k *Keeper) VerifyBalance(ctx sdk.Context, addr common.Address) {
	uhpxBalance := k.BankKeeper().GetBalance(ctx, k.GetPaxAddressOrDefault(ctx, addr), k.GetBaseDenom(ctx)).Amount
	weiBalance := k.bankKeeper.GetWeiBalance(ctx, k.GetPaxAddressOrDefault(ctx, addr))
	totalPaxBalance := uhpxBalance.Mul(sdk.NewInt(1_000_000_000_000)).Add(weiBalance).BigInt()
	ethBalance, err := k.EthClient.BalanceAt(ctx.Context(), addr, big.NewInt(k.GetReplayInitialHeight(ctx)+ctx.BlockHeight()))
	if err != nil {
		panic(err)
	}
	if totalPaxBalance.Cmp(ethBalance) != 0 {
		panic(fmt.Sprintf("difference for addr %s: pax balance is %s, eth balance is %s", addr.Hex(), totalPaxBalance, ethBalance))
	}
}
...
func (k *Keeper) VerifyAccount(ctx sdk.Context, addr common.Address, accountData ethtypes.Account) {
	// We no longer check the ETH balance because the EVM maximum refund is limited to 150% of used gas.
