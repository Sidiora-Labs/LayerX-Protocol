...
func (k *Keeper) PruneZeroStorageSlots(ctx sdk.Context, limit int) (int, int) {
	...
	paxmetrics.IncrEvmZeroStorageProcessedKeys(processedMetric)                          // TODO(PLT-330): remove once evm_zero_storage_processed_keys_total verified
	evmKeeperMetrics.zeroStorageProcessedKeys.Add(ctx.Context(), int64(processedMetric)) //nolint:gosec

	if deleted > 0 {
		paxmetrics.IncrEvmZeroStoragePrunedKeys(deletedMetric)                          // TODO(PLT-330): remove once evm_zero_storage_pruned_keys_total verified
		paxmetrics.IncrEvmZeroStoragePrunedBytes(bytesPruned)                           // TODO(PLT-330): remove once evm_zero_storage_pruned_bytes_total verified
