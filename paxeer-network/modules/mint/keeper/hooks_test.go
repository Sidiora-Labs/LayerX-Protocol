...
func TestEndOfEpochMintedCoinDistribution(t *testing.T) {
	...
	t.Run("Initial should be zero", func(t *testing.T) {
		paxApp := keepertest.TestApp(t)
		ctx := paxApp.BaseApp.NewContext(false, tmproto.Header{Time: time.Now()})

		header := tmproto.Header{
			Height: paxApp.LastBlockHeight() + 1,
			Time:   time.Now().UTC(),
		}
		legacyabci.BeginBlock(ctx, header.Height, []abci.VoteInfo{}, []abci.Misbehavior{}, paxApp.BeginBlockKeepers)
		require.Equal(t, int64(0), paxApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin().Amount.Int64())
	})
	...
	t.Run("even full release", func(t *testing.T) {
		...
		paxApp.MintKeeper.SetParams(ctx, mintParams)

		for i := 0; i < 25; i++ {
			currTime := genesisTime.AddDate(0, 0, i)
			currEpoch := getEpoch(genesisTime, currTime)
			paxApp.EpochKeeper.BeforeEpochStart(ctx, currEpoch)
			paxApp.EpochKeeper.AfterEpochEnd(ctx, currEpoch)
			mintParams = paxApp.MintKeeper.GetParams(ctx)

			// 250k / 25 days = 100000 per day
			expectedAmount := int64(100000)
			newMinter := paxApp.MintKeeper.GetMinter(ctx)
...
func TestNoEpochPassedNoDistribution(t *testing.T) {
	...
	header := tmproto.Header{Height: paxApp.LastBlockHeight() + 1}
	legacyabci.BeginBlock(ctx, header.Height, []abci.VoteInfo{}, []abci.Misbehavior{}, paxApp.BeginBlockKeepers)
	// Get mint params
	mintParams := paxApp.MintKeeper.GetParams(ctx)
	genesisTime := time.Date(2022, time.Month(7), 18, 10, 0, 0, 0, time.UTC)
	presupply := paxApp.BankKeeper.GetSupply(ctx, mintParams.MintDenom)
	startLastMintAmount := paxApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin()
	...
	for i := 0; i < 60*24*7*52-1; i++ {
		...
		currSupply := paxApp.BankKeeper.GetSupply(ctx, mintParams.MintDenom)
		require.True(t, currSupply.IsEqual(presupply))
	}
	// Ensure that EpochProvision hasn't changed
	endLastMintAmount := paxApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin()
