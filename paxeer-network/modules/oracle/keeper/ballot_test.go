...
func TestOrganizeAggregate(t *testing.T) {
	...
	sdrBallot := types.ExchangeRateBallot{
		types.NewVoteForTally(sdk.NewDec(17), utils.MicroPaxDenom, testutils.ValAddrs[0], power),
		types.NewVoteForTally(sdk.NewDec(10), utils.MicroPaxDenom, testutils.ValAddrs[1], power),
		types.NewVoteForTally(sdk.NewDec(6), utils.MicroPaxDenom, testutils.ValAddrs[2], power),
	}
	...
	sort.Sort(sdrBallot)
	sort.Sort(krwBallot)
	sort.Sort(ballotMap[utils.MicroPaxDenom])
	sort.Sort(ballotMap[utils.MicroAtomDenom])

	require.Equal(t, sdrBallot, ballotMap[utils.MicroPaxDenom])
