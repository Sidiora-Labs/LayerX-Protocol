...
func TestExchangeRate(t *testing.T) {
	...
	// Set & get rates
	input.OracleKeeper.SetBaseExchangeRate(input.Ctx, utils.MicroPaxDenom, cnyExchangeRate)
	rate, lastUpdate, _, err := input.OracleKeeper.GetBaseExchangeRate(input.Ctx, utils.MicroPaxDenom)
	...
	input.OracleKeeper.SetVoteTarget(input.Ctx, utils.MicroPaxDenom)
...
func TestIteratePaxExchangeRates(t *testing.T) {
	input := testutils.CreateTestInput(t)
...
func TestVoteTargetGetSet(t *testing.T) {
	...
	voteTargets := map[string]types.Denom{
		...
		utils.MicroPaxDenom:  {Name: utils.MicroPaxDenom},
	}
