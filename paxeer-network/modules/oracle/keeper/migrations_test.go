...
func TestMigrate2to3(t *testing.T) {
	...
	store.Set(types.GetExchangeRateKey(utils.MicroPaxDenom), bz)

	// validate legacy store value
	b := store.Get(types.GetExchangeRateKey(utils.MicroPaxDenom))
	dp := sdk.DecProto{}
	...
	// Get rate
	rate, lastUpdate, lastUpdateTimestamp, err := input.OracleKeeper.GetBaseExchangeRate(input.Ctx, utils.MicroPaxDenom)
