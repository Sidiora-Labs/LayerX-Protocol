...
var (
	...
	InitCoins  = sdk.NewCoins(sdk.NewCoin(utils.MicroPaxDenom, InitTokens))
...
func CreateTestInput(t *testing.T) TestInput {
	...
	totalSupply := sdk.NewCoins(sdk.NewCoin(utils.MicroPaxDenom, InitTokens.MulRaw(int64(len(Addrs)*10))))
	...
	stakingParams.BondDenom = utils.MicroPaxDenom
	...
	bankKeeper.SendCoinsFromModuleToModule(ctx, faucetAccountName, stakingtypes.NotBondedPoolName, sdk.NewCoins(sdk.NewCoin(utils.MicroPaxDenom, InitTokens.MulRaw(int64(len(Addrs))))))
...
func NewTestMsgCreateValidator(address sdk.ValAddress, pubKey cryptotypes.PubKey, amt sdk.Int) *stakingtypes.MsgCreateValidator {
	...
	msg, _ := stakingtypes.NewMsgCreateValidator(
		address, pubKey, sdk.NewCoin(utils.MicroPaxDenom, amt),
