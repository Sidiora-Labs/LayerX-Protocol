...
import (
	...
	"github.com/sidiora-labs/paxeer-network/sdk/tasks"
...
func (app *App) BeginBlock(
	...
	utilmetrics.GaugePaxdVersionAndCommit(app.versionInfo.Version, app.versionInfo.GitCommit) // TODO(PLT-327): remove once app_build_info observable gauge verified
...
func (app *App) CheckTx(ctx context.Context, req *abci.RequestCheckTxV2) *abci.ResponseCheckTxV2 {
	...
	res := &abci.ResponseCheckTxV2{
		...
		PaxSenderAddress:   txCtx.PaxSenderAddress(),
		IsEVM:              txCtx.IsEVM(),
...
func (app *App) EvmBalance(evmAddr common.Address, paxAddrBz []byte) uint256.Int {
	ctx := app.GetCheckCtx()
	balance := app.EvmKeeper.GetBalance(ctx, evmAddr[:])
	paxAddr := sdk.AccAddress(paxAddrBz)
	if !paxAddr.Equals(sdk.AccAddress(evmAddr[:])) {
		balance = new(big.Int).Add(balance, app.EvmKeeper.GetBalance(ctx, paxAddr))
	}
...
func (app *App) Commit(ctx context.Context) (res *abci.ResponseCommit, err error) {
	...
	// legacy: telemetry.MeasureSince in cosmos/baseapp/abci.go TODO(PLT-327)
