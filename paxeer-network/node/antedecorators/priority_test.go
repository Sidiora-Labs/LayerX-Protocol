...
func TestPriorityWithExactAnteChain_BankSend(t *testing.T) {
	...
	ctx := testApp.NewContext(false, tmproto.Header{ChainID: "pax-test"}).WithBlockHeight(2).WithIsCheckTx(true)
	...
	from, _ := sdk.AccAddressFromBech32("pax1y3pxq5dp900czh0mkudhjdqjq5m8cpmmsnt288")
	to, _ := sdk.AccAddressFromBech32("pax1jdppe6fnj2q7hjsepty5crxtrryzhuqsrq7tpd")
	msg := &banktypes.MsgSend{FromAddress: from.String(), ToAddress: to.String(), Amount: sdk.NewCoins(sdk.NewInt64Coin("uhpx", 1))}

	// fund the sender to cover fees
	fund := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 1_000_000_000))
	...
	txb.SetFeeAmount(sdk.NewCoins(sdk.NewInt64Coin("uhpx", 100_000)))
	tx := txb.GetTx()
