...
) ([]func(*baseapp.BaseApp), paxdb.StateStore) {
	scEnabled := cast.ToBool(appOpts.Get(FlagSCEnable))
	if !scEnabled {
		panic("PaxDB state-commit (SC) must be enabled; IAVL backend has been fully deprecated")
	}
	scConfig := parseSCConfigs(appOpts)
	logger.Info("PaxDB SC is enabled now", "sc-config", scConfig)
	ssConfig := parseSSConfigs(appOpts)
	if ssConfig.Enable {
		logger.Info("PaxDB SS is enabled", "backend", ssConfig.Backend)
	}
	if ssConfig.EVMSplit {
		logger.Info("PaxDB EVM StateStore optimization is enabled",
...
func validateConfigs(appOpts servertypes.AppOptions) {
	...
	if snapshotExportInterval > 0 && scEnabled {
		if !ssEnabled {
			panic(fmt.Sprintf("Config validation failed, PaxDB SS store needs to be enabled when snapshot interval %d > 0", snapshotExportInterval))
		}
	}
}
