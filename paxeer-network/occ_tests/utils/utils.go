...
import (
	...
	txtype "github.com/sidiora-labs/paxeer-network/sdk/types/tx"
	...
	wasmxtypes "github.com/sidiora-labs/paxeer-network/wasm/x/wasm/types"
...
func Funds(amount int64) sdk.Coins {
	return sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(amount)))
}
...
func NewTestContext(tb testing.TB, testAccts []TestAcct, blockTime time.Time, workers int, occEnabled bool) *TestContext {
	...
	amounts := sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(1000000000000000)), sdk.NewCoin("uusdc", sdk.NewInt(1000000000000000)))
...
func toTxBytes(testCtx *TestContext, msgs []*TestMessage) [][]byte {
	...
	for _, tm := range msgs {
		...
		if tm.IsEVM {
			amounts := sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(1000000000000000000)), sdk.NewCoin("uusdc", sdk.NewInt(1000000000000000)))
