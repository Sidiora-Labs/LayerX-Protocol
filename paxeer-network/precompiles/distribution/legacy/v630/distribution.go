...
func (p PrecompileExecutor) setWithdrawAddress(ctx sdk.Context, method *abi.Method, caller common.Address, args []interface{}, value *big.Int, evm *vm.EVM) (ret []byte, remainingGas uint64, rerr error) {
	...
	delegator, found := p.evmKeeper.GetPaxAddress(ctx, caller)
	if !found {
...
func (p PrecompileExecutor) accAddressFromArg(ctx sdk.Context, arg interface{}) (sdk.AccAddress, error) {
	...
	paxAddr, associated := p.evmKeeper.GetPaxAddress(ctx, addr)
	if !associated {
...
func (p PrecompileExecutor) rewards(ctx sdk.Context, method *abi.Method, args []interface{}) (ret []byte, remainingGas uint64, rerr error) {
	...
	paxDelegatorAddress, err := p.accAddressFromArg(ctx, args[0])
	if err != nil {
		rerr = err
		return
	}
	...
	req := &distrtypes.QueryDelegationTotalRewardsRequest{
		DelegatorAddress: paxDelegatorAddress.String(),
	}
...
func (p PrecompileExecutor) withdrawValidatorCommission(ctx sdk.Context, method *abi.Method, caller common.Address, evm *vm.EVM) (ret []byte, remainingGas uint64, rerr error) {
	...
	validatorPaxAddr, found := p.evmKeeper.GetPaxAddress(ctx, caller)
	if !found {
		rerr = types.NewAssociationMissingErr(caller.Hex())
		return
	}

	validatorAddr := sdk.ValAddress(validatorPaxAddr)
