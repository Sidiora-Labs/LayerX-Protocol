...
func TestGovPrecompile(t *testing.T) {
	...
	tests := []struct {
		name                string
		args                args
		setup               func(ctx sdk.Context, k *keeper.Keeper, evmAddr common.Address, paxAddr sdk.AccAddress)
		verify              func(t *testing.T, ctx sdk.Context, paxAddr sdk.AccAddress, proposalID uint64)
		wantErr             bool
		...
		},
		{
			...
			verify: func(t *testing.T, ctx sdk.Context, paxAddr sdk.AccAddress, proposalID uint64) {
				v, found := testApp.GovKeeper.GetVote(ctx, proposalID, paxAddr)
		...
		},
		{
			...
			setup: func(ctx sdk.Context, k *keeper.Keeper, evmAddr common.Address, paxAddr sdk.AccAddress) {},
			verify: func(t *testing.T, ctx sdk.Context, paxAddr sdk.AccAddress, proposalID uint64) {
				_, found := testApp.GovKeeper.GetVote(ctx, proposalID, paxAddr)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setup(ctx, k, evmAddr, paxAddr)
			...
			if tt.wantErr {
				...
				tt.verify(t, ctx, paxAddr, tt.args.proposal)
			}
		})
	}
}
...
func TestPrecompileExecutor_submitProposal(t *testing.T) {
	...
	privKey := testkeeper.MockPrivateKey()
	callerPaxAddress, callerEvmAddress := testkeeper.PrivateKeyToAddresses(privKey)
	recipientPaxAddress, recipientEvmAddress := testkeeper.MockAddressPair()

	type args struct {
		caller           common.Address
		callerPaxAddress sdk.AccAddress
		proposal         string
		value            *big.Int
	}
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				callerPaxAddress: callerPaxAddress,
				proposal: `{
                      "title":"Test Proposal",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			k.SetAddressMapping(ctx, tt.args.callerPaxAddress, tt.args.caller)
			k.SetAddressMapping(ctx, recipientPaxAddress, recipientEvmAddress)
			...
			require.Nil(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, evmtypes.ModuleName, tt.args.callerPaxAddress, sendAmt))
