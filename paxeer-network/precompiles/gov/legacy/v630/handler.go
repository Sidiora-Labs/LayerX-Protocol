...
type EVMKeeper interface {
	GetPaxAddress(ctx sdk.Context, evmAddr common.Address) (sdk.AccAddress, bool)
}
...
type CommunityPoolSpend struct {
	...
	Amount    string `json:"amount"`    // Amount in the format "1000000uhpx"
}
...
func (h CommunityPoolSpendProposalHandler) HandleProposal(ctx sdk.Context, proposal Proposal) (govtypes.Content, error) {
	...
	// Convert Ethereum address to Pax address using the EVM keeper
	ethAddr := common.HexToAddress(proposal.CommunityPoolSpend.Recipient)
	paxAddr, found := h.evmKeeper.GetPaxAddress(ctx, ethAddr)
	if !found {
		return nil, fmt.Errorf("no pax address found for ethereum address %s", ethAddr.Hex())
	}
