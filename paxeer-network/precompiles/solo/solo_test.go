...
func TestExecute(t *testing.T) {
	...
	ctx := testkeeper.EVMTestApp.GetContextForDeliverTx(nil).WithChainID("pax-test").WithIsEVM(true)
...
func TestClaim(t *testing.T) {
	k := &testkeeper.EVMTestApp.EvmKeeper
	origCtx := testkeeper.EVMTestApp.GetContextForDeliverTx(nil).WithChainID("pax-test")
	txConfig := testkeeper.EVMTestApp.GetTxConfig()
	a := pcommon.MustGetABI(solo.F, "abi.json")
	...
	require.Equal(t, sdk.NewInt(2), k.BankKeeper().GetBalance(ctx, k.GetPaxAddressOrDefault(ctx, claimer), "abc").Amount)
	require.Equal(t, sdk.NewInt(3), k.BankKeeper().GetBalance(ctx, k.GetPaxAddressOrDefault(ctx, claimer), "def").Amount)
...
func TestClaimSpecificCW20(t *testing.T) {
	k := &testkeeper.EVMTestApp.EvmKeeper
	origCtx := testkeeper.EVMTestApp.GetContextForDeliverTx(nil).WithChainID("pax-test").WithBlockTime(time.Now())
	txConfig := testkeeper.EVMTestApp.GetTxConfig()
	a := pcommon.MustGetABI(solo.F, "abi.json")
	...
	require.Equal(t, sdk.NewInt(1000000000), queryCW20Balance(ctx, testkeeper.EVMTestApp.WasmKeeper, contractAddr, k.GetPaxAddressOrDefault(ctx, claimer)))
}
...
func TestClaimSpecificCW721(t *testing.T) {
	...
	for i := 0; i < 15; i++ {
		require.Equal(t, k.GetPaxAddressOrDefault(ctx, claimer).String(), queryCW721Owner(ctx, testkeeper.EVMTestApp.WasmKeeper, contractAddr, fmt.Sprintf("%d", i)))
	}
}
...
func TestClaimSpecificNative(t *testing.T) {
	...
	require.NoError(t, err)
	require.Greater(t, remainingGas, uint64(900000))
	require.Equal(t, sdk.OneInt(), k.BankKeeper().GetBalance(ctx, k.GetPaxAddressOrDefault(ctx, claimer), "foo").Amount)
