...
var logger = paxlog.NewLogger("precompiles", "staking", "legacy", "v630")
...
func (p PrecompileExecutor) delegate(ctx sdk.Context, method *abi.Method, caller common.Address, args []interface{}, value *big.Int, hooks *tracing.Hooks, evm *vm.EVM) ([]byte, uint64, error) {
	...
	delegator, associated := p.evmKeeper.GetPaxAddress(ctx, caller)
	if !associated {
	...
	coin, err := pcommon.HandlePaymentUhpx(ctx, p.evmKeeper.GetPaxAddressOrDefault(ctx, p.address), delegator, value, p.bankKeeper, p.evmKeeper, hooks, evm.GetDepth())
	if err != nil {
		return nil, 0, err
	}
...
func (p PrecompileExecutor) delegation(ctx sdk.Context, method *abi.Method, args []interface{}, value *big.Int) ([]byte, uint64, error) {
	...
	paxDelegatorAddress, err := pcommon.GetPaxAddressFromArg(ctx, args[0], p.evmKeeper)
	if err != nil {
		return nil, 0, err
	}
	...
	delegationRequest := &stakingtypes.QueryDelegationRequest{
		DelegatorAddr: paxDelegatorAddress.String(),
...
func (p PrecompileExecutor) createValidator(ctx sdk.Context, method *abi.Method, caller common.Address, args []interface{}, value *big.Int, hooks *tracing.Hooks, evm *vm.EVM) ([]byte, uint64, error) {
	...
	// Get validator address (caller's associated Pax address)
	valAddress, associated := p.evmKeeper.GetPaxAddress(ctx, caller)
	if !associated {
		return nil, 0, types.NewAssociationMissingErr(caller.Hex())
	}
	...
	coin, err := pcommon.HandlePaymentUhpx(
		ctx,
		p.evmKeeper.GetPaxAddressOrDefault(ctx, p.address),
		valAddress,
		value,
		p.bankKeeper,
		p.evmKeeper,
