...
import (
	...
	ethtx "github.com/sidiora-labs/paxeer-network/modules/evm/types/ethtx"
...
func TestStakingPrecompileEventsEmission(t *testing.T) {
	...
	t.Run("TestDelegateEvent", func(t *testing.T) {
		...
		delegateAmount := big.NewInt(100_000_000_000_000) // 100 uhpx in wei
	...
	t.Run("TestRedelegateEvent", func(t *testing.T) {
		...
		delegateTx := createEVMTx(t, k, ctx, privKey, &addr, delegateArgs, big.NewInt(100_000_000_000_000)) // 100 uhpx in wei
		...
		abi := pcommon.MustGetABI(f, "abi.json")
		redelegateAmount := big.NewInt(50) // 50 uhpx (same as original test)
	...
	t.Run("TestRedelegateRewardsWithdrawnEvents", func(t *testing.T) {
		...
		abi := pcommon.MustGetABI(f, "abi.json")
		redelegateAmount := big.NewInt(50) // 50 uhpx
		args, err := abi.Pack("redelegate", valStr, valStr2, redelegateAmount)
		require.NoError(t, err)
	...
	t.Run("TestUndelegateEvent", func(t *testing.T) {
		...
		abi := pcommon.MustGetABI(f, "abi.json")
		undelegateAmount := big.NewInt(30) // 30 uhpx (same as original test)
	...
	t.Run("TestUndelegateRewardsWithdrawnEvent", func(t *testing.T) {
		...
		// Now undelegate some funds
		abi := pcommon.MustGetABI(f, "abi.json")
		undelegateAmount := big.NewInt(30) // 30 uhpx
		args, err := abi.Pack("undelegate", valStr, undelegateAmount)
		require.NoError(t, err)
	...
	t.Run("TestCreateValidatorEvent", func(t *testing.T) {
		...
		validatorPaxAddr, validatorEvmAddr := testkeeper.PrivateKeyToAddresses(validatorPrivKey)
		k.SetAddressMapping(ctx, validatorPaxAddr, validatorEvmAddr)

		// Fund the validator account
		amt := sdk.NewCoins(sdk.NewCoin(k.GetBaseDenom(ctx), sdk.NewInt(1000000000000)))
		require.NoError(t, k.BankKeeper().MintCoins(ctx, evmtypes.ModuleName, amt))
		require.NoError(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, evmtypes.ModuleName, validatorPaxAddr, amt))
		...
		addr := common.HexToAddress(staking.StakingAddress)
		delegateAmount := big.NewInt(1_000_000_000_000_000_000) // 1 PAX in wei (1,000,000 uhpx)
