...
func TestPrecompile_Run_Delegation(t *testing.T) {
	...
	validatorAddress := "paxvaloper134ykhqrkyda72uq7f463ne77e4tn99staqk0ny"
	...
	delegationResponse := &stakingtypes.QueryDelegationResponse{
		DelegationResponse: &stakingtypes.DelegationResponse{
			Delegation: stakingtypes.Delegation{
				DelegatorAddress: callerPaxAddress.String(),
				ValidatorAddress: validatorAddress,
				Shares:           sdk.NewDec(int64(shares)),
			},
			Balance: sdk.NewCoin("uhpx", sdk.NewInt(int64(shares))),
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				stakingQuerier: &TestStakingQuerier{
					Err: fmt.Errorf("delegation with delegator %s not found for validator", callerPaxAddress.String()),
				},
			},
			args: args{
				delegatorAddress: callerEvmAddress,
			...
			wantErr:    true,
			wantErrMsg: fmt.Sprintf("delegation with delegator %s not found for validator", callerPaxAddress.String()),
		},
		{
...
func TestPrecompile_Run_Validators(t *testing.T) {
	...
	val := stakingtypes.Validator{
		OperatorAddress: "paxvaloper1validator",
...
func TestPrecompile_Run_ValidatorDelegations(t *testing.T) {
	...
	callerPaxAddress, _ := testkeeper.MockAddressPair()
	validatorAddress := "paxvaloper134ykhqrkyda72uq7f463ne77e4tn99staqk0ny"
	shares := 100
	delegationResponse := &stakingtypes.DelegationResponse{
		Delegation: stakingtypes.Delegation{
...
func TestPrecompile_Run_Redelegations(t *testing.T) {
	...
	callerPaxAddress, _ := testkeeper.MockAddressPair()
	srcValidatorAddress := "paxvaloper1src"
	dstValidatorAddress := "paxvaloper1dst"
	...
	tests := []struct {
	...
	}{
		{
			...
			args:       []interface{}{callerPaxAddress.String(), srcValidatorAddress, dstValidatorAddress, []byte{}},
			value:      big.NewInt(1),
			wantErr:    true,
			wantErrMsg: "sending funds to a non-payable function",
		},
...
func setupCreateValidatorTest(t *testing.T) *createValidatorTestSetup {
	...
	paxAddr, evmAddr := testkeeper.PrivateKeyToAddresses(testPrivKey)
	k.SetAddressMapping(ctx, paxAddr, evmAddr)
	...
	require.NoError(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, evmtypes.ModuleName, paxAddr, amt))
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				msd:                     big.NewInt(1_000_000),                 // 1 PAX in uhpx
				value:                   big.NewInt(1_000_000_000_000_000_000), // 1 PAX in wei
				nonce:                   0,
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				paxAddr, _ := testkeeper.PrivateKeyToAddresses(setup.testPrivKey)
				valAddr := sdk.ValAddress(paxAddr)
...
func TestStakingPrecompileDelegateCallPrevention(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			name:   "delegate",
			method: "delegate",
			args:   []interface{}{"paxvaloper1test"},
			value:  big.NewInt(100),
		},
		{
			name:   "redelegate",
			method: "redelegate",
			args:   []interface{}{"paxvaloper1src", "paxvaloper1dst", big.NewInt(50)},
			value:  nil,
		},
		{
			name:   "undelegate",
			method: "undelegate",
			args:   []interface{}{"paxvaloper1test", big.NewInt(30)},
			value:  nil,
		},
		{
