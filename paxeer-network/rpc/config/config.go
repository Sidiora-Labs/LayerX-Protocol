...
type Config struct {
	...
	// EnabledLegacyPaxApis lists which gated pax_* and pax2_* JSON-RPC methods are allowed on the EVM HTTP endpoint.
	// Set in app.toml [evm] as enabled_legacy_pax_apis (see ReadConfig and ConfigTemplate defaults).
	EnabledLegacyPaxApis []string `mapstructure:"enabled_legacy_pax_apis"`
...
const (
	...
	flagEVMLegacyPaxApis             = "evm.enabled_legacy_pax_apis"
...
func ReadConfig(opts servertypes.AppOptions) (Config, error) {
	...
	if v := opts.Get(flagEVMLegacyPaxApis); v != nil {
		if cfg.EnabledLegacyPaxApis, err = cast.ToStringSliceE(v); err != nil {
			return cfg, err
		}
	}
...
# Legacy pax_* / pax2_* JSON-RPC (EVM HTTP only - not Cosmos REST on 1317).
#
# DEPRECATION: The pax_* and pax2_* JSON-RPC surfaces are deprecated and scheduled for removal. Do not
# build new integrations on them; use eth_* / debug_* and documented replacements. HTTP 200;
# gate errors use standard JSON-RPC error encoding (see rpc/AGENTS.md). Successful allowlisted
# responses are unchanged; nodes may set HTTP header Pax-Legacy-RPC-Deprecation (see AGENTS.md).
#
# Only methods listed in enabled_legacy_pax_apis are allowed. Init defaults enable the three
# address/Cosmos helpers; uncomment optional lines below to enable more legacy methods (include
# pax2_* block methods at the end of the list if you need them).
enabled_legacy_pax_apis = [
{{- range .EVM.EnabledLegacyPaxApis }}
  "{{ . }}",
{{- end }}
  ...
  # "pax_uninstallFilter",
  #
  # Optional pax2_* block namespace (bank transfers in blocks; HTTP only):
  # "pax2_getBlockByHash",
