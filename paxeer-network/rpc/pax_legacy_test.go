...
func TestBuildPaxLegacyEnabledSet_Empty(t *testing.T) {
	s := BuildPaxLegacyEnabledSet(nil)
	if len(s) != 0 {
		t.Fatalf("expected empty set, got %v", s)
	}
}

func TestBuildPaxLegacyEnabledSet_InitDefaults(t *testing.T) {
	s := BuildPaxLegacyEnabledSet([]string{"pax_getPaxAddress", "pax_getEVMAddress", "pax_getCosmosTx"})
	if len(s) != 3 {
		t.Fatalf("want 3 entries, got %d", len(s))
	}
	if _, ok := s["pax_getBlockByNumber"]; ok {
		t.Fatal("block should be off")
	}
}

func TestBuildPaxLegacyEnabledSet_Extra(t *testing.T) {
	s := BuildPaxLegacyEnabledSet([]string{"pax_getBlockByNumber", "PAX_GETBLOCKRECEIPTS"})
	if _, ok := s["pax_getBlockByNumber"]; !ok {
		t.Fatal("expected pax_getBlockByNumber")
	}
	if _, ok := s["pax_getBlockReceipts"]; !ok {
		t.Fatal("expected case-insensitive match")
	}
}

func TestPaxLegacyGateError_DisabledWhenEmptyAllowlist(t *testing.T) {
	err := paxLegacyGateError("pax_getBlockByNumber", BuildPaxLegacyEnabledSet(nil))
	if err == nil {
	...
	if withData.ErrorData() != "legacy_pax_deprecated" {
...
func TestPaxLegacyGateError_AllowedWhenListed(t *testing.T) {
	enabled := BuildPaxLegacyEnabledSet([]string{"pax_getBlockByNumber"})
	err := paxLegacyGateError("Pax_GetBlockByNumber", enabled)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
}

func TestPaxLegacyGateError_UnknownPaxNamespaceFailsClosed(t *testing.T) {
	enabled := BuildPaxLegacyEnabledSet([]string{"pax_getBlockByNumber"})
	err := paxLegacyGateError("pax_notARealRegisteredMethod", enabled)
	if err == nil {
		t.Fatal("expected error for unknown pax_* method when allowlist is active")
	}
	var withData rpc.DataError
	if !errors.As(err, &withData) {
		t.Fatalf("want rpc.DataError, got %T", err)
...
func TestPaxLegacyGateError_Pax2BlockedUnlessListed(t *testing.T) {
	err := paxLegacyGateError("pax2_getBlockByNumber", BuildPaxLegacyEnabledSet(nil))
	if err == nil {
		t.Fatal("expected error")
	}
	enabled := BuildPaxLegacyEnabledSet([]string{"pax2_getBlockByNumber"})
	if err := paxLegacyGateError("PAX2_GETBLOCKBYNUMBER", enabled); err != nil {
		t.Fatalf("unexpected: %v", err)
	}
}

func TestBuildPaxLegacyEnabledSet_IncludesPax2(t *testing.T) {
	s := BuildPaxLegacyEnabledSet([]string{"pax2_getBlockReceipts"})
	if _, ok := s["pax2_getBlockReceipts"]; !ok {
		t.Fatalf("got %v", s)
	}
	if _, ok := s["pax_getBlockReceipts"]; ok {
		t.Fatal("pax_* should not be enabled from pax2_ name only")
	}
}

func TestPaxLegacyGateError_NilAllowlistUngated(t *testing.T) {
	err := paxLegacyGateError("pax_getBlockByNumber", nil)
	if err != nil {
		t.Fatal(err)
	}
}

func TestWrapPaxLegacyHTTP_UnknownPaxMethodBlocked(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner should not run for unknown pax_* method")
	})
	enabled := BuildPaxLegacyEnabledSet([]string{"pax_getBlockByNumber"})
	h := wrapPaxLegacyHTTP(inner, enabled)
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(
		`{"jsonrpc":"2.0","id":1,"method":"pax_futureHypotheticalMethod","params":[]}`))
	...
	if errObj["data"] != "legacy_pax_deprecated" {
		t.Fatalf("error data: %v", errObj)
	}
}

func TestWrapPaxLegacyHTTP_BlocksDisabledMethod(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner should not run")
	})
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(
		`{"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":["0x1",false]}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	var resp map[string]interface{}
...
func TestWrapPaxLegacyHTTP_AllowedMethodPassthroughAndDeprecationHeader(t *testing.T) {
	called := false
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		called = true
	...
	h := wrapPaxLegacyHTTP(inner, enabled)
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(
		`{"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":["latest",false]}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if !called {
	...
	if rec.Header().Get(PaxLegacyDeprecationHTTPHeader) == "" {
		t.Fatal("expected deprecation HTTP header on allowlisted pax_* response")
	}
}

func TestWrapPaxLegacyHTTP_StringResultPassthrough(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"jsonrpc":"2.0","id":1,"result":"bech32addr"}`))
	})
	enabled := BuildPaxLegacyEnabledSet([]string{"pax_getPaxAddress"})
	h := wrapPaxLegacyHTTP(inner, enabled)
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(
		`{"jsonrpc":"2.0","id":1,"method":"pax_getPaxAddress","params":["0x0000000000000000000000000000000000000001"]}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	var resp map[string]interface{}
...
func TestWrapPaxLegacyHTTP_Pax2BlockedWhenNotAllowlisted(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner should not run")
	})
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(
		`{"jsonrpc":"2.0","id":1,"method":"pax2_getBlockByNumber","params":["latest",false]}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	var resp map[string]interface{}
...
func TestWrapPaxLegacyHTTP_Pax2AllowlistedPassthroughAndHeader(t *testing.T) {
	called := false
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		called = true
		_, _ = w.Write([]byte(`{"jsonrpc":"2.0","id":1,"result":{"number":"0x1"}}`))
...
func TestWrapPaxLegacyHTTP_EthPassthrough(t *testing.T) {
	called := false
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		called = true
...
func TestWrapPaxLegacyHTTP_BatchTrailingNonObjectDoesNotBypassGate(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner must not run when all batch slots are answered by the gate (blocked + invalid)")
	})
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	body := `[{"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":["0x1",false]},42]`
	...
	if err0 == nil || err0["data"] != "legacy_pax_deprecated" {
...
func TestWrapPaxLegacyHTTP_BatchLeadingNonObjectDoesNotBypassGate(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner must not run when all batch slots are answered by the gate")
	})
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	body := `[42,{"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":["0x1",false]}]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	...
	err1, _ := batch[1]["error"].(map[string]any)
	if err1 == nil || err1["data"] != "legacy_pax_deprecated" {
		t.Fatalf("slot 1 should be legacy gate error: %+v", batch[1])
	}
}

func TestWrapPaxLegacyHTTP_BatchMixed(t *testing.T) {
	...
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	body := `[{"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":[]},{"jsonrpc":"2.0","id":2,"method":"eth_chainId","params":[]}]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	...
	if err0["data"] != "legacy_pax_deprecated" {
		t.Fatalf("blocked error data: %+v", err0)
	}
}

func TestWrapPaxLegacyHTTP_BatchInvalidNonObjectNotForwarded(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestWrapPaxLegacyHTTP_BatchNotificationOmittedFromMergedResponse(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		var fwd []map[string]any
...
func TestWrapPaxLegacyHTTP_BatchNullIDIsNotNotification(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		var fwd []map[string]any
		if err := json.Unmarshal(b, &fwd); err != nil || len(fwd) != 2 {
...
// TestWrapPaxLegacyHTTP_BatchTwoNullIDsDifferentResults is the regression test for the bug where multiple
...
func TestWrapPaxLegacyHTTP_BatchTwoNullIDsDifferentResults(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		var fwd []map[string]any
	...
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	// Two null-ID requests + a notification + a regular request + a blocked pax_ method.
	...
	body := `[
		...
		{"jsonrpc":"2.0","method":"eth_chainId","params":[]},
		{"jsonrpc":"2.0","id":1,"method":"eth_chainId","params":[]},
		{"jsonrpc":"2.0","id":2,"method":"pax_getBlockByNumber","params":[]}
	]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	...
	if errObj == nil || errObj["data"] != "legacy_pax_deprecated" {
		t.Fatalf("slot 3: want legacy gate error, got %+v", batch[3])
	}
}

func TestWrapPaxLegacyHTTP_BatchLeadingNotificationMergedWithBlockedCall(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		var fwd []map[string]any
		if err := json.Unmarshal(b, &fwd); err != nil || len(fwd) != 1 {
	...
	body := `[
		{"jsonrpc":"2.0","method":"eth_chainId","params":[]},
		{"jsonrpc":"2.0","id":7,"method":"pax_getBlockByNumber","params":[]}
	]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestWrapPaxLegacyHTTP_BatchInvalidThenNotificationOneResponse(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner must not run")
	})
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	// Second item is a notification (no id) but gated pax_* — blocked and not forwarded; still no JSON-RPC response for it.
	body := `[42,{"jsonrpc":"2.0","method":"pax_getBlockByNumber","params":[]}]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
...
func TestWrapPaxLegacyHTTP_BatchAllBlockedNotificationsEmptyHTTPBody(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		t.Fatal("inner must not run")
	})
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	body := `[
		{"jsonrpc":"2.0","method":"pax_getBlockByNumber","params":[]},
		{"jsonrpc":"2.0","method":"pax_getPaxAddress","params":["0x0000000000000000000000000000000000000001"]}
	]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestWrapPaxLegacyHTTP_BatchSingleBlockedNotificationEmptyHTTPBody(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
	...
	body := `[{"jsonrpc":"2.0","method":"pax_getBlockByNumber","params":[]}]`
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
...
func TestWrapPaxLegacyHTTP_BatchInnerReorderedByID(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
	...
	h := wrapPaxLegacyHTTP(inner, BuildPaxLegacyEnabledSet(nil))
	body := `[
		{"jsonrpc":"2.0","id":1,"method":"pax_getBlockByNumber","params":[]},
...
func TestWrapPaxLegacyHTTP_BatchMissingInnerResponseForID(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestWrapPaxLegacyHTTP_BatchInnerNotJSONArray(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestMergePaxLegacyHTTPBatch_PatchesMismatchedResponseID(t *testing.T) {
	blocked := []bool{false}
	...
	invalid := []bool{false}
	out := mergePaxLegacyHTTPBatch(invalid, blocked, blockedErr, ids, synthIDs, 1, inner)
	if len(out) != 1 {
...
// TestWrapPaxLegacyHTTP_BatchInvalidIDTypes verifies that batch elements with object or array IDs
...
func TestWrapPaxLegacyHTTP_BatchInvalidIDTypes(t *testing.T) {
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		var fwd []map[string]any
		if err := json.Unmarshal(b, &fwd); err != nil || len(fwd) != 1 {
