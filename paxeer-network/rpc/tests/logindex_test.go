...
func TestGetTransactionReceiptWithMixedLogs(t *testing.T) {
	...
	testerPaxAddress := sdk.AccAddress(mixedLogTesterAddr.Bytes())
	tx0 := signAndEncodeCosmosTx(transferCW20MsgTo(mnemonic1, cw20, testerPaxAddress), mnemonic1, 9, 0)
	...
	SetupTestServer(t, [][][]byte{{tx0}, {txBz}}, mixedLogTesterInitializer(), mnemonicInitializer(mnemonic1), cw20Initializer(mnemonic1, false)).Run(
		func(port int) {
			...
			// the CW transaction in the first block should show up in pax_getTransactionReceipt,
			// with one synthetic log
			res = sendRequestWithNamespace("pax", port, "getTransactionReceipt", cwTxHash.Hex())
			...
			// the EVM transaction in the second block should show up in pax_getTransactionReceipt,
			// with two logs (one synthetic and one non-synthetic)
			res = sendRequestWithNamespace("pax", port, "getTransactionReceipt", signedTx.Hash().Hex())
			logs = res["result"].(map[string]any)["logs"].([]interface{})
			require.Len(t, logs, 2)
			...
			// the first block should have one receipt for pax_getBlockReceipts,
			// with one synthetic log
			res = sendRequestWithNamespace("pax", port, "getBlockReceipts", "0x2")
			receipts = res["result"].([]interface{})
			...
			// the second block should have one receipt for pax_getBlockReceipts,
			// with two logs (one synthetic and one non-synthetic)
			res = sendRequestWithNamespace("pax", port, "getBlockReceipts", "0x3")
			receipts = res["result"].([]interface{})
			require.Len(t, receipts, 1)
			logs = receipts[0].(map[string]any)["logs"].([]interface{})
			require.Len(t, logs, 2)
			...
			// pax_getLogs should return logs for both CW and EVM transaction. The CW
			// tx has one and the EVM tx has two.
			res = sendRequestWithNamespace("pax", port, "getLogs", map[string]any{
				"fromBlock": "0x1",
				"toBlock":   "latest",
			})
			logs = res["result"].([]interface{})
