...
// — we just propagate the right block's params onto the ctx so modules/evm's
...
func setLegacySstoreIfNeeded(ctx sdk.Context, a *app.App, version string) sdk.Context {
	...
	if params.PaxSstoreSetGasEip2200 == legacySstoreGas {
		return ctx
	}
	params.PaxSstoreSetGasEip2200 = legacySstoreGas
