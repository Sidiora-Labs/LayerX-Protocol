...
func TestGetTransactionByBlockNumberAndIndex(t *testing.T) {
	...
	SetupTestServer(t, [][][]byte{{tx1, tx2, tx3}}, mnemonicInitializer(mnemonic1), cw20Initializer(mnemonic1, true)).Run(
		func(port int) {
			...
			// if pax_, the first tx should be tx2 and the second tx should be tx3. tx1
			// is excluded because we don't support pax2_getTransaction*.
			// The first tx cannot be represented as RPCTransaction because it's not an EVM transaction.
			res = sendRequestWithNamespace("pax", port, "getTransactionByBlockNumberAndIndex", "0x2", "0x0")
			require.Contains(t, res["error"].(map[string]any)["message"].(string), "transaction is not an EVM transaction")
			res = sendRequestWithNamespace("pax", port, "getTransactionByBlockNumberAndIndex", "0x2", "0x1")
...
func TestGetTransactionByHash(t *testing.T) {
	...
	SetupTestServer(t, [][][]byte{{tx1, tx2, tx3}}, mnemonicInitializer(mnemonic1), cw20Initializer(mnemonic1, true)).Run(
		func(port int) {
			...
			// is excluded because we don't support pax2_getTransaction*.
			// The first tx cannot be represented as RPCTransaction because it's not an EVM transaction.
			res = sendRequestWithNamespace("pax", port, "getTransactionByHash", signedTx3.Hash().Hex())
			require.Equal(t, "0x1", res["result"].(map[string]any)["transactionIndex"].(string))
			require.Equal(t, signedTx3.Hash().Hex(), res["result"].(map[string]any)["hash"].(string))
			res = sendRequestWithNamespace("pax", port, "getTransactionByHash", common.Hash(sha256.Sum256(tx1)).Hex())
			require.Nil(t, res["result"])
			res = sendRequestWithNamespace("pax", port, "getTransactionByHash", common.Hash(sha256.Sum256(tx2)).Hex())
			require.Contains(t, res["error"].(map[string]any)["message"].(string), "transaction is not an EVM transaction")
		},
	)
}
