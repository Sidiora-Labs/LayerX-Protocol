...
func NewPaxDebugAPI(
	...
	embeddedDebugAPI := &DebugAPI{
		...
		profiledBlockTrace: debugCfg.EnableParallelizedBlockTrace,
		backend:            backend,
		// isPanicCache: nil, // Explicitly nil as per original structure for PaxDebugAPI's embedded DebugAPI
	}
...
func (api *PaxDebugAPI) TraceBlockByNumberExcludeTraceFail(ctx context.Context, number rpc.BlockNumber, config *tracers.TraceConfig) (result interface{}, returnErr error) {
	startTime := time.Now()
	defer func() {
		recordMetricsWithError(ctx, "pax_traceBlockByNumberExcludeTraceFail", api.connectionType, startTime, returnErr, recover())
	}()
...
func (api *PaxDebugAPI) TraceBlockByHashExcludeTraceFail(ctx context.Context, hash common.Hash, config *tracers.TraceConfig) (result interface{}, returnErr error) {
	startTime := time.Now()
	defer func() {
		recordMetricsWithError(ctx, "pax_traceBlockByHashExcludeTraceFail", api.connectionType, startTime, returnErr, recover())
	}()
...
// or meaningless.
//
// Per rpc/README.md ("Tracing Failure Management Endpoints"), the target
...
// (modules/evm/keeper/receipt.go) covers executed txs and sets EffectiveGasPrice
...
// (modules/evm/keeper/abci.go) writes EffectiveGasPrice=0 and GasUsed=0 for any
...
func (api *DebugAPI) GetRawHeader(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (_ hexutil.Bytes, returnErr error) {
	...
	return nil, &ErrEVMNotSupported{Msg: "debug_getRawHeader is not supported on Pax EVM RPC"}
}
...
func (api *DebugAPI) GetRawBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (_ hexutil.Bytes, returnErr error) {
	...
	return nil, &ErrEVMNotSupported{Msg: "debug_getRawBlock is not supported on Pax EVM RPC"}
}
...
func (api *DebugAPI) GetRawReceipts(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (_ []hexutil.Bytes, returnErr error) {
	...
	return nil, &ErrEVMNotSupported{Msg: "debug_getRawReceipts is not supported on Pax EVM RPC"}
}
...
func (api *DebugAPI) GetRawTransaction(ctx context.Context, hash common.Hash) (_ hexutil.Bytes, returnErr error) {
	...
	return nil, &ErrEVMNotSupported{Msg: "debug_getRawTransaction is not supported on Pax EVM RPC"}
}
