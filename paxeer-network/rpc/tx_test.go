...
func TestSign(t *testing.T) {
	homeDir := t.TempDir()
	txApi := evmrpc.NewTransactionAPI(nil, nil, nil, nil, homeDir, evmrpc.ConnectionTypeHTTP, utils.None[time.Duration](), &evmrpc.WatermarkManager{}, evmrpc.NewBlockCache(3000), &sync.Mutex{}, true)
	infoApi := evmrpc.NewInfoAPI(nil, nil, nil, nil, homeDir, 1024, evmrpc.ConnectionTypeHTTP, nil, nil, true)
...
// Pins the discriminator used by pax_getTransactionReceiptExcludeTraceFail.
// Per rpc/README.md, the endpoint should filter out txs that were
...
func TestGetTransactionReceiptExcludeTraceFail(t *testing.T) {
	call := func(hash string) map[string]interface{} {
		body := fmt.Sprintf("{\"jsonrpc\": \"2.0\",\"method\": \"pax_getTransactionReceiptExcludeTraceFail\",\"params\":[\"%s\"],\"id\":\"test\"}", hash)
		req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("http://%s:%d", TestAddr, TestPort), strings.NewReader(body))
		require.Nil(t, err)
		req.Header.Set("Content-Type", "application/json")
		res, err := http.DefaultClient.Do(req)
...
func TestGetTransactionReceiptExcludeTraceFailLateReceipt(t *testing.T) {
	...
	call := func() map[string]interface{} {
		body := fmt.Sprintf("{\"jsonrpc\": \"2.0\",\"method\": \"pax_getTransactionReceiptExcludeTraceFail\",\"params\":[\"%s\"],\"id\":\"test\"}", hash.Hex())
		req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("http://%s:%d", TestAddr, TestPort), strings.NewReader(body))
		require.Nil(t, err)
		req.Header.Set("Content-Type", "application/json")
		res, err := http.DefaultClient.Do(req)
