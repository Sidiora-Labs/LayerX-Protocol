...
paxd_wrapper_dir=$(mktemp -d)
cleanup_wrapper() {
    rm -rf "$paxd_wrapper_dir"
}
trap cleanup_wrapper EXIT

cat > "$paxd_wrapper_dir/paxd" <<'EOF'
#!/bin/bash
target_paxd="$HOME/go/bin/paxd"
cmd="$1"
if [ "$cmd" = "gentx" ]; then
    shift
    has_ip=false
    ...
    exec "$target_paxd" gentx "$@"
else
    shift
    exec "$target_paxd" "$cmd" "$@"
fi
EOF
chmod +x "$paxd_wrapper_dir/paxd"
export PATH="$paxd_wrapper_dir:$PATH"
