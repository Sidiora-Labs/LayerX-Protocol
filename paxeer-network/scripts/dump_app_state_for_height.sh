...
do
    $HOME/go/bin/iaviewer data $HOME/.pax/data/application.db "s/k:"$key"/" $HEIGHT > $HOME/state_$HEIGHT/$key.data
    $HOME/go/bin/iaviewer shape $HOME/.pax/data/application.db "s/k:"$key"/" $HEIGHT > $HOME/state_$HEIGHT/$key.shape
done
