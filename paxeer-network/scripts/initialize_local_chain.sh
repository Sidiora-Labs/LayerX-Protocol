...
~/go/bin/paxd init demo --chain-id pax-chain --overwrite
~/go/bin/paxd keys add $keyname --keyring-backend test
# add the key as a genesis account with massive balances of several different tokens
~/go/bin/paxd add-genesis-account $(~/go/bin/paxd keys show $keyname -a --keyring-backend test) 100000000000000000000uhpx,100000000000000000000uusdc,100000000000000000000uatom --keyring-backend test
# gentx for account
~/go/bin/paxd gentx $keyname 7000000000000000uhpx --chain-id pax-chain --keyring-backend test
...
cat ~/.pax/config/genesis.json | jq '.consensus_params["block"]["max_gas"]="35000000"' > ~/.pax/config/tmp_genesis.json && mv ~/.pax/config/tmp_genesis.json ~/.pax/config/genesis.json
...
cat ~/.pax/config/genesis.json | jq '.consensus_params["block"]["max_gas_wanted"]="50000000"' > ~/.pax/config/tmp_genesis.json && mv ~/.pax/config/tmp_genesis.json ~/.pax/config/genesis.json
...
~/go/bin/paxd config keyring-backend test
...
GORACE="log_path=/tmp/race/paxd_race" ~/go/bin/paxd start --trace --chain-id pax-chain
