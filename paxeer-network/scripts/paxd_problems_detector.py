...
def find_default_pax_home() -> Optional[Path]:
    for c in [Path.home()/".pax", Path("/root/.pax")]:
        if c.is_dir():
            return c
    return None
...
def main():
    failures = 0
    print(head("Paxd Problem Detector"))
    ...
    default_home = find_default_pax_home()
    if not default_home:
        print(style_fail("Could not find '.pax' directory in current user's home or in /root.")); failures += 1
    else:
        print(style_ok(f"Found '.pax' directory at: {default_home}"))
        mount_src, _ = findmnt_source_and_fstype(default_home)
        if not mount_src or not mount_src.startswith("/dev/"):
            print(style_fail(".pax directory is not on a block device.")); failures += 1
        else:
            block = root_block_for(mount_src)
            info = assess_storage(block)
            if not info.startswith("class=nvme"):
                print(style_fail(f".pax directory must be on local NVMe storage. {info}")); failures += 1
            else:
                print(style_ok(f".pax directory is on local NVMe storage"))
