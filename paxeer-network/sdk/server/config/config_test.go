...
func TestSetAndGetMinGasPrices(t *testing.T) {
	...
	cfg.MinGasPrices = "0.025uhpx;0.001uatom"
	parsed := cfg.GetMinGasPrices()
	require.Equal(t, 2, len(parsed))
	require.Equal(t, "uhpx", parsed[0].Denom)
...
func TestGetConfigStateCommit(t *testing.T) {
	...
	require.Equal(t, paxdbconfig.TestOnlyDualWrite, cfg.StateCommit.WriteMode)
...
func TestGetConfigEmptyWriteModeUsesDefault(t *testing.T) {
	...
	cfg, err := GetConfig(v)
	require.NoError(t, err)
	require.Equal(t, paxdbconfig.MemiavlOnly, cfg.StateCommit.WriteMode,
...
func TestDefaultStateStoreConfig(t *testing.T) {
	...
	require.Equal(t, paxdbconfig.DefaultSSAsyncBuffer, cfg.StateStore.AsyncWriteBuffer)
	require.Equal(t, paxdbconfig.DefaultSSKeepRecent, cfg.StateStore.KeepRecent)
	require.Equal(t, paxdbconfig.DefaultSSPruneInterval, cfg.StateStore.PruneIntervalSeconds)
	require.Equal(t, paxdbconfig.DefaultSSImportWorkers, cfg.StateStore.ImportNumWorkers)
