...
var (
	...
	logger           = paxlog.NewLogger("cosmos", "server")
)
...
func InterceptConfigsPreRunHandler(cmd *cobra.Command, customAppConfigTemplate string, customAppConfig any) error {
	...
	if logLvlFormat != "" {
		// For logging efficiency, paxlog takes the log format at the time of
		// initialization and intentionally does not provide the ability to dynamically
		// alter log format at runtime. This means paxlog can operate with
		// zero-allocations during logging operation.
		logger.Warn("log_format flag is deprecated, please use PAX_LOG_FORMAT and PAX_LOG_OUTPUT environment variables instead.")
	}
	...
	// If not, check if env var is set in which case it is already picked up by paxlog; nothing to do.
