...
func (cmgr *CommitKVStoreCacheManager) Reset() {
	for _, ckv := range cmgr.caches {
		// Keep the CommitKVStoreCache in the manager so Unwrap does not return nil.
...
// If the value doesn't exist in the cache, the query is delegated to the
...
// Reset purges all cached values.
