...
type Store struct {
	...
	changeSet paxdbproto.ChangeSet
}
...
func (st *Store) Set(key, value []byte) {
	st.changeSet.Pairs = append(st.changeSet.Pairs, &paxdbproto.KVPair{
		Key: key, Value: value,
	})
}
...
func (st *Store) PopChangeSet() paxdbproto.ChangeSet {
	cs := st.changeSet
	st.changeSet = paxdbproto.ChangeSet{}
	return cs
}
...
func (st *Store) GetChangedPairs(prefix []byte) (res []*paxdbproto.KVPair) {
