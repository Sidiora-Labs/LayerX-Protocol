...
import (
	...
	scmemiavl "github.com/sidiora-labs/paxeer-network/storage/state_db/sc/memiavl"
...
func withTestMemIAVL(cfg paxdbconfig.StateCommitConfig) paxdbconfig.StateCommitConfig {
...
func dualWriteConfig() paxdbconfig.StateCommitConfig {
	cfg := paxdbconfig.DefaultStateCommitConfig()
	cfg.WriteMode = paxdbconfig.TestOnlyDualWrite
	return withTestMemIAVL(cfg)
}

func evmMigratedConfig() paxdbconfig.StateCommitConfig {
	cfg := paxdbconfig.DefaultStateCommitConfig()
	cfg.WriteMode = paxdbconfig.EVMMigrated
	return withTestMemIAVL(cfg)
}

func flatKVOnlyConfig() paxdbconfig.StateCommitConfig {
	cfg := paxdbconfig.DefaultStateCommitConfig()
	cfg.WriteMode = paxdbconfig.FlatKVOnly
	return withTestMemIAVL(cfg)
}
...
func memiavlOnlyConfig() paxdbconfig.StateCommitConfig {
	cfg := paxdbconfig.DefaultStateCommitConfig()
	cfg.WriteMode = paxdbconfig.MemiavlOnly
	return withTestMemIAVL(cfg)
}
...
// blocks.
func migrateEVMConfig(keysPerBlock int) paxdbconfig.StateCommitConfig {
	cfg := paxdbconfig.DefaultStateCommitConfig()
	cfg.WriteMode = paxdbconfig.MigrateEVM
...
func restartRootMultiWithConfig(
	t *testing.T, store *Store, dir string, newCfg paxdbconfig.StateCommitConfig,
...
func newTestRootMulti(t *testing.T, dir string, scCfg paxdbconfig.StateCommitConfig) (*Store, map[string]*types.KVStoreKey) {
	t.Helper()
	return newTestRootMultiWithSS(t, dir, scCfg, paxdbconfig.StateStoreConfig{})
}
...
func newTestRootMultiWithSS(
	...
	scCfg paxdbconfig.StateCommitConfig,
	ssCfg paxdbconfig.StateStoreConfig,
) (*Store, map[string]*types.KVStoreKey) {
	t.Helper()
...
// behind cosmos.
func rollbackFlatKV(t *testing.T, dir string, cfg paxdbconfig.StateCommitConfig, target int64) {
	t.Helper()
...
// FlatKV.
func rollbackMemiavl(t *testing.T, dir string, cfg paxdbconfig.StateCommitConfig, target int64) {
	t.Helper()
...
func openFlatKVReadOnly(t *testing.T, dir string, cfg paxdbconfig.StateCommitConfig, version int64) flatkv.Store {
	t.Helper()
	flatkvCfg := cfg.FlatKVConfig
	flatkvCfg.DataDir = utils.GetFlatKVPath(dir)
...
func verifyFlatKVSelfConsistent(t *testing.T, dir string, cfg paxdbconfig.StateCommitConfig) {
	t.Helper()
...
// mirrors the conversion logic in convertFlatKVNodes (storage/state_db/ss/
...
// memiavl child store.
func collectFlatKVEVM(t *testing.T, dir string, cfg paxdbconfig.StateCommitConfig, version int64) map[string][]byte {
	t.Helper()
	flatkvCfg := cfg.FlatKVConfig
	flatkvCfg.DataDir = utils.GetFlatKVPath(dir)
