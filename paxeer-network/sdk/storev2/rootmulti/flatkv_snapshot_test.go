...
func TestFlatKVOnlySnapshotRestorePopulatesSS(t *testing.T) {
	cfg := flatKVOnlyConfig()
	ssCfg := paxdbconfig.DefaultStateStoreConfig()
	ssCfg.Enable = true
...
func simulateFlatKVOnlyBlock(
	...
	bankKV.Set([]byte("denom/pax"), []byte{0x53, b})

	if block%3 == 0 {
		accKV.Delete([]byte("acct-overwrite"))
	}
	if block%4 == 0 {
		bankKV.Delete([]byte("denom/pax"))
	}
	if block%5 == 0 {
...
// storage/state_db/sc/memiavl/store_test.go and storage/state_db/sc/flatkv/
