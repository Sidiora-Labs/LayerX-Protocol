...
var (
	logger = paxlog.NewLogger("cosmos", "storev2", "rootmulti")
...
func (rs *Store) GetStateStore() paxdbtypes.StateStore {
	return rs.ssStore
}
...
func (rs *Store) restore(height int64, protoReader protoio.Reader) (snapshottypes.SnapshotItem, error) {
	var (
		ssImporter   chan paxdbtypes.SnapshotNode
	...
	if rs.ssStore != nil {
		ssImporter = make(chan paxdbtypes.SnapshotNode, 10000)
		go func() {
	...
	for {
		...
		case *snapshottypes.SnapshotItem_IAVL:
			...
			if rs.ssStore != nil && node.Height == 0 && ssImporter != nil {
				ssImporter <- paxdbtypes.SnapshotNode{
					StoreKey: storeKey,
					Key:      node.Key,
					Value:    node.Value,
				}
