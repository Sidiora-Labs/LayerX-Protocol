...
func (suite *AnteTestSuite) TestAnteHandlerFees() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				err := apptesting.FundAccount(suite.app.BankKeeper, suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("uhpx", 149)))
				suite.Require().NoError(err)
			},
			false,
			false,
		...
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("uhpx"), sdk.NewInt(149)))

				err := apptesting.FundAccount(suite.app.BankKeeper, suite.ctx, addr0, sdk.NewCoins(sdk.NewInt64Coin("uhpx", 1)))
				suite.Require().NoError(err)
			},
			false,
			true,
		...
		},
		{
			...
			func() {
				...
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, modAcc.GetAddress()).AmountOf("uhpx"), sdk.NewInt(150)))
				require.True(sdk.IntEq(suite.T(), suite.app.BankKeeper.GetAllBalances(suite.ctx, addr0).AmountOf("uhpx"), sdk.NewInt(0)))
			},
			false,
			false,
			sdkerrors.ErrInsufficientFunds,
	...
	testCases := []TestCase{
		{
			...
			func() {
				feeAmount = sdk.NewCoins(sdk.NewInt64Coin("uhpx", 0))
				gasLimit = 0
			},
			false,
			false,
...
func (suite *AnteTestSuite) TestAnteHandlerReCheck() {
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("uhpx", sdk.NewDecWithPrec(1, 5)))
