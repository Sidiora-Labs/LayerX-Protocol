...
import (
	...
	vesting "github.com/sidiora-labs/paxeer-network/sdk/x/auth/vesting/types"
...
func (suite *IntegrationTestSuite) TestSendCoinsAndWei() {
	...
	require.Equal(sdk.NewInt(100), keeper.GetBalance(ctx, addr1, sdk.DefaultBondDenom).Amount)
	require.Equal(sdk.ZeroInt(), keeper.GetBalance(ctx, addr2, sdk.DefaultBondDenom).Amount)
	// should just do uhpx send if wei is zero
	...
	// sender gets escrowed one uhpx, recipient does not get redeemed
...
func (suite *IntegrationTestSuite) TestSpendableCoins() {
	...
	origCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 100))
	delCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 50))
...
func (suite *IntegrationTestSuite) TestVestingAccountSend() {
	...
	origCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 100))
	sendCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 50))
...
func (suite *IntegrationTestSuite) TestPeriodicVestingAccountSend() {
	...
	periods := vesting.Periods{
		vesting.Period{Length: int64(12 * 60 * 60), Amount: sdk.Coins{sdk.NewInt64Coin("uhpx", 50)}},
		vesting.Period{Length: int64(6 * 60 * 60), Amount: sdk.Coins{sdk.NewInt64Coin("uhpx", 25)}},
		vesting.Period{Length: int64(6 * 60 * 60), Amount: sdk.Coins{sdk.NewInt64Coin("uhpx", 25)}},
	}
...
func (suite *IntegrationTestSuite) TestDelegateCoinsFromAccountToModule() {
	...
	origCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 100))
	delCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 50))
	undelCoins := sdk.NewCoins(sdk.NewInt64Coin("uhpx", 20))
...
func (suite *IntegrationTestSuite) TestCanSendTo() {
	...
	suite.Require().NoError(apptesting.FundAccount(app.BankKeeper, ctx, sourceAddr, sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(100)))))
