...
func (k BaseSendKeeper) SubWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if amt.LTE(currentWeiBalance) {
		// no need to change uhpx balance
		return k.setWeiBalance(ctx, addr, currentWeiBalance.Sub(amt))
	}
	currentUhpxBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	currentAggregatedBalance := currentUhpxBalance.Mul(OneUhpxInWei).Add(currentWeiBalance)
	...
	uhpxBalance, weiBalance := SplitUhpxWeiAmount(postAggregatedbalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), uhpxBalance), true); err != nil {
		return err
	}
...
func (k BaseSendKeeper) AddWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if postWeiBalance.LT(OneUhpxInWei) {
		// no need to change uhpx balance
		return k.setWeiBalance(ctx, addr, postWeiBalance)
	}
	currentUhpxBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	uhpxCredit, weiBalance := SplitUhpxWeiAmount(postWeiBalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), currentUhpxBalance.Add(uhpxCredit)), true); err != nil {
		return err
	}
	return k.setWeiBalance(ctx, addr, weiBalance)
}
...
func SplitUhpxWeiAmount(amt sdk.Int) (sdk.Int, sdk.Int) {
	return amt.Quo(OneUhpxInWei), amt.Mod(OneUhpxInWei)
}
