...
type GenTxTestSuite struct {
	...
	ctx            sdk.Context
	app            *paxapp.App
	encodingConfig paxappparams.EncodingConfig
...
func (suite *GenTxTestSuite) SetupTest() {
	checkTx := false
	app := paxapp.Setup(suite.T(), checkTx, false, false)
	suite.ctx = app.BaseApp.NewContext(checkTx, tmproto.Header{ChainID: app.ChainID})
	suite.app = app
	suite.encodingConfig = paxapp.MakeEncodingConfig()
