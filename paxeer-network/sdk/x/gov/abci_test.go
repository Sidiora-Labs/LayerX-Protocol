...
func TestTickExpiredDepositPeriod(t *testing.T) {
	...
	addrs := paxapp.AddTestAddrs(app, ctx, 10, valTokens)
		...
		t.Run(tc.name, func(t *testing.T) {
			for _, tc := range testcases {
				t.Run(tc.name, func(t *testing.T) {
					...
					app := paxapp.Setup(t, false, false, false)
					ctx := app.BaseApp.NewContext(false, tmproto.Header{})
					addrs := paxapp.AddTestAddrs(app, ctx, 10, valTokens.Mul(sdk.NewInt(depositMultiplier)))
...
func TestEndBlockerProposalHandlerFailed(t *testing.T) {
	app := paxapp.Setup(t, false, false, false)
	ctx := app.BaseApp.NewContext(false, tmproto.Header{})
	addrs := paxapp.AddTestAddrs(app, ctx, 1, valTokens)
	params := app.StakingKeeper.GetParams(ctx)
	params.MinCommissionRate = sdk.NewDec(0)
	app.StakingKeeper.SetParams(ctx, params)
