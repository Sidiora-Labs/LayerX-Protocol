...
func TestUnJailNotBonded(t *testing.T) {
	...
	pks := paxapp.CreateTestPubKeys(6)
...
func TestHandleNewValidator(t *testing.T) {
	...
	addrDels := paxapp.AddTestAddrsIncremental(app, ctx, 1, app.StakingKeeper.TokensFromConsensusPower(ctx, 200))
	valAddrs := paxapp.ConvertAddrsToValAddrs(addrDels)
	pks := paxapp.CreateTestPubKeys(1)
...
func TestValidatorDippingInAndOut(t *testing.T) {
	...
	pks := paxapp.CreateTestPubKeys(3)
	paxapp.AddTestAddrsFromPubKeys(app, ctx, pks, app.StakingKeeper.TokensFromConsensusPower(ctx, 200))
