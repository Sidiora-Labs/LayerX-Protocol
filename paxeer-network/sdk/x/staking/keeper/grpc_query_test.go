...
func createValidators(t *testing.T, ctx sdk.Context, app *paxapp.App, powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []types.Validator) {
	addrs := paxapp.AddTestAddrsIncremental(app, ctx, 5, app.StakingKeeper.TokensFromConsensusPower(ctx, 300))
	valAddrs := paxapp.ConvertAddrsToValAddrs(addrs)
