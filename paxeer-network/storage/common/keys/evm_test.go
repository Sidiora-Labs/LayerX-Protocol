...
// Test-local copies of modules/evm/types key prefixes.
// Kept here (rather than importing evmtypes) to avoid a circular dependency:
//
//	common/evm (test) -> modules/evm/types -> cosmos-sdk/server/config
//	-> db/config -> storage/state_db/sc/flatkv -> common/evm
var (
	testEVMAddrToPaxPrefix = []byte{0x01}
	testPaxAddrToEVMPrefix = []byte{0x02}
)
...
func TestParseEVMKey(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "EVMAddressToPaxAddress goes to Legacy",
			key:       concat(testEVMAddrToPaxPrefix, addr),
			wantKind:  EVMKeyLegacy,
			wantBytes: concat(testEVMAddrToPaxPrefix, addr), // Full key preserved
		},
		{
			name:      "PaxAddressToEVMAddress goes to Legacy",
			key:       concat(testPaxAddrToEVMPrefix, addr),
			wantKind:  EVMKeyLegacy,
			wantBytes: concat(testPaxAddrToEVMPrefix, addr), // Full key preserved
		},
		{
