...
import (
	...
	commonerrors "github.com/sidiora-labs/paxeer-network/storage/common/errors"
...
var logger = paxlog.NewLogger("db", "state-db", "sc", "flatkv")
...
type CommitStore struct {
	...
	metadataDB paxdbtypes.KeyValueDB // Global version + LtHash watermark
	accountDB  paxdbtypes.KeyValueDB // "evm/"+0x0a+addr(20) → vtype.AccountData
	codeDB     paxdbtypes.KeyValueDB // "evm/"+0x07+addr(20) → vtype.CodeData
	storageDB  paxdbtypes.KeyValueDB // "evm/"+0x03+addr(20)||slot(32) → vtype.StorageData
	legacyDB   paxdbtypes.KeyValueDB // "module/"+key → vtype.LegacyData
...
func (s *CommitStore) dataDBs() []paxdbtypes.KeyValueDB {
	return []paxdbtypes.KeyValueDB{s.accountDB, s.codeDB, s.storageDB, s.legacyDB}
}

type namedDB struct {
	dir string
	db  paxdbtypes.KeyValueDB
}
...
func (s *CommitStore) routePhysicalKey(physicalKey []byte) (paxdbtypes.KeyValueDB, error) {
...
func NewCommitStore(
	...
	return &CommitStore{
		...
		phaseTimer:         metrics.NewPhaseTimer(flatkvMeter, "paxdb_main_thread"),
		readPool:           readPool,
		miscPool:           miscPool,
	}, nil
}
...
func (s *CommitStore) openPebbleDB(cfg *pebbledb.PebbleDBConfig, cacheCfg *dbcache.CacheConfig) (paxdbtypes.KeyValueDB, error) {
