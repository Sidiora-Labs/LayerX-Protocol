...
func (cs *CommitStore) LoadVersion(targetVersion int64, readOnly bool) (types.Committer, error) {
	logger.Info("PaxDB loading target memIAVL version", "version", targetVersion, "read-only", readOnly)
...
// wal.Write returns before the entry is durable (see db/wal/wal.go,
