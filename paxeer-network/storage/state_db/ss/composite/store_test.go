	...
	changesets := []*proto.NamedChangeSet{
		{
			...
			Changeset: proto.ChangeSet{
				Pairs: []*proto.KVPair{
					...
					{Key: legacyKey, Value: []byte("pax1abc")},
				},
			},
		},
	}
...
func TestE2E_AllEVMDBsReadableViaComposite(t *testing.T) {
	...
	tests := []struct {
		...
		{"Legacy (EVMToPaxAddr)", append([]byte{0x01}, addr...), []byte("pax1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5lzv7xu")},
	}
...
func TestImport_FlatKVLegacyKeysPreserveModule(t *testing.T) {
	...
	evmLegacyVal := vtype.NewLegacyData().SetValue([]byte("pax1abc")).Serialize()

	bankInnerKey := []byte("balances/addr1")
	bankPhysKey := ktype.ModulePhysicalKey("bank", bankInnerKey)
	bankLegacyVal := vtype.NewLegacyData().SetValue([]byte("1000uhpx")).Serialize()
	...
	for _, mode := range []bool{true, false} {
		t.Run(fmt.Sprintf("EVMSplit=%v", mode), func(t *testing.T) {
			...
			if store.evmStore != nil && mode {
				...
				require.NoError(t, err)
				require.Equal(t, []byte("pax1abc"), evmVal, "evm legacy key should land in EVM store")
			}

			bankVal, err := store.cosmosStore.Get("bank", 1, bankInnerKey)
			require.NoError(t, err)
			require.Equal(t, []byte("1000uhpx"), bankVal, "bank legacy key should land in cosmos under 'bank' module")
