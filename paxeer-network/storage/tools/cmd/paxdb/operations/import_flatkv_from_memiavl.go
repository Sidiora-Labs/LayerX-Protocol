...
run unless --force is supplied.`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resolvedHome, err := resolvePaxHome(homeDir, dataDir)
			if err != nil {
				return err
			}
	...
	cmd.Flags().String("home", "", "Pax home directory. Defaults to $HOME/.pax")
	cmd.Flags().String("data-dir", "", "Pax data directory or home directory. If the basename is data, its parent is used as home")
...
func resolvePaxHome(homeDir, dataDir string) (string, error) {
	if homeDir != "" {
	...
	return filepath.Join(home, ".pax"), nil
}
...
func importMemiavlModulesToFlatKV(ctx context.Context, homeDir string, modules []string, height int64, force bool) (err error) {
	...
	// storage/state_db/sc/composite/store.go) and silently roll memiavl
...
// shares resolvePaxHome with the import command and exists solely to support
