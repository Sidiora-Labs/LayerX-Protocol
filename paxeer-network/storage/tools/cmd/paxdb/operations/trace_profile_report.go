...
func runTraceProfileReport(endpoint string, startBlock, endBlock int64, outputDir string, concurrency, maxTransactions int, traceConfig map[string]interface{}) error {
	...
	rawFile, err := os.OpenFile(rawPath, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o600) //nolint:gosec // outputDir is an operator-supplied CLI flag for this offline paxdb tool
	if err != nil {
		return err
	}
