...
func TestPubkeyBytesToPaxPubKey(t *testing.T) {
	tests := []struct {
		name   string
		pubkey []byte
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			paxPubkey := PubkeyBytesToPaxPubKey(tt.pubkey)

			// Verify the key is properly converted
			require.NotNil(t, paxPubkey.Key)
			require.True(t, len(paxPubkey.Key) == 33) // Compressed key should be 33 bytes

			// Verify we can parse it back
			parsedKey, err := btcec.ParsePubKey(tt.pubkey)
			require.NoError(t, err)

			expectedPaxPubkey := secp256k1.PubKey{Key: parsedKey.SerializeCompressed()}
			require.Equal(t, expectedPaxPubkey, paxPubkey)
		})
	}
}
...
func TestGetAddressesFromPubkeyBytes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			evmAddr, paxAddr, paxPubkey, err := GetAddressesFromPubkeyBytes(tt.pubkey)

			if tt.expectError {
				require.Error(t, err)
				require.Contains(t, err.Error(), tt.errorMsg)
				require.Equal(t, common.Address{}, evmAddr)
				require.Equal(t, sdk.AccAddress{}, paxAddr)
				require.Nil(t, paxPubkey)
			} else {
				require.NoError(t, err)
				require.NotEqual(t, common.Address{}, evmAddr)
				require.NotEqual(t, sdk.AccAddress{}, paxAddr)
				...
				expectedPaxPubkey := PubkeyBytesToPaxPubKey(tt.pubkey)
				require.Equal(t, &expectedPaxPubkey, paxPubkey)
				require.Equal(t, sdk.AccAddress(expectedPaxPubkey.Address()), paxAddr)
			}
		})
	}
}
...
func TestGetAddresses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			evmAddr, paxAddr, paxPubkey, err := GetAddresses(tt.v, tt.r, tt.s, tt.data)
...
func TestRecoverAddressesFromTx(t *testing.T) {
	chainID := big.NewInt(713715) // Pax chain ID
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			evmAddr, paxAddr, pubkey, err := RecoverAddressesFromTx(signedTx, signer, chainID)
			require.NoError(t, err)
			...
			// Verify Pax address is derived correctly from pubkey
			require.NotNil(t, pubkey)
			expectedPaxAddr := sdk.AccAddress(pubkey.Address())
			require.Equal(t, expectedPaxAddr, paxAddr, "Pax address should be derived from pubkey")
...
func TestRecoverAddressesFromTx_MatchesPreprocessLogic(t *testing.T) {
	...
	evmAddr1, paxAddr1, pubkey1, err := RecoverAddressesFromTx(signedTx, signer, chainID)
	require.NoError(t, err)
	...
	evmAddr2, paxAddr2, pubkey2, err := GetAddresses(adjustedV, R, S, txHash)
	require.NoError(t, err)

	// Verify both methods produce the same results
	require.Equal(t, evmAddr1, evmAddr2, "EVM addresses should match between methods")
	require.Equal(t, paxAddr1, paxAddr2, "Pax addresses should match between methods")
