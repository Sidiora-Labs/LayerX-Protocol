...
func TestAddressConversions(t *testing.T) {
	...
	t.Run("pubkey conversion to pax pubkey", func(t *testing.T) {
		paxPubkey := PubkeyBytesToPaxPubKey(pubkeyBytes)
		require.NotNil(t, paxPubkey.Key)
		require.Equal(t, 33, len(paxPubkey.Key)) // Compressed key length
	})

	t.Run("addresses from pubkey bytes", func(t *testing.T) {
		derivedEvmAddr, paxAddr, paxPubkey, err := GetAddressesFromPubkeyBytes(pubkeyBytes)
		require.NoError(t, err)
		require.Equal(t, evmAddr, derivedEvmAddr)
		require.NotNil(t, paxAddr)
		require.NotNil(t, paxPubkey)

		// Verify the pax address is derived from the pubkey
		expectedPaxAddr := sdk.AccAddress(paxPubkey.Address())
		require.Equal(t, expectedPaxAddr, paxAddr)
	})
}
...
func TestKeyConversionConsistency(t *testing.T) {
	...
	// Convert to Pax pubkey
	paxPubkey := PubkeyBytesToPaxPubKey(uncompressedPubkey)
	...
	evmAddr3, paxAddr, returnedPaxPubkey, err := GetAddressesFromPubkeyBytes(uncompressedPubkey)
	require.NoError(t, err)

	// All EVM addresses should match
	require.Equal(t, evmAddr1, evmAddr3)

	// The returned pax pubkey should match
	require.Equal(t, paxPubkey, *returnedPaxPubkey.(*secp256k1.PubKey))

	// The pax address should be derived from the pubkey
	expectedPaxAddr := sdk.AccAddress(paxPubkey.Address())
	require.Equal(t, expectedPaxAddr, paxAddr)
}
...
func (m *mockEVMKeeper) SetAddressMapping(_ sdk.Context, paxAddress sdk.AccAddress, evmAddress common.Address) {
	if m.mappings == nil {
		m.mappings = map[string]common.Address{}
	}
	m.mappings[paxAddress.String()] = evmAddress
}
...
func (m *mockBankKeeper) SpendableCoins(sdk.Context, sdk.AccAddress) sdk.Coins {
	return sdk.NewCoins(sdk.NewCoin("uhpx", sdk.NewInt(100)))
}
...
func (m *mockBankKeeper) GetBalance(sdk.Context, sdk.AccAddress, string) sdk.Coin {
	return sdk.NewCoin("uhpx", sdk.NewInt(100))
}
...
func TestAssociateAddressesReusesEmptyCastAccount(t *testing.T) {
	...
	paxAddr := sdk.AccAddress(common.HexToAddress("0x2222222222222222222222222222222222222222").Bytes())
	...
	require.NoError(t, helper.AssociateAddresses(ctx, paxAddr, evmAddr, pubkey, false))

	require.Zero(t, ak.newAccountCalls)
	require.Nil(t, ak.GetAccount(ctx, castAddr))
	require.Equal(t, evmAddr, ek.mappings[paxAddr.String()])
	acc := ak.GetAccount(ctx, paxAddr)
...
func TestAssociateAddressesDoesNotReuseVestingCastAccount(t *testing.T) {
	...
	ak.SetAccount(ctx, vestingtypes.NewDelayedVestingAccount(baseAcc, sdk.NewCoins(sdk.NewInt64Coin("uhpx", 100)), 123, nil))
	bk := &mockBankKeeper{}
	ek := &mockEVMKeeper{}
...
func TestMigrateBalanceSkipsDirectCastAddress(t *testing.T) {
	ctx := sdk.Context{}
	evmAddr := common.HexToAddress("0x1111111111111111111111111111111111111111")
	paxAddr := sdk.AccAddress(evmAddr[:])

	bk := &mockBankKeeper{}
	helper := NewAssociationHelper(&mockEVMKeeper{}, bk, &mockAccountKeeper{})
	require.NoError(t, helper.MigrateBalance(ctx, evmAddr, paxAddr, false))
