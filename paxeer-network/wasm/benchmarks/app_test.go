...
func setup(t *testing.T, db dbm.DB, withGenesis bool, invCheckPeriod uint, opts ...wasm.Option) (*paxapp.App, paxapp.GenesisState) {
	wasmApp := paxapp.Setup(t, false, false, false)
	if withGenesis {
		return wasmApp, paxapp.NewDefaultGenesisState(paxapp.MakeEncodingConfig().Marshaler)
	}
	return wasmApp, paxapp.GenesisState{}
}

// SetupWithGenesisAccounts initializes a new WasmApp with the provided genesis
// accounts and possible balances.
func SetupWithGenesisAccounts(b testing.TB, db dbm.DB, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *paxapp.App {
	wasmApp, genesisState := setup(b.(*testing.T), db, true, 0)
	authGenesis := authtypes.NewGenesisState(authtypes.DefaultParams(), genAccs)
	appCodec := paxapp.MakeEncodingConfig().Marshaler
...
func InitializeWasmApp(b testing.TB, db dbm.DB, numAccounts int) AppInfo {
	...
	storeTx, err := paxapp.GenTx(txGen, []sdk.Msg{&storeMsg}, nil, 55123123, "", []uint64{0}, []uint64{0}, minter)
	require.NoError(b, err)
	...
	initTx, err := paxapp.GenTx(txGen, []sdk.Msg{&initMsg}, nil, gasWanted, "", []uint64{0}, []uint64{1}, minter)
	require.NoError(b, err)
	...
	return AppInfo{
		...
		AccNum:       0,
		SeqNum:       2,
		TxConfig:     paxapp.MakeEncodingConfig().TxConfig,
	}
}
