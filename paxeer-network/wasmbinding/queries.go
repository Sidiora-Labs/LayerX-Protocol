...
func (qp QueryPlugin) HandleOracleQuery(ctx sdk.Context, queryData json.RawMessage) ([]byte, error) {
	var parsedQuery oraclebindings.PaxOracleQuery
	...
	case parsedQuery.OracleTwaps != nil:
		...
		return bz, nil
	default:
		return nil, oracletypes.ErrUnknownPaxOracleQuery
	}
}

func (qp QueryPlugin) HandleEpochQuery(ctx sdk.Context, queryData json.RawMessage) ([]byte, error) {
	var parsedQuery epochbindings.PaxEpochQuery
	if err := json.Unmarshal(queryData, &parsedQuery); err != nil {
		return nil, epochtypes.ErrParsingPaxEpochQuery
	}
	switch {
	case parsedQuery.Epoch != nil:
		...
		return bz, nil
	default:
		return nil, epochtypes.ErrUnknownPaxEpochQuery
	}
}

func (qp QueryPlugin) HandleTokenFactoryQuery(ctx sdk.Context, queryData json.RawMessage) ([]byte, error) {
	var parsedQuery tokenfactorybindings.PaxTokenFactoryQuery
	if err := json.Unmarshal(queryData, &parsedQuery); err != nil {
		return nil, tokenfactorytypes.ErrParsingPaxTokenFactoryQuery
	}
	switch {
	...
	case parsedQuery.DenomsFromCreator != nil:
		...
		return bz, nil
	default:
		return nil, tokenfactorytypes.ErrUnknownPaxTokenFactoryQuery
	}
}
...
func (qp QueryPlugin) HandleEVMQuery(ctx sdk.Context, queryData json.RawMessage) (res []byte, err error) {
	...
	var parsedQuery evmbindings.PaxEVMQuery
	if err := json.Unmarshal(queryData, &parsedQuery); err != nil {
	...
	case evmbindings.GetEvmAddressType:
		...
		return qp.evmHandler.HandleGetEvmAddress(ctx, c.PaxAddress)
	case evmbindings.GetPaxAddressType:
		c := parsedQuery.GetPaxAddress
		return qp.evmHandler.HandleGetPaxAddress(ctx, c.EvmAddress)
