...
func validateProgramCall(call ProgramCall) error {
	if call.Budget.Fuel == 0 || len(call.Calldata) > MaximumProgramCalldataBytes || len(call.Capabilities) > MaximumProgramCapabilities || len(call.SignedActivity) == 0 { return errors.New("invalid program call") }
...
func VerifyProgramReceipt(execution ProgramExecutionDocument, authority AuthorizedBatch) (VerifiedProgramReceipt, error) {
	...
	if verified.Receipt.ActivityID != [32]byte(activity) || verified.Receipt.ModuleID != 9 || verified.Receipt.Operation != 3 || verified.Receipt.ModuleVersion < 1 || verified.Receipt.ModuleVersion > 3 { return VerifiedProgramReceipt{}, errors.New("program receipt binding failed") }
