...
fn candidate(id: u8, tenant: u8, opened: u64, expiry: u64) -> Lease {
    Lease::request(
        ...
        [4; 32], layerx_programs_runtime::hash_bytes(layerx_programs_runtime::HashAlgorithm::Sha256,
            &continuation_module()).unwrap_or_else(|error| panic!("hash: {error}")), 10_000,
...
fn snapshot_destroy_restore_and_continue_preserves_exact_execution_state() {
    ...
    let mut live_key = source.namespace().key_prefix().to_vec();
    live_key.extend_from_slice(b"counter");
    ...
    assert!(source_storage.protocol_prefix_bytes(source.namespace().storage_namespace()
        .unwrap_or_else(|error| panic!("namespace: {error}")), &source.namespace().key_prefix())
    ...
    assert_eq!(source_storage.protocol_prefix_bytes(destroyed.namespace().storage_namespace()
        .unwrap_or_else(|error| panic!("namespace: {error}")), &destroyed.namespace().key_prefix()), Ok(0));
...
fn aggregate_snapshot_chunks_never_cross_the_lease_namespace_ceiling() {
    ...
    assert!(refused);
    assert!(storage.protocol_prefix_bytes(lease.namespace().storage_namespace()
        .unwrap_or_else(|error| panic!("namespace: {error}")), &lease.namespace().key_prefix())
