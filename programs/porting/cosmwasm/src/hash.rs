//! `SHA-256` over published source, build inputs and emitted artifacts. The
//! programs registry hashes deployed code with `SHA-256`, so a ported contract
//! computes its own code hash and pinned-file digests the same way, and the
//! kit derives its message variant tags from the same function.
