//! Deterministic `WebAssembly` module assembly for ported contracts.
//!
...
//! data segments and imports drawn only from the `layerx_v1` module. The same
...
/// The `unreachable` opcode, which is a ported `revert`.
...
impl Code {
    ...
    /// Appends `unreachable`, the ported `revert`: every staged write and
    /// effect of the whole invocation is discarded.
