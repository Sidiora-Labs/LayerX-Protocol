//! `SHA-256`, which Solana uses for every address and discriminator the port
//! has to reproduce: program-derived addresses, Anchor account, instruction
//! and event discriminators. The programs registry also hashes deployed code
//! with `SHA-256`, so the same function computes the code hash.
