...
impl<'parser> FunctionTranslator<'parser> {
    ...
    fn translate_operators(&mut self) -> Result<usize, ModuleError> {
        ...
        while !reader.eof() {
            ...
            let mut instruction_reader = reader.get_binary_reader();
            self.func_builder.update_pos(pos);
            reader.visit_operator(&mut self.func_builder)??;
            let end = reader.original_position();
            let instruction = instruction_reader.read_bytes(end - pos)?;
            self.func_builder.set_last_operator_bytes(instruction);
        }
        reader.ensure_end()?;
