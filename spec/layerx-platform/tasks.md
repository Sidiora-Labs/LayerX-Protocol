...
- [ ] 33. Make programs discoverable, callable and authorable without a compiler
  ...
  - [x] 33.2 Freeze the calldata encoding convention
    ...
    - _Requirements: 40.3_
  - [ ] 33.3 Generate typed bindings from a published interface — **Implemented - qualification pending**
...
- [ ] 34. Rent a sandbox through a program, pay only for the compute used, and have it destroyed
  ...
  - [ ] 34.1 Model the sandbox lease as protocol state — **Implemented - qualification pending**
...
- [ ] 35. Settle rented compute the platform did not execute, without trusting the provider
  ...
  - [ ] 35.1 Commit to execution state per step — **Implemented - qualification pending**
...
- [ ] 37. Consolidate the Sidiora Labs ecosystem in one bounded monorepo
  - [x] 37.1 Integrate Paxeer Network and its automation into the LayerX monorepo
...
## Audit Repair Wave - Production Boundaries
...
- [ ] 38. Close the source-level blockers before human qualification
  ...
  - [ ] 38.3 Ship the privileged human component server
    ...
    - Fail closed on unavailable dependencies, malformed component frames, stale evidence and unknown operations without an in-memory fallback.
    - Ship a production binary and configuration surface plus real-boundary cases, leaving execution to human qualification.
    - _Requirements: 46.3, 1.1, 1.10_
  - [ ] 38.4 Ship the production LXKP KMS gateway
    - Implement the server side of the bounded LXKP protocol over mutual TLS for probe, create, describe, rotate, destroy and sign operations.
    - Back the gateway only with an explicitly configured non-exportable hardware or remote KMS provider and reject development envelope providers in production.
    - Authenticate the client identity, bind every operation to the principal key binding and verify every returned signature before responding.
  ...
  - [x] 38.5 Authenticate and isolate the hosted program build registry
    ...
    - Keep source verification reproducible and offline while preventing published archive executables from becoming the command authority.
    - Update gateway and registry secret-file configuration together and record deployment isolation assumptions for qualification.
    - _Requirements: 46.5, 29.5, 32.9_
  - [ ] 38.6 Restore canonical Programs receipt parity across all SDKs
    - Model the optional canonical Programs outcome in the shared SDK generator and emit its strict decoder in all seven languages.
    - Mirror the protocol decoder's required non-zero sequence, module, version, timestamp, activity and resulting-root invariants.
    - Preserve canonical unsigned-receipt reconstruction and signature verification when the optional outcome is present.
    - Add shared golden receipts with and without Programs outcomes and language-specific refusal vectors without running them during implementation.
    - Expose one identical typed receipt failure taxonomy across every generated verifier.
    - _Requirements: 46.6, 24.2, 30.6_
  - [ ] 38.7 Restore complete SDK generation-lock coverage
    - Classify every file under a generated output root as generated or explicitly handwritten and make the drift gate cover that classification.
    - Bring LocalVerifier.java, ConformanceMain.java and run-jvm.sh back into byte-for-byte agreement with the declared pipeline lock.
    - Reject untracked files inside explicit-list output roots unless the pipeline declares them handwritten.
  ...
  - [ ] 38.10 Align Programs event metering and capability transport bounds — **Implemented - qualification pending**
    ...
    - Keep capability count and encoded-byte ceilings mutually satisfiable for every constructible canonical capability set.
    - Write boundary and exhaustion cases for event count, event bytes and maximum capability sets without running qualification.
    - _Requirements: 46.10, 28.3, 30.1_
...
```json
{
  "waves": [
    ...
    { "id": 19, "tasks": ["33.1", "33.2", "33.3", "33.4", "33.5", "33.6", "33.7", "38.1", "38.2", "38.3", "38.5", "38.8", "38.9", "38.10"] },
    { "id": 20, "tasks": ["34.1", "34.2", "34.3", "34.4", "34.5", "34.6", "38.4"] },
    { "id": 21, "tasks": ["35.1", "35.2", "35.3", "35.4", "35.5", "35.6", "35.7"] },
    { "id": 22, "tasks": ["38.6", "38.7"] },
