...
enum { DESTROY_BYTES = 84, TERMINAL_BYTES = 352, CLEANUP_BYTES = 96,
...
static lxp_result reclaim_storage_head(lxp_module_ctx *ctx,const uint8_t *key,size_t key_length,
    ...
    *complete=false;s=lxp_ctx_kv_get(ctx,key,key_length,&head,&head_len);if(s==LXP_ERR_UNKNOWN_FIELD){*complete=true;return LXP_OK;}if(s!=LXP_OK)return s;
    if(head_len!=38U)return LXP_FATAL_INVARIANT;s=lxp_ctx_blob_get(ctx,head+6U,&manifest,&manifest_len);if(s!=LXP_OK)return s;
    ...
    for(i=0U;i<count;++i){uint16_t n;if(cursor+2U>manifest_len)return LXP_FATAL_INVARIANT;n=take_u16(manifest+cursor);cursor+=2U;
        ...
        if(i>=*next&&removed<CLEANUP_BLOBS_PER_ACTIVITY){*bytes+=(uint64_t)n+(uint64_t)take_u32(manifest+cursor+n+32U);s=lxp_ctx_blob_del(ctx,manifest+cursor+n);if(s!=LXP_OK)return s;++removed;++*next;}
        cursor+=(size_t)n+36U;}
    if(cursor!=manifest_len)return LXP_FATAL_INVARIANT;
    if(*next!=count)return LXP_OK;
    s=lxp_ctx_blob_del(ctx,head+6U);if(s!=LXP_OK)return s;
    s=lxp_ctx_kv_del(ctx,key,key_length);if(s==LXP_OK){*cells+=count;*bytes+=manifest_len;*complete=true;}return s;
}
...
lxp_result layerx_programs_sandbox_destroy_archive(uint64_t token,uint16_t kind,const uint8_t *bytes,uint32_t length){
    ...
    if(ctx!=active_destroy_ctx||active_destroy==NULL||kind>1U||bytes==NULL||length==0U)return LXP_ERR_AUTH_SCOPE;
...
lxp_result layerx_programs_sandbox_destroy_refund(uint64_t token,const uint8_t from[32],const uint8_t to[32],
    ...
    if(s!=LXP_OK)return s;(void)memcpy(set.legs[0].asset_id,asset,32U);set.legs[0].amount=(lxp_u128){hi,lo};
    ...
    if(s==LXP_OK)(void)memcpy(root,receipt.transfer_set_root,32U);return s;
}
...
lxp_result lxp_programs_sandbox_destroy_decode(lxp_module_ctx *ctx,
    ...
    if(status!=LXP_OK)return status; value=(sandbox_destroy_activity*)allocation;
...
lxp_result lxp_programs_sandbox_finalize_expiry_batch(lxp_module_ctx *ctx,uint64_t batch_number){
    uint8_t cursor_key[]="ex/head/v1";const uint8_t *cursor;size_t cursor_len;lxp_result status;
    if(ctx==NULL||batch_number==0U)return LXP_ERR_NON_CANONICAL;
    status=lxp_ctx_kv_get(ctx,cursor_key,sizeof(cursor_key)-1U,&cursor,&cursor_len);
    if(status==LXP_ERR_UNKNOWN_FIELD){uint8_t genesis[96]={0};(void)memcpy(genesis,"LXEX2",5U);put_u64(genesis+5U,batch_number-1U);
        return lxp_ctx_kv_put(ctx,cursor_key,sizeof(cursor_key)-1U,genesis,sizeof(genesis));}
    if(status!=LXP_OK||cursor_len!=96U||memcmp(cursor,"LXEX2",5U)!=0||take_u64(cursor+5U)+1U!=batch_number)
        return status==LXP_OK?LXP_ERR_BATCH_GAP:status;
    return layerx_programs_sandbox_sweep_host((uint64_t)(uintptr_t)ctx,batch_number,64U);
}

int32_t layerx_programs_sandbox_sweep_host(uint64_t token,uint64_t boundary,uint32_t limit){
    lxp_module_ctx *ctx=(lxp_module_ctx*)(uintptr_t)token;const lxp_module_kv_entry *due[6];size_t count=0U,i,j;
    uint8_t cursor_key[]="ex/head/v1",cursor[96]={0};lxp_result status=LXP_OK;
    if(ctx==NULL||limit==0U||limit>64U)return LXP_ERR_NON_CANONICAL;
    for(i=0U;i<ctx->kernel->module_kv_count;++i){const lxp_module_kv_entry *entry=&ctx->kernel->module_kv[i];
        if(entry->module_id!=LXP_MODULE_PROGRAMS||entry->key_length!=34U||entry->key[0]!=(uint8_t)'s'||entry->key[1]!=0U||
           entry->value_length<392U||take_u64(entry->value+383U)>boundary)continue;
        for(j=0U;j<count&&memcmp(due[j]->key+2U,entry->key+2U,32U)<0;++j){}
        if(j<6U){if(count<6U)++count;if(j+1U<count)(void)memmove(&due[j+1U],&due[j],(count-j-1U)*sizeof(due[0]));due[j]=entry;}
    }
    for(i=0U;status==LXP_OK&&i<count;++i){sandbox_destroy_activity value;lxp_activity activity;lxp_authority_resolved authority;uint8_t old_id[32],payload[DESTROY_BYTES];void *canonical=NULL;
        (void)memset(&value,0,sizeof(value));(void)memcpy(value.lease_id,due[i]->key+2U,32U);
        status=lxp_hash_sha256(due[i]->value,due[i]->value_length,value.expected_lease_root);value.expected_sequence=ctx->global_sequence;value.boundary=boundary;
        (void)memset(&activity,0,sizeof(activity));activity.activity_type=LX_PROGRAMS_SANDBOX_DESTROY;
        (void)memset(&authority,0,sizeof(authority));authority.kind=LXP_AUTHORITY_PROTOCOL_MODULE;
        payload[0]=1U;payload[1]=1U;payload[2]=0U;payload[3]=0U;(void)memcpy(payload+4U,value.lease_id,32U);(void)memcpy(payload+36U,value.expected_lease_root,32U);put_u64(payload+68U,value.expected_sequence);put_u64(payload+76U,value.boundary);
        (void)memcpy(old_id,ctx->activity_id,32U);if(status==LXP_OK)status=lxp_hash_domain(LXP_DOMAIN_CONTEXT_HASH,payload,sizeof(payload),ctx->activity_id);
        if(status==LXP_OK)status=lxp_programs_sandbox_destroy_decode(ctx,payload,sizeof(payload),&canonical);
        if(status==LXP_OK)status=lxp_programs_sandbox_destroy_execute(ctx,&activity,&authority,canonical,ctx->effects);
        (void)memcpy(ctx->activity_id,old_id,32U);
    }
    if(status==LXP_OK){uint8_t page_binding[20U+6U*32U]={0};(void)memcpy(cursor,"LXEX2",5U);put_u64(cursor+5U,boundary);put_u64(cursor+13U,ctx->global_sequence);
        if(count!=0U)(void)memcpy(cursor+21U,due[count-1U]->key+2U,32U);put_u32(cursor+53U,(uint32_t)count);
        put_u64(page_binding,boundary);put_u64(page_binding+8U,ctx->global_sequence);put_u32(page_binding+16U,(uint32_t)count);
        for(i=0U;i<count;++i)(void)memcpy(page_binding+20U+i*32U,due[i]->key+2U,32U);
        status=lxp_hash_domain(LXP_DOMAIN_CONTEXT_HASH,page_binding,20U+count*32U,cursor+57U);
        if(status!=LXP_OK)return status;
        status=lxp_ctx_kv_put(ctx,cursor_key,sizeof(cursor_key)-1U,cursor,sizeof(cursor));}
    return status;
}
