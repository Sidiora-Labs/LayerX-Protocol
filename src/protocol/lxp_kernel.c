...
lxp_result lxp_kernel_set_fee_charger(lxp_kernel *kernel,
                                      lxp_kernel_fee_charger charge_fee)
{
    if (kernel == NULL) return LXP_ERR_NON_CANONICAL;
    kernel->charge_fee = charge_fee;
    return LXP_OK;
}
...
static lxp_result receipt_state_root(const lxp_kernel *kernel,
                                     ...
                                     uint8_t root[32])
{
    uint8_t input[32U + 32U + 8U + 4U + 16U + 4U];
    size_t offset = 0U;
    size_t i;
                                       ...
                                       lxp_receipt *receipt)
{
    ...
    if (status == LXP_ERR_IDEMPOTENT_REPLAY) {
        if (prior_receipt_length == 0U ||
            prior_receipt_length > sizeof(*receipt))
            return LXP_FATAL_REPLAY_DIVERGENCE;
        (void)memset(receipt, 0, sizeof(*receipt));
        (void)memcpy(receipt, prior_receipt, prior_receipt_length);
    ...
    status = lxp_state_journal_open(kernel->state,
                                    ...
                                    kernel->journal);
    if (status != LXP_OK) return status;
    if (fee_policy.charge_fee)
        status = kernel->charge_fee == NULL ? LXP_FATAL_INVARIANT :
                 kernel->charge_fee(kernel, activity, fee_policy.fee_charged);
    if (status != LXP_OK) {
    ...
    status = receipt_state_root(kernel, receipt, receipt->resulting_state_root);
    if (status == LXP_OK)
